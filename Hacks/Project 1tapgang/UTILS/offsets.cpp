...
namespace OFFSETS
{
	...
	void InitOffsets()
	{
		...
		invalidateBoneCache = (DWORD)UTILS::FindSignature("client.dll", "80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81");
		...
		dwGlowObjectManager = UTILS::FindSignature("client.dll", "A1 ? ? ? ? A8 01 75 4B" + 0x3);
		...
		b_mCollisionGroup = UTILS::netvar_hook_manager.GetOffset("DT_BaseEntity", "m_CollisionGroup");
		m_iPrimaryReserveAmmoCount = UTILS::netvar_hook_manager.GetOffset("DT_BaseCombatWeapon", "m_iPrimaryReserveAmmoCount");
		m_bDormant = 0xED;
		animstate = 0x3914;
		m_dwBoneMatrix = 0x26A8;
		dwPlayerResource = 0x3181CD0; // 0x312CDAC
		m_MoveType = 0x25C;
		animlayer = 0x2990;
		m_hMyWeapons = 0x2DF8;
	}
