...
void interfaces::initialize() {
	...
	game_movement = reinterpret_cast<player_game_movement*>(utilities::game::capture_interface("client.dll", "GameMovement001"));
	prediction = reinterpret_cast<player_prediction*>(utilities::game::capture_interface("client.dll", "VClientPrediction001"));
	...
	render_beams = *reinterpret_cast<i_view_render_beams**>((utilities::pattern_scan(GetModuleHandleA("client.dll"), "A1 ? ? ? ? FF 10 A1 ? ? ? ? B9") + 1));
	glow_manager = (glow_manager_t *)(*(uintptr_t *)(utilities::pattern_scan(GetModuleHandleA("client.dll"), "0F 11 05 ? ? ? ? 83 C8 01") + 3));
	move_helper = **(player_move_helper***)(utilities::pattern_scan(GetModuleHandleA("client.dll"), "8B 0D ? ? ? ? 8B 45 ? 51 8B D4 89 02 8B 01") + 2); //test
