...
void CSkin::SetSkinConfig()
{
	int KnifeModelsType[14] =
	{
		...
		WEAPON_KNIFE_M9_BAYONET,WEAPON_KNIFE_TACTICAL,WEAPON_KNIFE_FALCHION,
		WEAPON_KNIFE_SURVIVAL_BOWIE,WEAPON_KNIFE_BUTTERFLY,WEAPON_KNIFE_PUSH,
		WEAPON_KNIFE_STILETTO, WEAPON_KNIFE_URSUS
	};
...
void CSkin::SetModelConfig()
{
	...
	//Failed to set custom material for 'knife_bayonet', no matching material name found on model models/weapons/w_knife_default_t.mdl
...
void Gloves_OnFrameStageNotify(ClientFrameStage_t Stage)
{
	...
	//g_Entities->GetClientEntity(hWearables[0] & 0xFFF);
	if (!Interfaces::EntityList()->GetClientEntityFromHandle((PVOID)pWareables[0]) & 0xFFF)
	{
		for (ClientClass* pClass = Interfaces::Client()->GetAllClasses(); pClass; pClass = pClass->m_pNext)
		{
		...
		*reinterpret_cast<int*>(uintptr_t(pGlove) + 0x64) = -1; // Fix console spam cant create material
		if (!pGlove)
			return;
