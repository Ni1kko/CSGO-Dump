...
ImVec2 ImGui::CalcTextSize2(const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
	ImGuiContext& g = *GImGui;
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec2 valuesize, const char* label, char* value)
{
	...
	window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(grab_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImColor(themecolor[0]*255, themecolor[1]*255, themecolor[2]*255));
	//if (isDown)
		//window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 6, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2)), 13, GetColorU32(g.ActiveId == id ? ImVec4(1.00f, 1.00f, 1.00f, 0.89f)/*Active*/ : ImVec4(1.00f, 1.00f, 1.00f, 0.89f), 0.2f));
	window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 6, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2)), 5, GetColorU32(g.ActiveId == id ? ImVec4(1.00f, 1.00f, 1.00f, 0.89f)/*Active*/ : ImVec4(1.00f, 1.00f, 1.00f, 0.89f), 0.2f));
	window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x, frame_bb.Min.y + 2), ImVec2(grab_bb.Max.x + valuesize.x, frame_bb.Min.y + 14), GetColorU32(ImVec4(0.11f, 0.10f, 0.11f, 0.00f)), 3);
