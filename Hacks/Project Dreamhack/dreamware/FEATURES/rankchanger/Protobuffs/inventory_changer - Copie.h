...
static int GetSlotID2(int definition_index);
	...
	for (int i = 0; i < objects.size(); i++)
	{
		...
		switch (object.get(SubscribedType::type_id).Int32())
		{
			case 1: // Inventory
			{
				...
				if (SETTINGS::settings.Inventarchanger.medalenabled)
					apply_medals(object);
				if (SETTINGS::settings.Inventarchanger.musickitenabled)
					apply_music_kits(object);
				if (SETTINGS::settings.Inventarchanger.enable)
...
static void apply_medals(ProtoWriter& object)
{
	uint32_t steamid = INTERFACES::g_SteamUser->GetSteamID().GetAccountID();
	...
	int i = 10000;
	if (SETTINGS::settings.Inventarchanger.medalid != 0)
		SETTINGS::settings.Inventarchanger.packets_medals.push_back(SETTINGS::settings.Inventarchanger.medalid);

	for (uint32_t MedalIndex : SETTINGS::settings.Inventarchanger.packets_medals)
	{
	...
	if (SETTINGS::settings.Inventarchanger.medalidequip)
	{
		medal.add(Field(CSOEconItem::def_index, TYPE_UINT32, (int64_t)SETTINGS::settings.Inventarchanger.medalidequip));
		medal.add(Field(CSOEconItem::inventory, TYPE_UINT32, (int64_t)i));
		medal.add(Field(CSOEconItem::id, TYPE_UINT64, (int64_t)i));
...
static void add_item(ProtoWriter& object, int index, int itemIndex, int rarity, int paintKit, int seed, float wear, std::string name)
{
	...
	// Equip new skins
	{
		...
		if (avalTeam == (int)TeamID::TEAM_SPECTATOR || avalTeam == (int)TeamID::TEAM_TERRORIST) {
			ProtoWriter equipped_state(2);
			equipped_state.add(Field(CSOEconItemEquipped::new_class, TYPE_UINT32, (int64_t)TeamID::TEAM_TERRORIST));
			equipped_state.add(Field(CSOEconItemEquipped::new_slot, TYPE_UINT32, (int64_t)GetSlotID2(itemIndex)));
			item.add(Field(CSOEconItem::equipped_state, TYPE_STRING, equipped_state.serialize()));
		}
		if (avalTeam == (int)TeamID::TEAM_SPECTATOR || avalTeam == (int)TeamID::TEAM_COUNTER_TERRORIST) {
			ProtoWriter equipped_state(2);
			equipped_state.add(Field(CSOEconItemEquipped::new_class, TYPE_UINT32, (int64_t)TeamID::TEAM_COUNTER_TERRORIST));
			equipped_state.add(Field(CSOEconItemEquipped::new_slot, TYPE_UINT32, (int64_t)GetSlotID2(itemIndex)));
			item.add(Field(CSOEconItem::equipped_state, TYPE_STRING, equipped_state.serialize()));
		}
	}
...
static int GetAvailableClassID(int definition_index)
{
	...
	case GLOVE_SPECIALIST:
		return (int)TeamID::TEAM_SPECTATOR;
	...
	case WEAPON_SAWEDOFF:
		return (int)TeamID::TEAM_TERRORIST;
	...
	case WEAPON_SCAR20:
		...
		return (int)TeamID::TEAM_COUNTER_TERRORIST;

	default:
		return (int)TeamID::TEAM_UNASSIGNED;
	}
}
