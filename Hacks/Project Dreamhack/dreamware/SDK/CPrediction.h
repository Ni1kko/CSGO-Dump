...
namespace SDK
{
	...
	{
	public:
		void set_host(SDK::CBaseEntity* host)
		{
			typedef void(__thiscall* OriginalFn)(PVOID, SDK::CBaseEntity *host);
			VMT::VMTHookManager::GetFunction<OriginalFn>(this, 1)(this, host);
		}
	};
	...
	public:
		...
		virtual void	ProcessMovement(SDK::CBaseEntity *pPlayer, CMoveData *pMove) = 0;
	...
	public:
		...
		virtual void CheckMovingGround(SDK::CBaseEntity* player, double frametime);//
		virtual void RunCommand(SDK::CBaseEntity* player, CUserCmd* cmd, IMoveHelper* moveHelper);//

		virtual void SetupMove(SDK::CBaseEntity* player, CUserCmd* cmd, IMoveHelper* pHelper, CMoveData* move);//20
		virtual void FinishMove(SDK::CBaseEntity* player, CUserCmd* cmd, CMoveData* move);//
		virtual void SetIdealPitch(int nSlot, SDK::CBaseEntity* player, const Vector& origin, const Vector& angles, const Vector& viewheight);//

		virtual void CheckError(int nSlot, SDK::CBaseEntity* player, int commands_acknowledged);//
	...
	public:
		...
		bool PerformPrediction(int nSlot, SDK::CBaseEntity* localPlayer, bool received_new_world_update, int incoming_acknowledged, int outgoing_command);
		...
		void DumpEntity(SDK::CBaseEntity* ent, int commands_acknowledged);
		...
		void RunSimulation(int current_command, float curtime, CUserCmd* cmd, SDK::CBaseEntity* localPlayer);

		void Untouch(int nSlot);

		void StorePredictionResults(int nSlot, int predicted_frame);

		bool ShouldDumpEntity(SDK::CBaseEntity* ent);

		void SmoothViewOnMovingPlatform(SDK::CBaseEntity* pPlayer, Vector& offset);

		void ResetSimulationTick();

		void ShowPredictionListEntry(int listRow, int showlist, SDK::CBaseEntity* ent, int& totalsize, int& totalsize_intermediate);
...
extern SDK::CPrediction* g_pPrediction;