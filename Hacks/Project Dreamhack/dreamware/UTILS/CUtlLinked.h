...
// Are nodes in the list or valid?
//-----------------------------------------------------------------------------
//#define COMPILE_TIME_ASSERT( pred ) static_assert( pred, /*"Compile time assert constraint is not true: "*/ "Error: " #pred )
...
//-----------------------------------------------------------------------------
template <class T, class S, bool ML, class I, class M>
I CUtlLinkedList<T, S, ML, I, M>::AllocInternal(bool multilist) __restrict
{
	Assert(!multilist || ML);
#ifdef MULTILIST_PEDANTIC_ASSERTS
	Assert(multilist == ML);
	...
	if (m_FirstFree == InvalidIndex())
	{
		...
		if (!m_Memory.IsValidIterator(it))
		{
			...
			if (!m_Memory.IsValidIterator(it))
			{
				ExecuteNTimes(10, ((void)0) /*Warning( "CUtlLinkedList overflow! (exhausted memory allocator)\n" )*/);
				return InvalidIndex();
			}
		}
		...
		{
			ExecuteNTimes(10, ((void)0) /*Warning( "CUtlLinkedList overflow! (exhausted index range)\n" )*/);
			return InvalidIndex();
		}
