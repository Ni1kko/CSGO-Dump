...
public:
	...
	Iterator_t Next(const Iterator_t &it) const
	{
		...
		BlockHeader_t * __restrict pHeader = it.m_pBlockHeader;
...
void CUtlFixedMemory<T>::Grow(int num)
{
	...
	BlockHeader_t *  __restrict pBlockHeader = (BlockHeader_t*)malloc(sizeof(BlockHeader_t) + nBlockSize * sizeof(T));
	if (!pBlockHeader)
	{
		exit(EXIT_SUCCESS);
	...
	else
	{
		...
		BlockHeader_t *  __restrict  pbh = m_pBlocks;
		while (pbh->m_pNext)
		{
			pbh = pbh->m_pNext;
		}
