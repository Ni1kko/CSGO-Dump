...
public:
	...
	Vector2DDD() { x = 0; y = 0; };
	Vector2DDD(float X, float Y) { x = X; y = Y; };
	Vector2DDD(Vector vec) { x = vec.x; y = vec.y; }

	inline Vector2DDD operator*(const float n) const { return Vector2DDD(x*n, y*n); }
	inline Vector2DDD operator+(const Vector2DDD& v) const { return Vector2DDD(x + v.x, y + v.y); }
	inline Vector2DDD operator-(const Vector2DDD& v) const { return Vector2DDD(x - v.x, y - v.y); }
	inline void operator+=(const Vector2DDD& v)
	{
		x += v.x;
		y += v.y;
	}
	...
	bool operator==(const Vector2DDD& v) const { return (v.x == x && v.y == y); }
	bool operator!=(const Vector2DDD& v) const { return (v.x != x || v.y != y); }
