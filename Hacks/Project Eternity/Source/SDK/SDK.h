...
private:
	...
	void* GetPointer3(const char* Module, const char* InterfaceName)
	{
		void* Interface = NULL;
		char PossibleInterfaceName[1024];
...
public:
	void InitialiseSDK()
	{
		CreateInterface_t ClientFactory = (CreateInterface_t)GetProcAddress((HMODULE)GetModuleHandle("client_panorama.dll"), "CreateInterface");
		...
		CreateInterface_t DataFactory = (CreateInterface_t)GetProcAddress((HMODULE)GetModuleHandle("datacache.dll"), "CreateInterface");

		pSurface = ( ISurface* )GetInterfacePtr( "VGUI_Surface", "g_pSurface", VGUISurfaceFactory);
		pPanel = (IPanel*)GetPointer3(("vgui2.dll"), ("VGUI_Panel"));
		...
		g_pMDLCache = (IMDLCache*)GetInterfacePtr("MDLCache004", "g_pMDLCache", DataFactory);
		pGlobalVars = *( CGlobalVars** )( ( ( *( PDWORD* )pClient )[ 0 ] ) + 0x1B );
		pClientMode = **(IClientModeShared***)((*(uintptr_t**)pClient)[10] + 0x5);
		pGlobalVars = ( CGlobalVars* )*( PDWORD )pGlobalVars;
		g_pGlowObjectManager = *(CGlowObjectManager**)(Utils.FindPattern("client_panorama.dll", "0F 11 05 ? ? ? ? 83 C8 01") + 3);
		Client_State = **(CClientState***)(Utils.FindPattern("engine.dll", "A1 ? ? ? ? 8B 80 ? ? ? ? C3") + 1);
		g_pMoveHelper = **reinterpret_cast<IMoveHelper***>((Utils.FindPattern("client_panorama.dll", "8B 0D ? ? ? ? 8B 46 08 68") + 0x2));
		cout << iblue << "pGlobalVars " << igreen << ": 0x" << pGlobalVars << white << endl;
		pInput = *(CInput**)(Utils.FindPattern (("client_panorama.dll"),("B9 ? ? ? ? F3 0F 11 04 24 FF 50 10")) + 0x1);
		...
		GameResources = ( DWORD )Utils.PatternSearch( "client_panorama.dll", ( PBYTE )"\x8B\x3D\x00\x00\x00\x00\x85\xFF\x0F\x84\x00\x00\x00\x00\x81\xC7", "xx????xxxx????xx", NULL, NULL ) - ( DWORD )pClient;
		...
		ViewRenderBeams = *(IViewRenderBeams**)(Utils.FindPattern(("client_panorama.dll"), ("B9 ? ? ? ? A1 ? ? ? ? FF 10 A1 ? ? ? ? B9")) + 1);
