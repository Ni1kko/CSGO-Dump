...
void c_combo::draw( ) {
	...
	g_renderer_d3d->menu->draw_text( this->items[ config->get_int( this->var_name ) ], { draw_position.x + 21, draw_position.y + 17 }, Color( 220, 220, 220 ), font_flags_d3d::font_dropshadow );
	...
	if ( menu->is_blocking( this ) ) {
		...
		for ( int i = 0; i < this->items.size( ); i++ ) {
			...
			bool is_active = config->get_int( this->var_name ) == i;
