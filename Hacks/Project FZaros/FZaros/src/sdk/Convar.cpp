...
#include "interfaces/characterset.hpp"
...
public:
	virtual bool RegisterConCommandBase(ConCommandBase *pVar)
	{
		...
		g_Interfaces->cvar->RegisterConCommand(pVar);
		return true;
	}
};
...
{
	if (!g_Interfaces->cvar || s_bRegistered)
		return;

	assert(s_nDLLIdentifier < 0);
	s_bRegistered = true;
	s_nCVarFlag = nCVarFlag;
	s_nDLLIdentifier = g_Interfaces->cvar->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
	...
	assert(s_nDLLIdentifier >= 0);
	g_Interfaces->cvar->UnregisterConCommands(s_nDLLIdentifier);
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
{
	if (g_Interfaces->cvar)
	{
		g_Interfaces->cvar->UnregisterConCommand(this);
	}
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
	...
	if (g_Interfaces->cvar)
		g_Interfaces->cvar->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
...
{
	int color = (int)cValue.a(); //rawcolor or some shit
...
{
	m_pOriginalCVar = g_Interfaces->cvar->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::SpoofedConvar(ConVar* pCVar)
{
	...
	if (!IsSpoofed() && m_pOriginalCVar)
	{
		...
		//Register it
		g_Interfaces->cvar->RegisterConCommand(m_pDummyCVar);
...
void SpoofedConvar::Restore()
{
	if (IsSpoofed())
	{
		...
		g_Interfaces->cvar->UnregisterConCommand(m_pDummyCVar);
