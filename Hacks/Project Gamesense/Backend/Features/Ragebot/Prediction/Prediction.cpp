...
void engineprediction::start() {
	...
	if (!initialized) {
		prediction_random_seed = *(int**)(Cheat::Utilities->Memory_PatternScan("client.dll", "8B 0D ? ? ? ? BA ? ? ? ? E8 ? ? ? ? 83 C4 04") + 2);
		initialized = true;
	}

	*prediction_random_seed = MD5_PseudoRandom(G::UserCmd->command_number) & 0x7FFFFFFF;

	old_cur_time = Interfaces::Globals->curtime;
	old_frame_time = Interfaces::Globals->frametime;

	Interfaces::Globals->curtime = G::LocalPlayer->GetTickBase() * Interfaces::Globals->interval_per_tick;
	...
	Interfaces::Prediction->SetupMove(G::LocalPlayer, G::UserCmd, Interfaces::MoveHelper, &move_data);
	Interfaces::GameMovement->ProcessMovement(G::LocalPlayer, &move_data);
	Interfaces::Prediction->FinishMove(G::LocalPlayer, G::UserCmd, &move_data);
}
...
void engineprediction::end() {
	...
	*prediction_random_seed = -1;
	...
	Interfaces::Globals->frametime = old_cur_time;
}