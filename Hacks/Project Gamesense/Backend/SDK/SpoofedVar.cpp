...
{
	m_pOriginalCVar = Interfaces::Var->FindVar( szCVar );
	Spoof();
}
...
SpoofedConvar::~SpoofedConvar()
{
	if ( IsSpoofed() )
	{
		...
		Interfaces::Var->UnregisterConCommand( m_pDummyCVar );
		free( m_pDummyCVar );
...
void SpoofedConvar::Spoof()
{
	if ( !IsSpoofed() && m_pOriginalCVar )
	{
		...
		sprintf_s( m_szDummyName, 128, xorstr_("d_%s"), m_szOriginalName );
		...
		//Register it
		Interfaces::Var->RegisterConCommand( m_pDummyCVar );
		//Fix xorstr_("write access violation") bullshit
		DWORD dwOld;
		VirtualProtect( ( LPVOID )m_pOriginalCVar->pszName, 128, PAGE_READWRITE, &dwOld );
		//Rename the cvar
