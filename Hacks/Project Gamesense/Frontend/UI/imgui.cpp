...
namespace ui
{
	static bool             BeginChildExWeapon(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
...
ImGuiStyle::ImGuiStyle()
{
	...
	ItemSpacing = ImVec2(8, 6);      // Horizontal and vertical spacing between widgets/lines
...
void ui::RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end, float wrap_width)
{
	...
	if (text != text_end)
	{
		window->DrawList->AddText(g.Font, g.FontSize, pos - (ImVec2(1, 1)), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, g.Style.Alpha), text, text_end, wrap_width);
		window->DrawList->AddText(g.Font, g.FontSize, pos - (ImVec2(2, 2)), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), text, text_end, wrap_width);
...
void ui::RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float rounding)
{
	...
	if (border_size > 0.0f)
	{
		window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max + ImVec2(1, 1), ImColor(0.f, 0.f, 0.f), rounding, ImDrawCornerFlags_All, border_size);
		window->DrawList->AddRect(p_min, p_max, ImColor(0.f, 0.f, 0.f), rounding, ImDrawCornerFlags_All, border_size);
	}
}
...
static bool ui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	bool ret = Begin(title, NULL, flags, border);
...
static bool ui::BeginChildExWeapon(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* parent_window = g.CurrentWindow;
	...
	if (!border)
		g.Style.ChildBorderSize = 0.0f;
	bool ret = BeginWeapon(title, NULL, flags);
	g.Style.ChildBorderSize = backup_border_size;
...
bool ui::BeginChildWeapon(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	return BeginChildExWeapon(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}
...
bool ui::BeginWeapon(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
			...
			if (!(flags & ImGuiWindowFlags_NoBackground))
			{
				if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip)) {
					...
					if (flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_ChildFrame) && !(flags & ImGuiWindowFlags_Tooltip)) {
						...
						window->DrawList->AddLine(window->Pos, window->Pos + ImVec2(window->Size.x, 0), ImColor(12 / 255.f, 12 / 255.f, 12 / 255.f, g.Style.Alpha));
						...
						window->DrawList->AddLine(window->Pos + ImVec2(10, 1), window->Pos + ImVec2(window->Size.x - 1, 1), ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha));
					}
...
bool ui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags, bool border)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	IM_ASSERT(name != NULL && name[0] != '\0');     // Window name required
			...
			if (!(flags & ImGuiWindowFlags_NoBackground))
			{
				if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip)) {
					...
					if (flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_ChildFrame) && !(flags & ImGuiWindowFlags_Tooltip)) {
						...
						ImColor defcolor = border ? ImColor(159 / 255.f, 202 / 255.f, 43 / 255.f, g.Style.Alpha) : ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha);
...
void ui::End()
{
	...
	if (window->Flags & ImGuiWindowFlags_ChildWindow && !(window->Flags & ImGuiWindowFlags_ChildFrame)) {
		window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(2, 2), window->Pos + ImVec2(window->Size.x - 6, 20),
			ImColor(24 / 255.f, 24 / 255.f, 24 / 255.f, g.Style.Alpha),
			ImColor(24 / 255.f, 24 / 255.f, 24 / 255.f, g.Style.Alpha),
			ImColor(24 / 255.f, 24 / 255.f, 24 / 255.f, 0.f),
			ImColor(24 / 255.f, 24 / 255.f, 24 / 255.f, 0.f));
		...
		window->DrawList->AddText(window->Pos + ImVec2(14, textSize.y / -2 + 2), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, g.Style.Alpha), window->Name);
		window->DrawList->AddText(window->Pos + ImVec2(13, textSize.y / -2 + 1), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), window->Name);
		PopFont();
	}
