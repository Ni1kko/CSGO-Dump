...
bool ImFontAtlasBuildWithFreeType(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    buf_bitmap_buffers.push_back((unsigned char*)ui::MemAlloc(BITMAP_BUFFERS_CHUNK_SIZE));
    ...
    for (int buf_i = 0; buf_i < buf_bitmap_buffers.Size; buf_i++)
        ui::MemFree(buf_bitmap_buffers[buf_i]);
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
...
static void* ImFreeTypeDefaultAllocFunc(size_t size, void* user_data)	{ IM_UNUSED(user_data); return ui::MemAlloc(size); }
static void  ImFreeTypeDefaultFreeFunc(void* ptr, void* user_data)	    { IM_UNUSED(user_data); ui::MemFree(ptr); }
