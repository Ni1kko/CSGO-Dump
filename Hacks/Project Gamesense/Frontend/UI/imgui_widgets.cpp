...
void ui::TabButton(const char* label, int* selected, int num, int total) {
	...
	else
	{
		...
		if (ui::GetWindowSize().y > 630) {
			...
			window->DrawList->AddRectFilled(window->Pos + ImVec2(7, 16) + ImVec2(0, (560 - 33) / total * 8 - 2), window->Pos + ImVec2(7, 16) + ImVec2(0, (560 - 16) / total * 8 - 7) + ImVec2(75, window->Size.y - 560), ImColor(12 / 255.f, 12 / 255.f, 12 / 255.f, g.Style.Alpha));
			window->DrawList->AddLine(window->Pos + ImVec2(7, 16) + ImVec2(0, (560 - 33) / total * 8 - 2) + ImVec2(size.x, -1), window->Pos + ImVec2(7, 16) + ImVec2(0, (560 - 16) / total * 8 - 7) + ImVec2(75, window->Size.y - 560), ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha));
		}
	}
...
bool ui::CheckboxYellow(const char* label, bool* v)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (*v)
		window->DrawList->AddRectFilled(pos + ImVec2(2, 2), pos + ImVec2(10, 10), c_menu::get()->get_accent_color());
	else {
		if (hovered)
	...
	window->DrawList->AddText(pos + ImVec2(15, 0), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, g.Style.Alpha), label);
	window->DrawList->AddText(pos + ImVec2(14, -1), ImColor(180 / 255.f, 180 / 255.f, 100 / 255.f, g.Style.Alpha), label);
...
bool ui::MultiSelect(const char* label, std::unordered_map<int, bool>* data, std::vector<const char*> items) {
	...
	if (ui::BeginCombo(label, (howmuchsel(*data) > 0 ? std::string(whatsel(*data, items)) : "-").c_str(), 0, items.size())) {
		for (int i = 0; i < items.size(); i++)
...
bool ui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt)
{
	...
	window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), c_menu::get()->get_accent_color());
...
bool SliderPower(const char* label, const ImVec2& size_arg)
{
	...
	const ImRect bb(pos, { pos.x + size.x , pos.y + 10 + size.y });
	...
	window->DrawList->AddText({ bb.Min.x + 1,bb.Min.y + 14 }, ImColor(0, 0, 0, 255), label);
	window->DrawList->AddText({ bb.Min.x+1,bb.Min.y + 13 }, ImColor(90, 90, 90, 255), label);
...
/*void SliderInt(const char* label, int* v, int v_min, int v_max, const char* format)
{
	ui::PushItemWidth(144);
	Spacing(4);
	ui::Spacing();
	if (*v != v_min)
	{
		ui::SameLine(20);
		if (SliderPower("-", ImVec2{ 10,10 }))
			*v = *v - 1;
	}
	ui::SameLine(36);
	ui::SliderInt7(label, v, v_min, v_max, format);
	if (*v != v_max)
	{
		ui::SameLine(36 + 144);
		if (SliderPower("+", ImVec2{ 10,10 }))
...
bool ui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		SetNextWindowSize(ImVec2(214, 194));
...
// Helper for ColorPicker4()
static void RenderArrowsForHorizontalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float picker_size, float pickerpos)
{
	draw_list->AddRect(ImVec2(pos.x, ImClamp(pos.y, pickerpos + 2, pickerpos + picker_size - 2)) - ImVec2(2, 0), ImVec2(pos.x, ImClamp(pos.y, pickerpos + 2, pickerpos + picker_size - 2)) + ImVec2(2, 15), ImColor(0.f, 0.f, 0.f, 1.f));
}

static void RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float picker_size, float pickerpos)
{
	draw_list->AddRect(ImVec2(pos.x, ImClamp(pos.y, pickerpos + 2, pickerpos + picker_size - 2)) - ImVec2(0, 2), ImVec2(pos.x, ImClamp(pos.y, pickerpos + 2, pickerpos + picker_size - 2)) + ImVec2(15, 2), ImColor(0.f, 0.f, 0.f, 1.f));
}
...
bool ui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (alpha_bar)
	{
		SetCursorScreenPos(ImVec2(picker_pos.x, picker_pos.y + sv_picker_size + 4));
		InvisibleButton("alpha", ImVec2(sv_picker_size, 11));
		if (IsItemActive())
		{
			col[3] = 1.0f - ImSaturate((sv_picker_size - (io.MousePos.x - picker_pos.x)) / (sv_picker_size - 1));
			value_changed = true;
		}
	}
	...
	ImU32 col32_alpha = ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], col[3]));
	...
	else if (flags & ImGuiColorEditFlags_PickerHueBar)
	{
		...
		// Render Hue Bar
		for (int i = 0; i < 6; ++i)
			draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width - 5, picker_pos.y + (i + 1) * (sv_picker_size / 6)), hue_colors[i], hue_colors[i], hue_colors[i + 1], hue_colors[i + 1]);
		float bar0_line_y = (float)(int)(picker_pos.y + H * sv_picker_size + 0.5f);
		//RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 0.0f);
		draw_list->AddRect(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width - 5, picker_pos.y + sv_picker_size), ImColor(0.f, 0.f, 0.f, 1.f));
		RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, sv_picker_size, picker_pos.y);
	}
	...
	if (alpha_bar)
	{
		...
		ImRect bar1_bb(picker_pos.x, picker_pos.y + sv_picker_size + 4, picker_pos.x + sv_picker_size, picker_pos.y + sv_picker_size + 15);
		//RenderColorRectWithAlphaCheckerboard(bar1_bb.Min - ImVec2(5, 0), bar1_bb.Max - ImVec2(10, 0), IM_COL32(0, 0, 0, 0), bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f));
		draw_list->AddRectFilled(bar1_bb.Min, bar1_bb.Max, col32_alpha);
		float bar1_line_y = (float)(int)(picker_pos.y + (1.0f - alpha) * sv_picker_size + 0.5f);
		//RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 0.0f);
		draw_list->AddRect(bar1_bb.Min, bar1_bb.Max, ImColor(0.f, 0.f, 0.f, 1.f));
		RenderArrowsForHorizontalBar(draw_list, ImVec2(bar1_pos_x - 5 - 1, bar1_line_y), ImVec2(bars_triangles_half_sz - 10 + 1, bars_triangles_half_sz), bars_width + 2.0f, sv_picker_size, bar1_bb.Min.x);
	}
...
bool ui::ColorButton(const char* desc_id, ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	else
	{
		...
		window->DrawList->AddRectFilledMultiColor(bb_inner.Min, bb_inner.Max,
			ImColor(1.f, 1.f, 1.f, min(0.1f, g.Style.Alpha)), ImColor(1.f, 1.f, 1.f, min(0.1f, g.Style.Alpha)),
			ImColor(0.f, 0.f, 0.f, min(0.1f, g.Style.Alpha)), ImColor(0.f, 0.f, 0.f, min(0.1f, g.Style.Alpha)));


	}
	window->DrawList->AddRect(bb_inner.Min - ImVec2(1, 1), bb_inner.Max + ImVec2(1, 1), ImColor(10 / 255.f, 10 / 255.f, 10 / 255.f, g.Style.Alpha));
	...
	if (popup_open) {
		...
		if (Selectable("Copy")) {
			clip::set_text((rgb2hex((int)(col.x * 255.f), (int)(col.y * 255.f), (int)(col.z * 255.f), (int)(col.w * 255.f))));
			/*clip::set_text(
...
bool ui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	window->DrawList->PushClipRect(bb.Min, bb.Max, true);
	window->DrawList->AddText(bb.Min + ImVec2(8, size.y / 2 - label_size.y / 2 + 2), selected ? c_menu::get()->get_accent_color() : ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), label);
	window->DrawList->PopClipRect();
...
bool ui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
	...
	ImVec2 frame_size = ImVec2(196, ImMax(size.y, label_size.y));
