...
bool ImGui::ButtonExT( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border, ImColor clr ) {
	...
	if ( page == in ) {
		...
		RenderTextClipped( bb.Min + style.FramePadding - ImVec2( 0, 1 ), bb.Max - style.FramePadding - ImVec2( 0, 1 ), label, NULL, &label_size, ImVec2( 0.5, 0.5 ), &bb );
		PopStyleColor( );
...
bool ImGui::Checkbox( const char* label, bool* v ) {
	...
	window->DrawList->AddRectFilled( check_bb.Min + ImVec2( pad - 1, pad - 1 ), check_bb.Max - ImVec2( pad - 1, pad - 1 ), GetColorU32( ImGuiCol_Border ), style.FrameRounding );
	window->DrawList->AddRectFilled( check_bb.Min + ImVec2( pad, pad ), check_bb.Max - ImVec2( pad, pad ), GetColorU32( ImGuiCol_FrameBg ), style.FrameRounding );
