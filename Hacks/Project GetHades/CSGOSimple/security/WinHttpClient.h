...
public:
    ...
    // It is a synchronized current_method and may take a long time to finish.
    ...
    inline const BYTE* GetRawResponseContent ( void ) const;
    inline unsigned int GetRawResponseContentLength ( void ) const;
    inline unsigned int GetRawResponseReceivedContentLength ( void ) const;
    ...
    inline DWORD GetLastError ( void ) const;
...
private:
    ...
    inline bool SetProgress ( unsigned int byteCountReceived ) const;
        ...
        if ( hConnect != NULL )
        {
            ...
            if ( hRequest != NULL )
            {
                ...
                while ( !bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES )
                {
                    ...
                    {
                        if ( !::WinHttpAddRequestHeaders ( hRequest, m_additionalRequestHeaders.c_str( ), m_additionalRequestHeaders.size( ),
                    ...
                    if ( m_proxy.size( ) > 0 )
                    {
                        ...
                        {
                            if ( !::WinHttpSetOption ( hRequest, WINHTTP_OPTION_PROXY_USERNAME, ( LPVOID )m_proxyUsername.c_str( ),
                                                       m_proxyUsername.size( ) * sizeof ( wchar_t ) ) )
                                m_dwLastError = ::GetLastError( );

                            if ( m_proxyPassword.size( ) > 0 )
                            {
                                if ( !::WinHttpSetOption ( hRequest, WINHTTP_OPTION_PROXY_PASSWORD, ( LPVOID )m_proxyPassword.c_str( ),
                                                           m_proxyPassword.size( ) * sizeof ( wchar_t ) ) )
                                    m_dwLastError = ::GetLastError( );
                            }
                        }
                    }
...
bool WinHttpClient::SetProgress ( unsigned int byteCountReceived ) const
{
    bool bReturn = false;
...
const BYTE* WinHttpClient::GetRawResponseContent ( void ) const
{
    return m_pResponse;
}

unsigned int WinHttpClient::GetRawResponseContentLength ( void ) const
{
    return m_responseByteCount;
}

unsigned int WinHttpClient::GetRawResponseReceivedContentLength ( void ) const
{
    return m_responseByteCountReceived;
}

DWORD WinHttpClient::GetLastError ( void ) const
{
