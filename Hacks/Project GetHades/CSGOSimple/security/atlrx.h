...
#ifdef _RELEASE
        void GetMatch ( UINT nIndex, const wchar_t** szStart, const wchar_t** szEnd )
        {
            GetMatch ( nIndex, ( const RECHAR** )szStart, ( const RECHAR** )szEnd );
        }
#endif // DEBUG
...
#ifdef _RELEASE
        REParseError Parse ( const wchar_t* szRE, BOOL bCaseSensitive = TRUE )
        {
            return Parse ( ( const RECHAR* )szRE, bCaseSensitive );
        }
#endif
...
#ifdef _RELEASE
        BOOL Match ( const wchar_t* szIn, CAtlREMatchContext<CharTraits>* pContext, const wchar_t** ppszEnd = NULL )
        {
            return Match ( ( const RECHAR* )szIn, pContext, ( const RECHAR** )ppszEnd );
        }
#endif
                ...
                case RE_PREVIOUS:
                    {
                        ...
                        if ( m_bCaseSensitive )
                        {
                            bMatch = !CharTraits::Strncmp ( sz, pContext->m_Matches[ GetInstruction ( ip ).prev.nGroup ].szStart,
                                                            pContext->m_Matches[ GetInstruction ( ip ).prev.nGroup ].szEnd - pContext->m_Matches[
                                                                GetInstruction ( ip ).prev.nGroup ].szStart );
                        }
                        else
                        {
                        ...
                        if ( bMatch )
                        {
                            sz += pContext->m_Matches[ GetInstruction ( ip ).prev.nGroup ].szEnd - pContext->m_Matches[ GetInstruction ( ip ).prev.nGroup ].
                                szStart;
                            ip++;
                            break;
                        }
    ...
    protected:
        ...
        int ParseSE ( const RECHAR** ppszRE, bool& bEmpty )
        {
            ...
            if ( MatchToken ( ppszRE, '\\' ) )
            {
                ...
                {
                    int p = ParseAbbrev ( ppszRE, bEmpty );
        ...
        //This function fix these pointers to point into the original, user supplied buffer (first param to Match current_method).
