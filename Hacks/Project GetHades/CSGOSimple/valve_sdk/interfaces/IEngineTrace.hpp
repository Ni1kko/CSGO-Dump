...
//-----------------------------------------------------------------------------
// Classes are expected to inherit these + implement the ShouldHitEntity current_method
//-----------------------------------------------------------------------------
...
public:
    ...
    bool ShouldHitEntity ( IHandleEntity* pEntityHandle, int /*contentsMask*/ )
    {
        return pEntityHandle != pEnt && ( ( IClientEntity* )pEntityHandle )->get_client_class( )->class_id == CCSPlayer;
    }
...
{
public:
    bool IsDispSurface ( void ) const
    {
        return ( ( dispFlags & DISPSURF_FLAG_SURFACE ) != 0 );
    }

    bool IsDispSurfaceWalkable ( void ) const
    {
        return ( ( dispFlags & DISPSURF_FLAG_WALKABLE ) != 0 );
    }

    bool IsDispSurfaceBuildable ( void ) const
    {
        return ( ( dispFlags & DISPSURF_FLAG_BUILDABLE ) != 0 );
    }

    bool IsDispSurfaceProp1 ( void ) const
    {
        return ( ( dispFlags & DISPSURF_FLAG_SURFPROP1 ) != 0 );
    }

    bool IsDispSurfaceProp2 ( void ) const
    {
...
public:
    ...
    virtual int get_coint_contents_world_only ( const Vector& vecAbsPosition, int contentsMask = MASK_ALL ) = 0;
