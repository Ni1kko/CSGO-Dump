...
struct ModelRenderInfo_t
{
    ...
    QAngle angles;
    char pad[4]{ };
    IClientRenderable* p_renderable{ };
    const model_t* p_model{ };
    const matrix3x4_t* p_model_to_world;
    const matrix3x4_t* p_lighting_offset;
    const Vector* p_lighting_origin;
    int flags{ };
    int entity_index{ };
    int skin{ };
    int body{ };
    ...
    ModelRenderInfo_t ( )
    {
        p_model_to_world = nullptr;
        p_lighting_offset = nullptr;
        p_lighting_origin = nullptr;
    }
};
...
{
public:
    virtual int draw_model ( int flags, IClientRenderable* pRenderable, ModelInstanceHandle_t instance, int entity_index, const model_t* model,
                             Vector const& origin, QAngle const& angles, int skin, int body, int hitboxset, const matrix3x4_t* modelToWorld = nullptr,
                             const matrix3x4_t* p_lighting_offset = nullptr ) = 0;
    virtual void forced_material_override ( IMaterial* newMaterial, OverrideType_t nOverrideType = 0, int nOverrides = 0 ) = 0;
    virtual bool is_forced_material_override ( void ) = 0;
    virtual void set_view_target ( const CStudioHdr* pStudioHdr, int nBodyIndex, const Vector& target ) = 0;
    ...
    virtual void AddDecal ( ModelInstanceHandle_t handle, Ray_t const& ray, Vector const& decalUp, int decalIndex, int body, bool noPokeThru,
                            int maxLODToDecal ) = 0;
    virtual void RemoveAllDecals ( ModelInstanceHandle_t handle ) = 0;
    virtual bool ModelHasDecals ( ModelInstanceHandle_t handle ) = 0;
    virtual void RemoveAllDecalsFromAllModels ( ) = 0;
    virtual matrix3x4_t* draw_model_shadow_setup ( IClientRenderable* p_renderable, int body, int skin, DrawModelInfo_t* pInfo,
                                                   matrix3x4_t* pCustomBoneToWorld = nullptr ) = 0;
    virtual void draw_model_shadow ( IClientRenderable* pRenderable, const DrawModelInfo_t& info, matrix3x4_t* pCustomBoneToWorld = nullptr ) = 0;
    virtual bool RecomputeStaticLighting ( ModelInstanceHandle_t handle ) = 0;
    virtual void ReleaseAllStaticPropColorData ( void ) = 0;
    virtual void RestoreAllStaticPropColorData ( void ) = 0;
    virtual int draw_model_ex ( ModelRenderInfo_t& pInfo ) = 0;
    virtual int draw_model_ex_static_prop ( ModelRenderInfo_t& pInfo ) = 0;
    virtual bool draw_model_setup ( ModelRenderInfo_t& pInfo, DrawModelState_t* pState, matrix3x4_t** ppBoneToWorldOut ) = 0;
    virtual void draw_model_execute ( IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& pInfo,
                                      matrix3x4_t* pCustomBoneToWorld = nullptr ) = 0;
    ...
    virtual void ComputeStaticLightingState ( int nCount, const StaticLightingQuery_t* pQuery, MaterialLightingState_t* pState,
                                              MaterialLightingState_t* pDecalState, ColorMeshInfo_t** ppStaticLighting, ITexture** ppEnvCubemapTexture,
