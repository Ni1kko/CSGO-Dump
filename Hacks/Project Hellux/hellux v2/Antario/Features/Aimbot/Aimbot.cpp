...
bool ShouldBaim(C_BaseEntity* pEnt) // probably dosnt make sense
{
	...
	/*if (GetAsyncKeyState(c_config::get().iBodyKey))
	{
		std::vector<int> hitbox_array;

		hitbox_array.push_back(HITBOX_PELVIS);
		hitbox_array.push_back(HITBOX_THORAX);
		hitbox_array.push_back(HITBOX_LOWER_CHEST);
		hitbox_array.push_back(HITBOX_UPPER_CHEST);
	}*/
