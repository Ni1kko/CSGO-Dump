...
void Real()
{
	...
	if (c_config::get().desync == 5) //static
	{
		...
		//if (next_lby_update(Globals::pCmd) && !Globals::bSendPacket)
		if (next_lby_update(Globals::pCmd->viewangles.y, Globals::pCmd) && !Globals::bSendPacket)
			Globals::pCmd->viewangles.y -= 100;
		else
			Globals::pCmd->viewangles.y += sFlip ? -135 : 135;
	}
