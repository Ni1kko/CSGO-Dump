...
void ImGui::NewFrame()
{
	...
	g.IO.Frametime = g.FramerateSecPerFrameAccum;
...
static bool BeginChildExNew(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	if (extra_flags == ImGuiWindowFlags_ChildZeus)
	{
		...
		char childname[65];
		sprintf_s(childname, 65, "%s", name);
		window->DrawList->AddText(child_bb + ImVec2(5, -14), ImColor(255, 255, 255, 255), childname);
	}
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
	...
	{ int current = g.ColorModifiers.Size;      if (write) *p_backup = current; }
	{ int current = g.StyleModifiers.Size;      if (write) *p_backup = current; }
	{ int current = g.FontStack.Size;           if (write) *p_backup = current; }
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		else if (flags & ImGuiWindowFlags_ChildMenu)
		{
			...
			window->PosFloat = FindBestPopupWindowPos(window->PosFloat, window->Size, (int*)&window->AutoPosLastDirection, rect_to_avoid);
		}
		...
		if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api)
		{
			...
			window->PosFloat = FindBestPopupWindowPos(g.IO.MousePos, window->Size, (int*)&window->AutoPosLastDirection, rect_to_avoid);
...
bool ImGui::IsClippedExxd(const ImRect& bb, ImGuiID id, bool clip_even_when_logged)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	if (!bb.Overlaps(window->ClipRect))
...
bool ImGui::ItemAddxd(const ImRect& bb, ImGuiID id)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	const bool is_clipped = IsClippedExxd(bb, id, false);
...
bool ImGui::Lined(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	IM_ASSERT(name != NULL);                        // Window name required
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	const ImU32 col = GetColorU32(ImColor(202, 45, 100, 255));
...
bool ImGui::MyButtonExXDIco(const char* label, const ImVec2& size_arg, bool inuse, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (inuse)
	{
		/*window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y - 3), ImVec2(bb.Max.x, bb.Max.y - 3), ImColor(161, 31, 73, 180));

		window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 2), ImVec2(bb.Max.x, bb.Max.y + 1), ImColor(252, 35, 87, 255));

		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y + 2), ImVec2(bb.Max.x, bb.Max.y + 2), ImColor(161, 31, 73, 180));*/
		window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Max.y - 3), ImVec2(bb.Max.x, bb.Max.y - 1), GetColorU32(ImColor(252, 35, 87, 240)), GetColorU32(ImColor(152, 35, 87, 240)), GetColorU32(ImColor(152, 35, 87, 240)), GetColorU32(ImColor(252, 35, 87, 240)));
...
bool ImGui::MyButtonIco(const char* label, bool inuse, const ImVec2& size_arg)
{
	return MyButtonExXDIco(label, size_arg, inuse, 0);
}
...
bool ImGui::MyButtonExXD(const char* label, const ImVec2& size_arg, bool inuse, ImGuiButtonFlags flags)
{
	...
	if (inuse)
	{
		...
		RenderTextClipped(bb.Min + style.FramePadding - ImVec2(0.f, 3.f), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::MyButton2(const char* label, bool inuse, const ImVec2& size_arg)
{
	return MyButtonExXD2(label, size_arg, inuse, 0);
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	// Draw frame
	//frame_bb.Min + ImVec2(7.f, 6.f), bb_slider.Max + ImVec2(-5.f, -6.f)
	const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBg : g.HoveredId == id ? ImGuiCol_FrameBg : ImGuiCol_FrameBg);
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(3.5f, 3.f), frame_bb.Max + ImVec2(-2.5f, -3.f), GetColorU32(ImColor(28, 23, 64, 240)), GetColorU32(ImColor(28, 23, 64, 240)), GetColorU32(ImColor(32, 27, 67, 240)), GetColorU32(ImColor(32, 27, 67, 240)));
	//window->DrawList->AddRect(frame_bb.Min + ImVec2(3.5f, 3.f), frame_bb.Max + ImVec2(-2.5f, -3.f), GetColorU32(ImVec4(0.05f, 0.05f, 0.05f, 0.05f)), 0, 15, 1.2f);
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(7.f, 6.f), bb_slider.Max + ImVec2(-5.f, -3.f), GetColorU32(ImColor(202, 45, 100, 240)), GetColorU32(ImColor(202, 45, 100, 240)), GetColorU32(ImColor(172, 39, 86, 240)), GetColorU32(ImColor(172, 39, 86, 240)));
	PopClipRect();

	//window->DrawList->AddRectFilledMultiColor(grab_bb.Min, grab_bb.Max, GetColorU32(ImGuiCol_MainTheme2), GetColorU32(ImGuiCol_MainTheme2), GetColorU32(ImGuiCol_MainTheme), GetColorU32(ImGuiCol_MainTheme));
	//window->DrawList->AddRect(grab_bb.Min, grab_bb.Max, GetColorU32(ImVec4(0.05f, 0.05f, 0.05f, 1.f)), 0, 15, 1.2f);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	if (hovered)
		window->DrawList->AddRect(frame_bb.Min + ImVec2(3.5f, 3.f), frame_bb.Max + ImVec2(-2.5f, 0.f), GetColorU32(ImColor(172, 39, 86, 240)), 0, 15, 0);
	else
		window->DrawList->AddRect(frame_bb.Min + ImVec2(3.5f, 3.f), frame_bb.Max + ImVec2(-2.5f, 0.f), GetColorU32(ImColor(59, 54, 93, 240)), 0, 15, 0);	/*Обводка*/
...
bool ImGui::Inputint9(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
void ImGui::TreeNode(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	float freq = 1.f / 0.5f;
	float step = freq * GetIO().Frametime;

	//window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(0.13f, 0.13f, 0.13f, 1.f)), GetColorU32(ImVec4(0.13f, 0.13f, 0.13f, 1.f)), GetColorU32(ImVec4(0.07f, 0.07f, 0.07f, 1.f)), GetColorU32(ImVec4(0.07f, 0.07f, 0.07f, 1.f)));

	if (*v)
	{

		//window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(3.5f, 3), check_bb.Max + ImVec2(-2.5f, -3.f), GetColorU32(ImGuiCol_MainTheme), GetColorU32(ImGuiCol_MainTheme2), GetColorU32(ImGuiCol_MainTheme), GetColorU32(ImGuiCol_MainTheme2));
		//window->DrawList->AddCircle(check_bb.GetCenter() + ImVec2(0.f, 1.f), 7, GetColorU32(ImColor(162, 69, 113, 255)));
		//window->DrawList->AddCircleFilled(check_bb.GetCenter() + ImVec2(0.f, 1.f), 1, GetColorU32(ImGuiCol_MainTheme));

		window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImColor(119, 36, 82, 255)));


		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(2.f, 2.f), check_bb.Max + ImVec2(-2.f, -2.f), GetColorU32(ImColor(202, 45, 100, 255)), GetColorU32(ImColor(202, 45, 100, 255)), GetColorU32(ImColor(172, 39, 86, 255)), GetColorU32(ImColor(172, 39, 86, 255)));

	}
	else
		//window->DrawList->AddCircle(check_bb.GetCenter() + ImVec2(0.f, 1.f), 7, GetColorU32(ImColor(108, 103, 144, 255)));
	{

		window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImColor(59, 54, 93, 240)));
		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1.f, 1.f), check_bb.Max + ImVec2(-1.f, -1.f), GetColorU32(ImColor(28, 23, 64, 240)), GetColorU32(ImColor(28, 23, 64, 240)), GetColorU32(ImColor(32, 27, 67, 240)), GetColorU32(ImColor(32, 27, 67, 240)));
	}

	//window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(0.05f, 0.05f, 0.05f, 0.f)), 0, 15, 0.8f);
...
void ImGui::OpenPopupExNew(ImGuiID id, bool reopen_existing)
{
	ImGuiContext& g = *GImGui;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float custom_h, ImGuiComboFlags flags)
{
	// Always consume the SetNextWindowSizeConstraint() call in our early return paths
	ImGuiContext& g = *GImGui;
	ImGuiCond backup_next_window_size_constraint = g.SetNextWindowSizeConstraint;
	g.SetNextWindowSizeConstraint = 0;
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + (style.FramePadding.y + custom_h) * 2.0f));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, &id))
		return false;
	...
	static bool b_cfg_combo; b_cfg_combo = !strcmp("##configselect", label);

	if (b_cfg_combo) {
		window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, ImColor(40, 40, 40), ImColor(40, 40, 40), ImColor(0, 0, 0), ImColor(0, 0, 0));
		window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(0, 0, 0));
	}
	else {
		RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
		RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(popup_open || hovered ? ImGuiCol_FrameBg : ImGuiCol_FrameBg), true, style.FrameRounding); // FIXME-ROUNDING
	}
	if (b_cfg_combo)
		RenderTriangle(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y - 2.f, frame_bb.Min.y + style.FramePadding.y + custom_h), ImGuiDir_Down);
	else
		RenderTriangle(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + custom_h), ImGuiDir_Down);

	if (preview_value != NULL)
		RenderTextClipped(frame_bb.Min + style.FramePadding - ImVec2(-1.f, 1.f - custom_h), value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y + custom_h), label);

	if (pressed && !popup_open)
	{
		OpenPopupExNew(id, false);
		popup_open = true;
	}
	...
	{
		g.SetNextWindowSizeConstraint = backup_next_window_size_constraint;
	...
	if (ImGuiWindow* popup_window = FindWindowByName(name))
		if (popup_window->WasActive)
		{
			...
			ImVec2 pos = FindBestWindowPosForPopup(frame_bb.GetBL(), size_expected, (ImGuiDir*)&popup_window->AutoPosLastDirection, frame_bb, ImGuiPopupPositionPolicy_ComboBox);
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	if (popup_open || hovered)
	{
		...
		window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImColor(172, 39, 86, 255)));
	}
	else
	{
		window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImColor(59, 54, 93, 240)));
		
	}
	//window->DrawList->AddRect(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 1), ImVec2(frame_bb.Max.x - 1, frame_bb.Max.y - 1), GetColorU32(ImColor(172, 39, 86, 240)));
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(popup_open || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding); // FIXME-ROUNDINGNDING
	window->DrawList->AddRectFilledMultiColor(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(ImColor(202, 45, 100, 240)), GetColorU32(ImColor(202, 45, 100, 240)), GetColorU32(ImColor(172, 39, 86, 240)), GetColorU32(ImColor(172, 39, 86, 240)));
	...
	if (*current_item >= 0 && *current_item < items_count)
	{
		...
		if (items_getter(data, *current_item, &item_text))
			RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(1.f, -1.f), ImVec2(value_bb.Max.x, value_bb.Max.y) + ImVec2(1.f, -1.f), item_text, NULL, NULL, ImVec2(0.0f, -0.f));
	}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (flags & ImGuiSelectableFlags_Disabled) PushStyleColor(ImGuiCol_MainTheme, g.Style.Colors[ImGuiCol_TextDisabled]);
	if (selected)
		ImGui::PushStyleColor(ImGuiCol_Text, ImColor(202, 45, 100, 230));
	else
		ImGui::PushStyleColor(ImGuiCol_Text, (held && hovered) ? ImColor(202, 45, 100, 230) : hovered ? ImColor(255, 255, 255, 230) : ImColor(177, 175, 215, 230));
...
void ImGui::Separator(bool rgb, bool orange)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	float flSpeed = 0.0001f;
	/*
	209 160 46 - светложёлтый
	209 137 35 - средний между жёлтым и оранжевым
	209 114 25 - потемнее
	25 32 38 - фон
	185 190 190 - цвет текста
	220 230 230 - цвет окна
	*/
	//window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Border)); 
	if (rgb)
	{
		if (!orange)
			window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(bb.Max.x, bb.Min.y + 2), ImColor(209, 114, 25, 255), ImColor(209, 160, 46, 255), ImColor(209, 114, 25, 255), ImColor(209, 160, 46, 255)); //DrawRectRainbow(bb.Min.x, bb.Min.y, bb.Max.x, bb.Min.y + 1, flSpeed, flRainbow); 
		else
			window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(bb.Max.x, bb.Min.y + 2), ImColor(89, 45, 137, 255), ImColor(150, 24, 71, 255), ImColor(150, 24, 71, 255), ImColor(89, 45, 137, 255));
	}
	else
		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Border));
