...
// a key identifier (ImGui-side enum)   // enum ImGuiKey_
...
namespace ImGui
{
	...
	IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, float custom_h, ImGuiComboFlags flags = 0);
	...
	IMGUI_API void          StageFix(IDirect3DDevice9* fix);
	IMGUI_API void          ClearFix(IDirect3DDevice9 * fix);
	IMGUI_API void          FinalFix(IDirect3DDevice9* fix);
	IMGUI_API void          DestroyFix();
	...
	// Cursor / Layout
	IMGUI_API void          Separator(bool rgb = false, bool orange = false);                                                        // horizontal line
	...
	IMGUI_API bool          Lined(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha = -1.0f, ImGuiWindowFlags flags = 0); // OBSOLETE. this is the older/longer API. the extra parameters aren't very relevant. call SetNextWindowSize() instead if you want to set a window size. For regular windows, 'size_on_first_use' only applies to the first time EVER the window is created and probably not what you want! might obsolete this API eventually.
	...
	IMGUI_API bool MyButtonExXDIco(const char * label, const ImVec2 & size_arg, bool inuse, ImGuiButtonFlags flags);
	IMGUI_API bool MyButtonIco(const char * label, bool inuse, const ImVec2 & size_arg);
	IMGUI_API bool MyButtonExXD2(const char * label, const ImVec2 & size_arg, bool inuse, ImGuiButtonFlags flags);
	IMGUI_API bool MyButton2(const char * label, bool inuse, const ImVec2 & size_arg);
	...
	IMGUI_API bool          Inputint9(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), int frame_padding = -1, const ImVec4& bg_col = ImVec4(0, 0, 0, 0), const ImVec4& tint_col = ImVec4(1, 1, 1, 1));  
	...
	IMGUI_API void          TreeNode(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), const ImVec4& tint_col = ImVec4(1, 1, 1, 1), const ImVec4& border_col = ImVec4(0, 0, 0, 0));
	...
	IMGUI_API bool Combo(const char * label, int * current_item, std::function<const char*(int)> lambda, int items_count, int height_in_items);
...
enum ImGuiCol_
{
	...
	ImGuiCol_WindowBg,  
	ImGuiCol_MainMenu,// Background of normal windows
	ImGuiCol_MainTheme,// Background of normal windows
	ImGuiCol_MainTheme2,// Background of normal windows
