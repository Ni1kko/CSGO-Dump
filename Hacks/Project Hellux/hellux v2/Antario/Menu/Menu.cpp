...
void DrawAimbotAccuracy_Subtab(int& current_players_esp_subtab, int tab_amount, Vector _pos, int MenuAlpha_Main)
{
	...
	for (int i = 0; i < tab_amount; i++)
	{
		...
		int tab_area[4] = {
			_pos.x + TopLeftGroupBox_X + GroupBoxSize_Width + 25 + 30 + (i * (in_sizew_esp_player_subtabs / tab_amount)),
			_pos.y + 30 + 15, (in_sizew_esp_player_subtabs / tab_amount), 20
		};
...
{
	int in_sizew_esp_player_subtabs = GroupBoxSize_Width - 8;
	static std::string ESP_Player_SubTabs_Names[4] = { "Standing", "Moving", "Air" };
	...
	for (int i = 0; i < tab_amount; i++)
	{
		...
		int tab_area[4] = {
			_pos.x + 15 + (i * (in_sizew_esp_player_subtabs / tab_amount)), _pos.y + 30 + 15,
			(in_sizew_esp_player_subtabs / tab_amount), 20
		};
...
void Menu::Render()
{
	...
	int _width =789; // 750
	int _height = 651; // 650
	int _width_menu2 = 534; // 514
	int _height_menu2 = 478; // 473
	...
	if (menuOpened)
	{
		...
		if (g_pSurface->MouseInRegion(_pos.x, _pos.y - 80, _width, _height_menu2 - 410) && !(g_pSurface->MouseInRegion(
			_pos.x, _pos.y, _width, _height)))
		{
			_dragging = true;
	...
	if (menuOpened) {
		...
		g_pSurface->FilledRect(_pos.x, _pos.y, _width, _height, Color(26, 25, 25, MenuAlpha_Main));
		g_pSurface->FilledRect(_pos.x, _pos.y - 80, _width, _height_menu2 - 410, Color(28, 27, 27, MenuAlpha_Main));
		GradientH(_pos.x, _pos.y - 80, _width, 5, Color(c_config::get().menu_color_r, c_config::get().menu_color_g, c_config::get().menu_color_b, MenuAlpha_Main), Color(c_config::get().menu_color_r, c_config::get().menu_color_g, c_config::get().menu_color_b, MenuAlpha_Main));
	}
	else {
	...
	g_pSurface->OutlinedRect(_pos.x, _pos.y, _width, _height, Color(15, 127, 255, MenuAlpha_Main));
	//g_pSurface->OutlinedRect(_pos.x, _pos.y + 19, _width, 1, Color(52,73,94, MenuAlpha_Main));

	RECT TitleText_Size = g_pSurface->GetTextSizeRect(Globals::CourierNew, "HELLUX");

	g_pSurface->DrawT(_pos.x + 5, _pos.y + (20 / 2) - (TitleText_Size.bottom + 65), Color(255, 255, 255, MenuAlpha_Text), Globals::CourierNew, false, "<< HELLUX.XYZ >>");
	g_pSurface->DrawT(_pos.x + 5, _pos.y + (16 / 2) - (TitleText_Size.bottom + 40), Color(30, 122, 250, MenuAlpha_Text), Globals::CourierNew, false, " Blue Hell. <3");
	...
	g_pSurface->DrawT(_pos.x + 10, _pos.y + (20 / 2) - (TitleText_Size.bottom + 30), Color(255, 255, 255, MenuAlpha_Text), Globals::CourierNew, false, MenuWatermark.c_str());


	int in_sizew = 679; // 630

	static int tab_selected = 0;
	static const int tab_amount = 9;
	static std::string tab_names[tab_amount] = { "LEGITBOT", "RAGEBOT", "ANTIAIM", "PLAYERS", "VISUALS", "SKINS", "MISC", "CONFIGS", "LUA" };
	...
	std::string resolvemethod[2] = { "AA", "Stable" };

	std::string pitchresolver[2] = { "None", "Auto" };

	if (tab_selected == 0)
	{
		groupbox(10, 30, GroupBoxSize_Width, GroupBoxSize_Height - 140, "Legitbot");
		...
		groupbox(10, 120, GroupBoxSize_Width, GroupBoxSize_Height - 130, "Aimbot Settings");
		...
		groupbox(10, 220, GroupBoxSize_Width, GroupBoxSize_Height - 130, "RCS Settings");
		slider(100, "RCS X", &c_config::get().rcs_x, "", 1);
		slider(100, "RCS Y", &c_config::get().rcs_y, "", 1);
	}
	else if (tab_selected == 1)
	{
			groupbox(10, 30, GroupBoxSize_Width, GroupBoxSize_Height - 60, "Ragebot");
			...
			groupbox(10, 200, GroupBoxSize_Width, GroupBoxSize_Height - 140, "Auto Stop");
			checkbox("Enabled", &c_config::get().autostop);
			...
			groupbox(10, 290, GroupBoxSize_Width, GroupBoxSize_Height - 130, "Multipoint");
			slider(100, "Head Scale", &c_config::get().aimbot_headpointscale, "%", 1);
			slider(100, "Body Scale", &c_config::get().aimbot_bodypointscale, "%", 1);

			groupbox(10, 390, GroupBoxSize_Width, GroupBoxSize_Height - 40, "Bodyaim");
			...
			slider(101, "Body-Aim health", &c_config::get().bodyaim_health, "HP", 1);
			slider(10, "Body-Aim missed Shots", &c_config::get().bodyaim_shots, "X", 1);

			groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 50, 220, GroupBoxSize_Width, (GroupBoxSize_Height) - 90, "Resolver");
			checkbox("Enabled", &c_config::get().aimbot_resolver);
			combobox(2, "Resolver mode", resolvemethod, &c_config::get().resolvemethod);
			combobox(2, "Pitch resolver mode", pitchresolver, &c_config::get().pitchresolver);

			static int current_subtab_selection_accuracy = 0;
			groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 50, 30, GroupBoxSize_Width, (GroupBoxSize_Height) - 160, "Weapons", false,
				true);
			DrawAimbotAccuracy_Subtab(current_subtab_selection_accuracy, 5, _pos, MenuAlpha_Main);

			groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 50, 100, GroupBoxSize_Width, (GroupBoxSize_Height) - 130, "Weapons settings");
		...
		std::string desync[6] = { "Disabled", "Ephemeral", "Cromulent", "Ornery", "Jitter", "Static" };
		std::string freestand_mode[3] = { "Threat", "Crosshair", "Distance" };

		groupbox(TopLeftGroupBox_X + GroupBoxSize_Width - 245, 30, GroupBoxSize_Width, (GroupBoxSize_Height) - 160, "Condition", false, true);
		DrawAntiAim_Subtab(current_subtab_antiaim_selection, 3, _pos, MenuAlpha_Main);
		
		groupbox(10, 240, GroupBoxSize_Width, (GroupBoxSize_Height) - 130, "Auto-direction");
		checkbox("Enabled", &c_config::get().antiaim_freestanding);
		...
		groupbox(10, 100, GroupBoxSize_Width, (GroupBoxSize_Height)-90, "Antiaim Settings");
		checkbox("Enabled", &c_config::get().antiaim_enabled);
		switch (current_subtab_antiaim_selection)
		{
		case 0:
		{
			combobox(5, "Pitch", pitch, &c_config::get().pitch_standing);
			combobox(5, "Real", yaw, &c_config::get().yaw_standing);
		}
		break;
		case 1:
		{
			combobox(5, "Pitch", pitch, &c_config::get().pitch_moving);
			combobox(5, "Real", yaw, &c_config::get().yaw_moving);
		}
		break;
		case 2:
		{
			combobox(5, "Pitch", pitch, &c_config::get().pitch_air);
			combobox(5, "Real", yaw, &c_config::get().yaw_air);
		}
		break;
		}

		groupbox(10, 340, GroupBoxSize_Width, (GroupBoxSize_Height) - 50, "Desync");
		...
		groupbox(10, 520, GroupBoxSize_Width, (GroupBoxSize_Height) - 160, "Fakelag");
		slider(14, "Fakelag", &c_config::get().fakelag, "t", 1);

		groupbox(10, 590, GroupBoxSize_Width, (GroupBoxSize_Height) - 160, "Slowwalk");
		slider(100, "Slowwalk", &c_config::get().slowwalk_speed, "%", 1);
	...
	else if (tab_selected == 3)
	{
		...
		groupbox(10, 30, GroupBoxSize_Width, GroupBoxSize_Height - 30, "ESP");
		...
		groupbox(10, 230, GroupBoxSize_Width, GroupBoxSize_Height - 60, "Model");
	...
	groupbox(10, 30, GroupBoxSize_Width, GroupBoxSize_Height, "Flags");
	...
	checkbox("Spectators", &c_config::get().visual_spectator_list);

	groupbox(10, 260, GroupBoxSize_Width, GroupBoxSize_Height - 120, "Remove");
	...
	groupbox(10, 370, GroupBoxSize_Width, GroupBoxSize_Height - 130, "World");
	...
	groupbox(10, 470, GroupBoxSize_Width, GroupBoxSize_Height - 50, "View");
	...
	groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 50, 30, GroupBoxSize_Width, (GroupBoxSize_Height)- 140, "Help Visuals");
	...
	groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 50, 120, GroupBoxSize_Width, (GroupBoxSize_Height)- 50, "Other Visuals");
	...
	groupbox(10, 30, GroupBoxSize_Width, GroupBoxSize_Height - 130, "Movement");
	...
	groupbox(10, 130, GroupBoxSize_Width, GroupBoxSize_Height - 100, "Sound");
	combobox(3, "Hit Marker Sound", Hitmarker_Type, &c_config::get().hitmarker_sound);

	groupbox(10, 260, GroupBoxSize_Width, GroupBoxSize_Height - 140, "Clantag");
	checkbox("Clan tag spammer", &c_config::get().misc_clantag);

	}

	else if (tab_selected == 7)
	{

		groupbox(10, 30, GroupBoxSize_Width, GroupBoxSize_Height - 100, "Bind Settings");
		std::string KeyBindList[4] = { "Thirdperson", "Slowwalk", "Swap Manual", "Fake Duck" };
		static int CurrentKeybind = 0;


		combobox(4, "Keybind", KeyBindList, &CurrentKeybind);
		...
		case 2: keybind(&c_config::get().manual_swap_bind, "c");
			break;
		case 3: keybind(&c_config::get().fakeduck_bind, "c");
			break;
		}
		...
		groupbox(10, 160, GroupBoxSize_Width, GroupBoxSize_Height,"Configs");
	...
	else if (tab_selected == 8)
	{
	groupbox(10, 30, GroupBoxSize_Width, GroupBoxSize_Height, "Lua");
	/* луа файлы тут должны быть, секрет папку с луа не скажу 
	 также надо доработать это так что да, удачи :333333333333333 */

	{
	(button(Load_Config, "Load LUA Config"));

	std::string luafiles[10] = { "INVLD", "INVLD", "INVLD", "INVLD", "INVLD", "INVLD", "INVLD", "INVLD", "INVLD", "INVLD" };

	combobox(10, "Luas", luafiles, &c_config::get().luafiles);
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/* чтобы поставить вместо invld норм луашку, найди папку луашек и поставь туда луашку вашу и помен€йте один бокс на любое название */
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	}
  ...
  //  #include <d3dx9shape.h>
	...
	{
		groupbox(10, 130, GroupBoxSize_Width, GroupBoxSize_Height - 100, "Extra");

		checkbox("Legit Anti-Aim", &c_config::get().extra_lua_option1);
		button(Import_Config, "Import Config");
		button(Export_Config, "Export Config");

		// кроме импорта и экспорта, все не работает, фиксите сами
		...
		for (int i = 0; i < g_pGlobalVars->maxClients; i++) {
			...
			if (g_pEngine->GetPlayerInfo(i, &EntityInfo)) {
				...
				if (selected_player == SteamID) {
					...
					g_pSurface->FilledRect(Area.left, Area.top, Area.right, Area.bottom, Color(15, 127, 255, MenuAlpha_Main));
				}

				g_pSurface->OutlinedRect(Area.left, Area.top, Area.right, Area.bottom, Color(40, 44, 53, MenuAlpha_Main));

				g_pSurface->DrawT(Area.left + (Area.right / 2), Area.top + 1, Color(15, 127, 255, MenuAlpha_Main), Globals::SmallText, true, EntityInfo.szName);
...
{
	RECT text_size = g_pSurface->GetTextSizeRect(Globals::CourierNew, name.c_str());
	if (!dont_draw)
	{
		//	g_pSurface->OutlinedRect(_pos.x + x - 1, _pos.y + y - 1, w + 2, h + 2, Color(0, 0, 0, 185));
		g_pSurface->FilledRect(_pos.x + x, _pos.y + y, w, h, Color(25, 26, 33, MenuAlpha_Main));
		g_pSurface->OutlinedRect(_pos.x + x, _pos.y + y, w, h, Color(40, 44, 53, MenuAlpha_Main));
		g_pSurface->FilledRect(_pos.x + x, _pos.y + y, w, 3, Color(c_config::get().menu_color_r, c_config::get().menu_color_g, c_config::get().menu_color_b, MenuAlpha_Main));
		g_pSurface->DrawT(_pos.x + x, _pos.y + y - (text_size.bottom + 5), Color(c_config::get().menu_color_r, c_config::get().menu_color_g, c_config::get().menu_color_b, MenuAlpha_Main), Globals::CourierNew, false, name.c_str());
		auto a = _pos;
	}
...
void Menu::checkbox(std::string name, bool* item)
{
	...
	{
		g_pSurface->DrawT(x_offset + 20, y_offset - 3, Color(15, 127, 255, MenuAlpha_Main), Globals::SmallText, false,
			name.c_str());
