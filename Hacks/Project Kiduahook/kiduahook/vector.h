...
{
private:
	inline void VectorCrossProduct( const Vector& a, const Vector& b, Vector& result ) const
	{
...
public:
	...
	float& operator[]( int i );
	inline void Zero();
	inline float Dist( const Vector &vOther ) const;
...
{
	return (Vector(*this) / (sqrt(x * x + y * y + z * z)));
/*	Vector vector;
...
//===============================================
inline void Vector::MultAdd( const Vector& vOther, float fl )
{
	x += fl * vOther.x;
	y += fl * vOther.y;
	z += fl * vOther.z;
