...
void misc_tab() {
	...
	case 1: {
		...
		ImGui::BeginChild("##secondchild", ImVec2(0, 0)); {
			...
			if (ImGui::Button("Delete", ImVec2(-1, 20)) && fs::remove("C:\\killaura" + current_config)) {
...
void legit_tab() {
	...
	ImGui::BeginGroup(); {
		ImGui::BeginChild("##aimbot.misc", ImVec2(0, 0)); {
			ImGui::PushItemWidth(-1);
...
void Menu::Render() {
	...
	colors[ImGuiCol_CheckMark]			  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 1.0f);
	colors[ImGuiCol_Header]				  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 1.0f);
	colors[ImGuiCol_HeaderHovered]		  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 0.86f);
	colors[ImGuiCol_HeaderActive]		  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 1.00f);
	colors[ImGuiCol_SliderGrab]			  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 0.74f);
	colors[ImGuiCol_SliderGrabActive]	  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 1.00f);
	colors[ImGuiCol_ScrollbarGrab]		  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 1.00f);
	colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 1.00f);
	colors[ImGuiCol_ScrollbarGrabActive]  = ImVec4(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2], 1.00f);

	if (Cfg.misc_radar) DrawRadar();
	...
	ImGui::SetNextWindowSize({ 490,460 });
	if (ImGui::Begin("killaura aka killaura meme edition", &_opened, flags)) {
		ImGui::PushFont(MainCaps);
		if (ImGui::BeginChild("Line", ImVec2(-1, 25), false, flags)) {
			ImVec2 p = ImGui::GetCursorScreenPos();

			ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(p.x, p.y + 3), 
				ImVec2(p.x + ImGui::GetWindowWidth(), p.y + -3), ImColor(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2]));
			...
			ImGui::SetCursorPosX((ImGui::GetWindowWidth() - ImGui::CalcTextSize("killaura").x) / 2);
			ImGui::TextColored(ImColor(IM_COL32_WHITE), "killaura");
		}
		ImGui::EndChild();
		ImGui::PopFont();

		ImGui::PushFont(Main);
		if (ImGui::BeginChild("Tab", ImVec2(-1, 50), false, flags)) {
		...
		if (ImGui::BeginChild("Main", ImVec2(-1, -1), false, flags)) {
			switch (active_tab) {
