...
void Render::BeginScene() {
	...
	if (Cfg.misc_watermark) {
		...
		std::string incoming = g_LocalPlayer ? std::to_string((int)(net_channel->GetLatency(FLOW_INCOMING) * 1000)) : "0";

		//get tickrate
		int rate = (int)std::round(1.f / g_GlobalVars->interval_per_tick);

		//get framerate.
		int fps = (int)std::round(1.f / g_GlobalVars->frametime);

		std::string logo = "killaura";

		std::stringstream ss;

		if (g_EngineClient->IsConnected())
			ss << logo << " | " << "rtt: " << incoming.c_str() << " | " << "fps: " << fps << " | " << rate << "tick";
		else
			ss << logo << " | " << time.str().data();

		auto sz = MainCaps->CalcTextSizeA(14.f, FLT_MAX, 0.0f, ss.str().c_str());

		int x = sz.x + 14, y = sz.y + 14;

		Render::Get().RenderBoxFilled(7, 6, x, 8, Color(Cfg.color_menu[0], Cfg.color_menu[1], Cfg.color_menu[2]));
		Render::Get().RenderBoxFilled(7, 7, x, y, Color(10, 10, 10, 100));
		Render::Get().RenderText(ss.str(), 10, 10, 14.f, Color::White, false, false, MainCaps);
	}
