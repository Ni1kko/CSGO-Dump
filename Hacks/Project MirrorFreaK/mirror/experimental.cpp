...
void C_LagCompensation::frame_net_update_end() const
{
	...
	{
		auto entity = Interfaces::EntList->GetClientEntity(index);

		if (entity->GetIndex() != ragebot->TargetID)
			continue;
...
{
	auto predicted_arrival_tick = TIME_TO_TICKS(Interfaces::Engine->GetNetChannelInfo()->GetAvgLatency(0) + Interfaces::Engine->GetNetChannelInfo()->GetAvgLatency(1) + (ragebot->cmd->tick_count + Interfaces::m_pClientState->chokedcommands + 1));
