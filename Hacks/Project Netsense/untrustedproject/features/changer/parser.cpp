...
auto game_data::initialize_kits() -> void
{
	...
	const auto sig_address = Utils::PatternScan(GetModuleHandleA("client.dll"), "E8 ?? ?? ?? ?? FF 76 0C 8D 48 04 E8");
	...
	// Dump sticker kits
	/*{
		const auto sticker_sig = platform::find_pattern("client_panorama.dll", "\x53\x8D\x48\x04\xE8\x00\x00\x00\x00\x8B\x4D\x10", "xxxxx????xxx") + 4;
		// Skip the opcode, read rel32 address
...
int game_data::ParseSkins()
{
		...
		if (child->ToObject())
		{
			for (auto weapon : weaponNames)
			{
				...
				if (pos != std::string::npos)
				{
					...
					g_SkinChanger->weaponSkins[weapon].insert(
		...
		if (skinData->ToObject())
		{
			for (auto skin : skinData->children)
			{
				if (skin->ToObject())
				{
					...
					g_SkinChanger->skinMap[skinName] = si;
				}
			}
		}
	}
	...
	for (auto child : PaintKitNames->children)
	{
		if (child->ToKeyValue())
		{
			...
			{
				g_SkinChanger->skinNames[key] = child->ToKeyValue()->Value.toString();
			}
		}
	}
		...
		if (rarity->ToObject()) {
			for (const auto& child : rarity->children) {
				if (child->ToKeyValue()) {
					...
					auto skinInfo = &g_SkinChanger->skinMap[paint_kit_name];
