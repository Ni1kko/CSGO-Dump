...
void CCustomWeaponGroups::RenderSelector(std::string& selected, float customSpasing, std::function<void(std::string)> callback) {
	if (ImGui::ListBoxHeader("##wgroup_groupselector", ImVec2(-1, ImGui::GetWindowSize().y - customSpasing - 2))) {
...
void CCustomWeaponGroups::Menu(std::string& selected_group) { // TODO: Add breaks to delete functions
	...
	auto delete_group = [&](std::string name) -> void {
		setts.erase(name);
		g_Notification.Push("Information", ("Group " + name + " was removed!").c_str());
	};
	//
	...
	if (dndrop_released) {
		...
		if (dndrop_target == DNDropTarget_New && current_wgroup->lock)
			g_Notification.Push("Access denied", "No way, that group is locked");
		...
		if (dndrop_target == DNDropTarget_New && !current_wgroup->lock) {
			...
			g_Notification.Push("Success", ("New group \"" + values::WNames[dndrop_payload] + "\" was created!").c_str());
		}
			...
			for (auto& [wgroup_name, wgroup] : setts) {
				if (wgroup_name == dndrop_extra_payload) {
					...
					g_Notification.Push("Success", ("Pushed \"" + values::WNames[dndrop_payload] + "\" to \"" + dndrop_extra_payload + "\"").c_str());
					break;
				}
			}
		}
