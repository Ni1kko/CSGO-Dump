...
/*
 ...
 Q: How can I help?
 A: - If you are Untrustedd with Dear ImGui and C++, look at the github issues, or TODO.txt and see how you want/can help!
...
#endif
#include "../addons/clip/clip.h"
#include "../addons/clip/clip_lock_impl.h"
...
void ImGui::UpdateMouseWheel()
{
    ...
    while ((window->Flags & ImGuiWindowFlags_ChildWindow) && (window->Flags & ImGuiWindowFlags_NoScrollWithMouse) && !(window->Flags & ImGuiWindowFlags_NoScrollbar)  && window->ParentWindow)
    ...
    {
        const float max_step = window->Scroll.y + scroll_speed * 2.0;
    ...
    window->MaxStep = ImVec2(0.0f, 25.0f);
    // Vertical Mouse Wheel Scrolling (hold Shift to scroll horizontally)
    if (g.IO.MouseWheel != 0.0f && !g.IO.KeyShift)
    {
        float scroll_step = ImFloor(ImMin(8 * window->CalcFontSize(), window->MaxStep.y));
        window->ScrollNext.y = window->Scroll.y - g.IO.MouseWheel * scroll_step;
        //SetWindowScrollY(window, window->Scroll.y - g.IO.MouseWheel * scroll_step);
    }
...
void ImGui::Render(ImDrawList* our_list)
{
    ...
    for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_front_most); n++)
        if (windows_to_render_front_most[n] && IsWindowActiveAndVisible(windows_to_render_front_most[n])) // NavWindowingTarget is always temporarily displayed as the front-most window
            AddWindowToDrawData(&g.DrawDataBuilder.Layers[0], windows_to_render_front_most[n]);
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (flags & ImGuiWindowFlags_MenuBar)
            {
                ...
                if (style.FrameBorderSize > 2.0f && menu_bar_rect.Max.y < window->Pos.y + window->Size.y)
                    window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32(ImGuiCol_Border), style.FrameBorderSize);
                    window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32(ImGuiCol_Border), style.FrameBorderSize);
                    window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32(ImGuiCol_Border), style.FrameBorderSize);
                    window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32(ImGuiCol_Border), style.FrameBorderSize);
                    window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32(ImGuiCol_Border), style.FrameBorderSize);
                //window->DrawList->AddRectFilledMultiColor(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 200), ImColor(255, 255, 255, 200));
            }
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ...
    float win_size_avail_v = (horizontal ? window->SizeFull.x : window->SizeFull.y);
...
/*bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    {
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.00f, 0.00f, 0.00f, 1.0f), ImVec4(1.00f, 1.00f, 1.00f, 1.0f), t));
        checkbg = ImGui::GetColorU32(ImLerp(ImVec4(1.00f, 1.00f, 1.00f, 1.0f), ImVec4(0.00f, 0.00f, 0.00f, 1.0f), t));
    }
    ...
    draw_list->AddRectFilled(p, ImVec2(p.x - 2 + width - 4, p.y + height - 6), col_bg, height * 0.9f);
    draw_list->AddCircleFilled(ImVec2(p.x + radius - 3 + t * (width - radius * 2.0f), p.y + radius - 3), radius - 1.5f, checkbg);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
        RenderFrame(ImVec2(check_bb.Min.x - 5, check_bb.Min.y + 3), ImVec2(check_bb.Max.x - 10, check_bb.Max.y - 2), ImColor(menur, menug, menub, 255), false, (3.f));
    else
        RenderFrame(ImVec2(check_bb.Min.x - 5, check_bb.Min.y + 3), ImVec2(check_bb.Max.x - 10, check_bb.Max.y - 2), ImColor(0.15f, 0.16f, 0.17f, 1.0f), true, (3.f));
    ...
    if (label_size.x > 0.0f)
    {
        ...
        if (*v)
        {
            ...
            RenderText(ImVec2(check_bb.Max.x - 5, check_bb.Min.y + style.FramePadding.y), label);
            ImGui::PopStyleColor();
        }
        else
        {
...
bool ImGui::ColorButton(const char* desc_id, ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    ImRect bb_inner = bb;
    float off = -2.f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like a good middle ground to reduce those artifacts.
    bb_inner.Expand(off);
