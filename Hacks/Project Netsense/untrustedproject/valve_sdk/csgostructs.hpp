...
public:
	...
	NETVAR(float, flMaxSpeed, "DT_BasePlayer", "m_flMaxspeed");
...
public:
	...
	bool IsFiredWeaponType() {
		...
		int WType = this->GetCSWeaponData()->iWeaponType;
...
public:
	...
	NETVAR(Vector, m_vecAbsVelocity, "DT_BasePlayer", "m_vecAbsVelocity");
	...
	void PVSFix(int framecount) {
		...
		*(int*)((DWORD)this + 0xA30) = framecount; //we'll skip occlusion checks now
		*(int*)((DWORD)this + 0xA28) = 0;//clear occlusion flags-
	}

	bool IsReloading();
	void SetAbsAngles(QAngle angle);
	Vector GetHitboxPos1(int hitbox_id);
	Vector GetHitboxPos2(int hitbox_id, matrix3x4_t* boneMatrix);
	void SetAbsOrigin(const Vector& origin);
	Vector GetHitboxPos3(int hitbox_id, matrix3x4_t* boneMatrix, float& Radius, Vector& Min, Vector& Max, int& Bone);
	void SetAbsAngles(Vector angle);
	float GetMaxDesyncDelta();
	float DesyncValue();
	int GetChockedPacketsAmount();
	float m_flOldSimulationTime();
};

struct Inferno_t : public C_BaseEntity {
	float GetSpawnTime() {
