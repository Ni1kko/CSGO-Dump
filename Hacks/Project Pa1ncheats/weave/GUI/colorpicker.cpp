...
void c_colorpicker::render_sliderbg(Vector2D pos, float alpha)
{
	...
	if (img)
		g_Render->_drawList->AddImage((void*)img, ImVec2(pos.x, pos.y), ImVec2(pos.x + 150, pos.y + 10), ImVec2(0, 0), ImVec2(1, 1),
			color_t(255, 255, 255, alpha).u32());
}
...
void c_colorpicker::render() {
	...
	if (dialogue.open) {
		...
		g_Render->FilledRect(pos.x + 1, pos.y + 1, size.x - 2, 163, color_t(50, 50, 50, alpha));
		...
		g_Render->filled_rect_gradient(pos.x + 15, pos.y + 145, size.x - 30.f, 10.f,
			color_t(50, 50, 50, 0), clr, clr, color_t(50, 50, 50, 0));
