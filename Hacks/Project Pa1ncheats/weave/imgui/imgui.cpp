...
// [SECTION] MISC HELPERS/UTILITIES (color_t functions)
...
/*
 ...
 - 2019/02/26 (1.69) - renamed ImGuicolor_tEditFlags_RGB/ImGuicolor_tEditFlags_HSV/ImGuicolor_tEditFlags_HEX to ImGuicolor_tEditFlags_DisplayRGB/ImGuicolor_tEditFlags_DisplayHSV/ImGuicolor_tEditFlags_DisplayHex. Kept redirection enums (will obsolete).
 ...
 - 2017/11/18 (1.53) - Style, Begin: removed ImGuiWindowFlags_ShowBorders window flag. Borders are now fully set up in the ImGuiStyle structure (see e.g. style.FrameBorderSize, style.WindowBorderSize). Use ImGui::ShowStyleEditor() to look them up.
                       Please note that the style system will keep evolving (hopefully stabilizing in Q1 2018), and so custom styles will probably subtly break over time. It is recommended you use the Stylecolor_tsClassic(), Stylecolor_tsDark(), Stylecolor_tsLight() functions.
 ...
 - 2017/08/20 (1.51) - renamed GetStyleColName() to GetStylecolor_tName() for consistency.
 - 2017/08/20 (1.51) - added PushStylecolor_t(ImGuiCol idx, ImU32 col) overload, which _might_ cause an "ambiguous call" compilation error if you are using Imcolor_t() with implicit cast. Cast to ImU32 or ImVec4 explicily to fix.
 ...
 - 2017/08/09 (1.51) - removed Valuecolor_t() helpers, they are equivalent to calling Text(label) + SameLine() + color_tButton().
 - 2017/08/08 (1.51) - removed color_tEditMode() and ImGuicolor_tEditMode in favor of ImGuicolor_tEditFlags and parameters to the various color_t*() functions. The Setcolor_tEditOptions() allows to initialize default but the user can still change them with right-click context menu.
                     - changed prototype of 'color_tEdit4(const char* label, float col[4], bool show_alpha = true)' to 'color_tEdit4(const char* label, float col[4], ImGuicolor_tEditFlags flags = 0)', where passing flags = 0x01 is a safe no-op (hello dodgy backward compatibility!). - check and run the demo window, under "color_t/Picker Widgets", to understand the various new options.
                     - changed prototype of rarely used 'color_tButton(ImVec4 col, bool small_height = false, bool outline_border = true)' to 'color_tButton(const char* desc_id, ImVec4 col, ImGuicolor_tEditFlags flags = 0, ImVec2 size = ImVec2(0,0))'
 ...
 - 2016/04/03 (1.48) - removed style.WindowFillAlphaDefault setting which was redundant. Bake default BG alpha inside style.color_ts[ImGuiCol_WindowBg] and all other Bg color values. (ref github issue #337).
 ...
 - 2015/12/04 (1.47) - renamed color_t() helpers to Valuecolor_t() - dangerously named, rarely used and probably to be made obsolete.
...
ImGuiStyle::ImGuiStyle()
{
    ...
    color_tButtonPosition     = ImGuiDir_Right;   // Side of the color button in the color_tEdit4 widget (left/right). Defaults to ImGuiDir_Right.
...
//-----------------------------------------------------------------------------
// [SECTION] MISC HELPERS/UTILTIES (color_t functions)
// Note: The Convert functions are early design which are not consistent with other API.
//-----------------------------------------------------------------------------

ImVec4 ImGui::color_tConvertU32ToFloat4(ImU32 in)
{
    float s = 1.0f/255.0f;
    return ImVec4(
...
ImU32 ImGui::color_tConvertFloat4ToU32(const ImVec4& in)
{
    ImU32 out;
...
void ImGui::color_tConvertRGBtoHSV(float r, float g, float b, float& out_h, float& out_s, float& out_v)
{
    float K = 0.f;
    if (g < b)
    {
...
void ImGui::color_tConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b)
{
    if (s == 0.0f)
    {
        // gray
...
ImU32 ImGui::Getcolor_tU32(ImGuiCol idx, float alpha_mul)
{
    ImGuiStyle& style = GImGui->Style;
    ImVec4 c = style.color_ts[idx];
    c.w *= style.Alpha * alpha_mul;
    return color_tConvertFloat4ToU32(c);
}

ImU32 ImGui::Getcolor_tU32(const ImVec4& col)
{
    ImGuiStyle& style = GImGui->Style;
    ImVec4 c = col;
    c.w *= style.Alpha;
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, Getcolor_tU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	...
	//RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding, buf_display, NULL, NULL, Getcolor_tU32(ImGuiCol_Text), style.ButtonTextAlign, &clip_rect);
	//draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos, Getcolor_tU32(ImGuiCol_Text), buf_display, NULL, 0.0f, &clip_rect);
...
const ImVec4& ImGui::GetStylecolor_tVec4(ImGuiCol idx)
{
    ImGuiStyle& style = GImGui->Style;
    return style.color_ts[idx];
}

ImU32 ImGui::Getcolor_tU32(ImU32 col)
{
    ImGuiStyle& style = GImGui->Style;
    if (style.Alpha >= 1.0f)
        return col;
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ...
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, Getcolor_tU32(ImGuiCol_Text), text, text_display_end);
        if (g.LogEnabled)
...
void ImGui::RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end, float wrap_width)
{
    ...
    if (text != text_end)
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, Getcolor_tU32(ImGuiCol_Text), text, text_end, wrap_width);
        if (g.LogEnabled)
...
void ImGui::RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    if (need_clipping)
    {
        ...
        draw_list->AddText(NULL, 0.0f, pos, Getcolor_tU32(ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        draw_list->AddText(NULL, 0.0f, pos, Getcolor_tU32(ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
    }
}
...
void ImGui::RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, const char* text, const char* text_end_full, const ImVec2* text_size_if_known)
{
    ...
    if (text_size.x > pos_max.x - pos_min.x)
    {
        ...
        if (ellipsis_x + ellipsis_total_width <= ellipsis_max_x)
            ...
            {
                font->RenderChar(draw_list, font_size, ImVec2(ellipsis_x, pos_min.y), Getcolor_tU32(ImGuiCol_Text), ellipsis_char);
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
    ...
    {
        window->DrawList->AddRect(p_min+ImVec2(1,1), p_max+ImVec2(1,1), Getcolor_tU32(ImGuiCol_BorderShadow), rounding, ImDrawCornerFlags_All, border_size);
        window->DrawList->AddRect(p_min, p_max, Getcolor_tU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
    }
}
...
void ImGui::RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags)
{
    ...
    if (flags & ImGuiNavHighlightFlags_TypeDefault)
    {
        ...
        window->DrawList->AddRect(display_rect.Min + ImVec2(THICKNESS*0.5f,THICKNESS*0.5f), display_rect.Max - ImVec2(THICKNESS*0.5f,THICKNESS*0.5f), Getcolor_tU32(ImGuiCol_NavHighlight), rounding, ImDrawCornerFlags_All, THICKNESS);
        if (!fully_visible)
            window->DrawList->PopClipRect();
    }
    if (flags & ImGuiNavHighlightFlags_TypeThin)
    {
        window->DrawList->AddRect(display_rect.Min, display_rect.Max, Getcolor_tU32(ImGuiCol_NavHighlight), rounding, ~0, 1.0f);
    }
}
...
void ImGui::UpdateDebugToolItemPicker()
{
    ...
    if (g.DebugItemPickerActive)
    {
        ...
        ImGui::Textcolor_ted(GetStylecolor_tVec4(hovered_id ? ImGuiCol_Text : ImGuiCol_TextDisabled), "Click to break in debugger!");
        ImGui::EndTooltip();
    }
}
...
bool ImGui::BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    PushStylecolor_t(ImGuiCol_ChildBg, style.color_ts[ImGuiCol_FrameBg]);
...
static ImGuiCol GetWindowBgcolor_tIdxFromFlags(ImGuiWindowFlags flags)
{
...
static bool ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        if (resize_grip_n == 0 || held || hovered)
            resize_grip_col[resize_grip_n] = Getcolor_tU32(held ? ImGuiCol_ResizeGripActive : hovered ? ImGuiCol_ResizeGripHovered : ImGuiCol_ResizeGrip);
    }
    ...
    if (g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window)
    {
        ...
        if (nav_resize_delta.x != 0.0f || nav_resize_delta.y != 0.0f)
        {
            ...
            g.NavDisableMouseHover = true;
            resize_grip_col[0] = Getcolor_tU32(ImGuiCol_ResizeGripActive);
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
    ...
    if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground))
        window->DrawList->AddRect(window->Pos, window->Pos + window->Size, Getcolor_tU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
    ...
    if (border_held != -1)
    {
        ...
        window->DrawList->PathStroke(Getcolor_tU32(ImGuiCol_SeparatorActive), false, ImMax(2.0f, border_size)); // Thicker than usual
    }
    if (g.Style.FrameBorderSize > 0 && !(window->Flags & ImGuiWindowFlags_NoTitleBar))
    {
        float y = window->Pos.y + window->TitleBarHeight() - 1;
        window->DrawList->AddLine(ImVec2(window->Pos.x + border_size, y), ImVec2(window->Pos.x + window->Size.x - border_size, y), Getcolor_tU32(ImGuiCol_Border), g.Style.FrameBorderSize);
    }
}
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    if (window->Collapsed)
    {
        ...
        ImU32 title_bar_col = Getcolor_tU32((title_bar_is_highlight && !g.NavDisableHighlight) ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBgCollapsed);
    ...
    else
    {
        ...
        {
            ImU32 bg_col = Getcolor_tU32(GetWindowBgcolor_tIdxFromFlags(flags));
            float alpha = 1.0f;
        ...
        {
            ImU32 title_bar_col = Getcolor_tU32(title_bar_is_highlight ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg);
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
            ...
            window->DrawList->AddRectFilled(menu_bar_rect.Min + ImVec2(window_border_size, 0), menu_bar_rect.Max - ImVec2(window_border_size, 0), Getcolor_tU32(ImGuiCol_MenuBarBg), (flags & ImGuiWindowFlags_NoTitleBar) ? window_rounding : 0.0f, ImDrawCornerFlags_Top);
            if (style.FrameBorderSize > 0.0f && menu_bar_rect.Max.y < window->Pos.y + window->Size.y)
                window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), Getcolor_tU32(ImGuiCol_Border), style.FrameBorderSize);
        }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        {
            const ImU32 dim_bg_col = Getcolor_tU32(dim_bg_for_modal ? ImGuiCol_ModalWindowDimBg : ImGuiCol_NavWindowingDimBg, g.DimBgRatio);
        ...
        if (dim_bg_for_window_list && window == g.NavWindowingTargetAnim)
        {
            ...
            if (!bb.Contains(viewport_rect)) // Avoid drawing if the window covers all the viewport anyway
                window->DrawList->AddRectFilled(bb.Min, bb.Max, Getcolor_tU32(ImGuiCol_NavWindowingHighlight, g.NavWindowingHighlightAlpha * 0.25f), g.Style.WindowRounding);
        }
        ...
        if (g.NavWindowingTargetAnim == window)
        {
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, Getcolor_tU32(ImGuiCol_NavWindowingHighlight, g.NavWindowingHighlightAlpha), rounding, ~0, 3.0f);
        }
...
void ImGui::End(bool fade_out)
{
	...
	if (window->Flags & ImGuiWindowFlags_ChildWindow && fade_out) {
		//window->DrawList->AddRectFilledMulticolor_t(window->Pos + ImVec2(2, 2), window->Pos + ImVec2(window->Size.x - 6, 20),
		//	Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, g.Style.Alpha),
		//	Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, g.Style.Alpha),
		//	Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, 0.f),
		//	Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, 0.f));

		window->DrawList->AddRectFilledMulticolor_t(window->Pos + ImVec2(2, window->Size.y - 22), window->Pos + ImVec2(window->Size.x - 6, window->Size.y - 1),
			Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, 0.f),
			Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, 0.f),
			Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, g.Style.Alpha),
			Imcolor_t(19 / 255.f, 19 / 255.f, 19 / 255.f, g.Style.Alpha));
		...
		//window->DrawList->AddText(window->Pos + ImVec2(13, textSize.y / -2 + 2), Imcolor_t(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), window->Name);
	}
...
void ImGui::PushStylecolor_t(ImGuiCol idx, ImU32 col)
{
    ImGuiContext& g = *GImGui;
    ImGuicolor_tMod backup;
    backup.Col = idx;
    backup.BackupValue = g.Style.color_ts[idx];
    g.color_tModifiers.push_back(backup);
    g.Style.color_ts[idx] = color_tConvertU32ToFloat4(col);
}

void ImGui::PushStylecolor_t(ImGuiCol idx, const ImVec4& col)
{
    ImGuiContext& g = *GImGui;
    ImGuicolor_tMod backup;
    backup.Col = idx;
    backup.BackupValue = g.Style.color_ts[idx];
    g.color_tModifiers.push_back(backup);
    g.Style.color_ts[idx] = col;
}

void ImGui::PopStylecolor_t(int count)
{
    ImGuiContext& g = *GImGui;
    while (count > 0)
    {
        ImGuicolor_tMod& backup = g.color_tModifiers.back();
        g.Style.color_ts[backup.Col] = backup.BackupValue;
        g.color_tModifiers.pop_back();
        count--;
    }
}
...
const char* ImGui::GetStylecolor_tName(ImGuiCol idx)
{
...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow* window, bool write)
{
    ...
    { int n = g.color_tModifiers.Size;      if (write) *p = (short)n; else IM_ASSERT(*p >= n && "PushStylecolor_t/PopStylecolor_t Mismatch!");       p++; }    // Too few or too many PopStylecolor_t()
...
void ImGui::BeginTooltip()
{
    ...
    if (g.DragDropWithinSourceOrTarget)
    {
        ...
        SetNextWindowBgAlpha(g.Style.color_ts[ImGuiCol_PopupBg].w * 0.60f);
        //PushStyleVar(ImGuiStyleVar_Alpha, g.Style.Alpha * 0.60f); // This would be nice but e.g color_tButton with checkboard has issue with transparent colors :(
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_AcceptNoDrawDefaultRect) && payload.Preview)
    {
        ...
        window->DrawList->AddRect(r.Min, r.Max, Getcolor_tU32(ImGuiCol_DragDropTarget), 0.0f, ~0, 2.0f);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            {
                ImGui::SameLine();
                ImGui::Textcolor_ted(ImVec4(1.0f,0.4f,0.4f,1.0f), "CURRENTLY APPENDING"); // Can't display stats for active draw list! (we don't have the data double-buffered)
