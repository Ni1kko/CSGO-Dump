...
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    if (show_app_style_editor)        { ImGui::Begin("Style Editor", &show_app_style_editor); ImGui::ShowStyleEditor(); ImGui::End(false); }
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        // color_t buttons, demonstrate using PushID() to add unique identifier in the ID stack, and changing style.
        for (int i = 0; i < 7; i++)
        {
            if (i > 0)
                ImGui::SameLine();
            ImGui::PushID(i);
            ImGui::PushStylecolor_t(ImGuiCol_Button, (ImVec4)Imcolor_t::HSV(i/7.0f, 0.6f, 0.6f));
            ImGui::PushStylecolor_t(ImGuiCol_ButtonHovered, (ImVec4)Imcolor_t::HSV(i/7.0f, 0.7f, 0.7f));
            ImGui::PushStylecolor_t(ImGuiCol_ButtonActive, (ImVec4)Imcolor_t::HSV(i/7.0f, 0.8f, 0.8f));
            ImGui::Button("Click");
            ImGui::PopStylecolor_t(3);
            ImGui::PopID();
        }
...

        {
            ...
            ImGui::color_tEdit3("color 1", col1);
            ImGui::SameLine(); HelpMarker("Click on the colored square to open a color picker.\nClick and hold to use drag and drop.\nRight-click on the colored square to show options.\nCTRL+click on individual component to input value.\n");

            ImGui::color_tEdit4("color 2", col2);
        }
    ...
    {
        if (ImGui::TreeNode("color_ted Text"))
        {
            // Using shortcut. You can use PushStylecolor_t()/PopStylecolor_t() for more flexibility.
            ImGui::Textcolor_ted(ImVec4(1.0f,0.0f,1.0f,1.0f), "Pink");
            ImGui::Textcolor_ted(ImVec4(1.0f,1.0f,0.0f,1.0f), "Yellow");
    ...
    if (ImGui::TreeNode("color_t/Picker Widgets"))
    {
        ...
        ImGuicolor_tEditFlags misc_flags = (hdr ? ImGuicolor_tEditFlags_HDR : 0) | (drag_and_drop ? 0 : ImGuicolor_tEditFlags_NoDragDrop) | (alpha_half_preview ? ImGuicolor_tEditFlags_AlphaPreviewHalf : (alpha_preview ? ImGuicolor_tEditFlags_AlphaPreview : 0)) | (options_menu ? 0 : ImGuicolor_tEditFlags_NoOptions);

        ImGui::Text("color_t widget:");
        ImGui::SameLine(); HelpMarker("Click on the colored square to open a color picker.\nCTRL+click on individual component to input value.\n");
        ImGui::color_tEdit3("Mycolor_t##1", (float*)&color, misc_flags);

        ImGui::Text("color_t widget HSV with Alpha:");
        ImGui::color_tEdit4("Mycolor_t##2", (float*)&color, ImGuicolor_tEditFlags_DisplayHSV | misc_flags);

        ImGui::Text("color_t widget with Float Display:");
        ImGui::color_tEdit4("Mycolor_t##2f", (float*)&color, ImGuicolor_tEditFlags_Float | misc_flags);

        ImGui::Text("color_t button with Picker:");
        ImGui::SameLine(); HelpMarker("With the ImGuicolor_tEditFlags_NoInputs flag you can hide all the slider/text inputs.\nWith the ImGuicolor_tEditFlags_NoLabel flag you can pass a non-empty label which will only be used for the tooltip and picker popup.");
        ImGui::color_tEdit4("Mycolor_t##3", (float*)&color, ImGuicolor_tEditFlags_NoInputs | ImGuicolor_tEditFlags_NoLabel | misc_flags);

        ImGui::Text("color_t button with Custom Picker Popup:");
        ...
        if (saved_palette_init)
        {
            ...
            {
                ImGui::color_tConvertHSVtoRGB(n / 31.0f, 0.8f, 0.8f, saved_palette[n].x, saved_palette[n].y, saved_palette[n].z);
        ...
        bool open_popup = ImGui::color_tButton("Mycolor_t##3b", color, misc_flags);
        ...
        if (ImGui::BeginPopup("mypicker"))
        {
            ...
            ImGui::Separator();
            ImGui::color_tPicker4("##picker", (float*)&color, misc_flags | ImGuicolor_tEditFlags_NoSidePreview | ImGuicolor_tEditFlags_NoSmallPreview);
            ImGui::SameLine();

            ImGui::BeginGroup(); // Lock X position
            ImGui::Text("Current");
            ImGui::color_tButton("##current", color, ImGuicolor_tEditFlags_NoPicker | ImGuicolor_tEditFlags_AlphaPreviewHalf, ImVec2(60,40));
            ImGui::Text("Previous");
            if (ImGui::color_tButton("##previous", backup_color, ImGuicolor_tEditFlags_NoPicker | ImGuicolor_tEditFlags_AlphaPreviewHalf, ImVec2(60,40)))
                color = backup_color;
            ImGui::Separator();
            ImGui::Text("Palette");
            for (int n = 0; n < IM_ARRAYSIZE(saved_palette); n++)
            {
                ...
                if (ImGui::color_tButton("##palette", saved_palette[n], ImGuicolor_tEditFlags_NoAlpha | ImGuicolor_tEditFlags_NoPicker | ImGuicolor_tEditFlags_NoTooltip, ImVec2(20,20)))
                    color = ImVec4(saved_palette[n].x, saved_palette[n].y, saved_palette[n].z, color.w); // Preserve alpha!

                // Allow user to drop colors into each palette entry
                // (Note that color_tButton is already a drag source by default, unless using ImGuicolor_tEditFlags_NoDragDrop)
        ...
        ImGui::Text("color_t button only:");
        ImGui::color_tButton("Mycolor_t##3c", *(ImVec4*)&color, misc_flags, ImVec2(80,80));

        ImGui::Text("color_t picker:");
        ...
        {
            ImGui::SameLine();
            ImGui::Checkbox("With Ref color_t", &ref_color);
            if (ref_color)
            {
                ImGui::SameLine();
                ImGui::color_tEdit4("##Refcolor_t", &ref_color_v.x, ImGuicolor_tEditFlags_NoInputs | misc_flags);
            }
        }
        ImGui::Combo("Display Mode", &display_mode, "Auto/Current\0None\0RGB Only\0HSV Only\0Hex Only\0");
        ImGui::SameLine(); HelpMarker("color_tEdit defaults to displaying RGB inputs if you don't specify a display mode, but the user can change it with a right-click.\n\ncolor_tPicker defaults to displaying RGB+HSV+Hex if you don't specify a display mode.\n\nYou can change the defaults using Setcolor_tEditOptions().");
        ImGui::Combo("Picker Mode", &picker_mode, "Auto/Current\0Hue bar + SV rect\0Hue wheel + SV triangle\0");
        ImGui::SameLine(); HelpMarker("User can right-click the picker to change mode.");
        ImGuicolor_tEditFlags flags = misc_flags;
        if (!alpha)            flags |= ImGuicolor_tEditFlags_NoAlpha;        // This is by default if you call color_tPicker3() instead of color_tPicker4()
        if (alpha_bar)         flags |= ImGuicolor_tEditFlags_AlphaBar;
        if (!side_preview)     flags |= ImGuicolor_tEditFlags_NoSidePreview;
        if (picker_mode == 1)  flags |= ImGuicolor_tEditFlags_PickerHueBar;
        if (picker_mode == 2)  flags |= ImGuicolor_tEditFlags_PickerHueWheel;
        if (display_mode == 1) flags |= ImGuicolor_tEditFlags_NoInputs;       // Disable all RGB/HSV/Hex displays
        if (display_mode == 2) flags |= ImGuicolor_tEditFlags_DisplayRGB;     // Override display mode
        if (display_mode == 3) flags |= ImGuicolor_tEditFlags_DisplayHSV;
        if (display_mode == 4) flags |= ImGuicolor_tEditFlags_DisplayHex;
        ImGui::color_tPicker4("Mycolor_t##4", (float*)&color, flags, ref_color ? &ref_color_v.x : NULL);

        ImGui::Text("Programmatically set defaults:");
        ImGui::SameLine(); HelpMarker("Setcolor_tEditOptions() is designed to allow you to set boot-time default.\nWe don't have Push/Pop functions because you can force options on a per-widget basis if needed, and the user can change non-forced ones with the options menu.\nWe don't have a getter to avoid encouraging you to persistently save values that aren't forward-compatible.");
        if (ImGui::Button("Default: Uint8 + HSV + Hue Bar"))
            ImGui::Setcolor_tEditOptions(ImGuicolor_tEditFlags_Uint8 | ImGuicolor_tEditFlags_DisplayHSV | ImGuicolor_tEditFlags_PickerHueBar);
        if (ImGui::Button("Default: Float + HDR + Hue Wheel"))
            ImGui::Setcolor_tEditOptions(ImGuicolor_tEditFlags_Float | ImGuicolor_tEditFlags_HDR | ImGuicolor_tEditFlags_PickerHueWheel);

        // HSV encoded support (to avoid RGB<>HSV round trips and singularities when S==0 or V==0)
        static ImVec4 color_stored_as_hsv(0.23f, 1.0f, 1.0f, 1.0f);
        ImGui::Spacing();
        ImGui::Text("HSV encoded colors");
        ImGui::SameLine(); HelpMarker("By default, colors are given to color_tEdit and color_tPicker in RGB, but ImGuicolor_tEditFlags_InputHSV allows you to store colors as HSV and pass them to color_tEdit and color_tPicker as HSV. This comes with the added benefit that you can manipulate hue values with the picker even when saturation or value are zero.");
        ImGui::Text("color_t widget with InputHSV:");
        ImGui::color_tEdit4("HSV shown as RGB##1", (float*)&color_stored_as_hsv, ImGuicolor_tEditFlags_DisplayRGB | ImGuicolor_tEditFlags_InputHSV | ImGuicolor_tEditFlags_Float);
        ImGui::color_tEdit4("HSV shown as HSV##1", (float*)&color_stored_as_hsv, ImGuicolor_tEditFlags_DisplayHSV | ImGuicolor_tEditFlags_InputHSV | ImGuicolor_tEditFlags_Float);
    ...
    if (ImGui::TreeNode("Vertical Sliders"))
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            ImGui::PushID(i);
            ImGui::PushStylecolor_t(ImGuiCol_FrameBg, (ImVec4)Imcolor_t::HSV(i/7.0f, 0.5f, 0.5f));
            ImGui::PushStylecolor_t(ImGuiCol_FrameBgHovered, (ImVec4)Imcolor_t::HSV(i/7.0f, 0.6f, 0.5f));
            ImGui::PushStylecolor_t(ImGuiCol_FrameBgActive, (ImVec4)Imcolor_t::HSV(i/7.0f, 0.7f, 0.5f));
            ImGui::PushStylecolor_t(ImGuiCol_SliderGrab, (ImVec4)Imcolor_t::HSV(i/7.0f, 0.9f, 0.9f));
            ImGui::VSliderFloat("##v", ImVec2(18,160), &values[i], 0.0f, 1.0f, "");
            if (ImGui::IsItemActive() || ImGui::IsItemHovered())
                ImGui::SetTooltip("%.3f", values[i]);
            ImGui::PopStylecolor_t(4);
            ImGui::PopID();
        }
        ImGui::PopID();
    ...
    if (ImGui::TreeNode("Drag and Drop"))
    {
        ...
        {
            // color_tEdit widgets automatically act as drag source and drag target.
            // They are using standardized payload strings IMGUI_PAYLOAD_TYPE_COLOR_3F and IMGUI_PAYLOAD_TYPE_COLOR_4F to allow your own widgets
            // to use colors in their drag and drop interaction. Also see the demo in color_t Picker -> Palette demo.
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
    {
        ...
        ImGui::Combo("Item Type", &item_type, "Text\0Button\0Button (w/ repeat)\0Checkbox\0SliderFloat\0InputText\0InputFloat\0InputFloat3\0color_tEdit4\0MenuItem\0TreeNode\0TreeNode (w/ double-click)\0ListBox\0", 20);
        ImGui::SameLine();
        ...
        if (item_type == 8) { ret = ImGui::color_tEdit4("ITEM: color_tEdit4", col4f); }                     // Testing multi-component items (IsItemXXX flags are reported merged)
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Child windows"))
    {
        ...
        //   See "Widgets" -> "Querying Status (Active/Focused/Hovered etc.)" section for more details about this.
        {
            ...
            ImGui::PushStylecolor_t(ImGuiCol_ChildBg, IM_COL32(255, 0, 0, 100));
    ...
    if (ImGui::TreeNode("Basic Horizontal Layout"))
    {
        ...
        ImGui::Textcolor_ted(ImVec4(1,1,0,1), "Sailor");
    ...
    if (ImGui::TreeNode("Scrolling"))
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (window_visible) // Avoid calling SetScrollHereY when running with culled items
            {
                for (int item = 0; item < 100; item++)
                {
                    ...
                    {
                        ImGui::Textcolor_ted(ImVec4(1, 1, 0, 1), "Item %d", item);
        ...
        for (int line = 0; line < lines; line++)
        {
            ...
            for (int n = 0; n < num_buttons; n++)
            {
                ...
                float hue = n*0.05f;
                ImGui::PushStylecolor_t(ImGuiCol_Button, (ImVec4)Imcolor_t::HSV(hue, 0.6f, 0.6f));
                ImGui::PushStylecolor_t(ImGuiCol_ButtonHovered, (ImVec4)Imcolor_t::HSV(hue, 0.7f, 0.7f));
                ImGui::PushStylecolor_t(ImGuiCol_ButtonActive, (ImVec4)Imcolor_t::HSV(hue, 0.8f, 0.8f));
...
static void ShowDemoWindowPopups()
{
    ...
    if (ImGui::TreeNode("Modals"))
    {
        ...
        if (ImGui::BeginPopupModal("Stacked 1", NULL, ImGuiWindowFlags_MenuBar))
        {
            ...
            ImGui::Text("Hello from Stacked The First\nUsing style.color_ts[ImGuiCol_ModalWindowDimBg] behind it.");

            // Testing behavior of widgets stacking their own regular popups over the modal.
            static int item = 1;
            static float color[4] = { 0.4f,0.7f,0.0f,0.5f };
            ImGui::Combo("Combo", &item, "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");
            ImGui::color_tEdit4("color", color);
...
static void ShowDemoWindowMisc()
{
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        if (ImGui::TreeNode("Dragging"))
        {
            ...
            if (ImGui::IsItemActive())
                ImGui::GetForegroundDrawList()->AddLine(io.MouseClickedPos[0], io.MousePos, ImGui::Getcolor_tU32(ImGuiCol_Button), 4.0f); // Draw a line between the button and the mouse cursor
...
bool ImGui::ShowStyleSelector(const char* label)
{
    ...
    if (ImGui::Combo(label, &style_idx, "Classic\0Dark\0Light\0"))
    {
        switch (style_idx)
        {
        case 0: ImGui::Stylecolor_tsClassic(); break;
        case 1: ImGui::Stylecolor_tsDark(); break;
        case 2: ImGui::Stylecolor_tsLight(); break;
        }
        return true;
    }
    return false;
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    if (ImGui::ShowStyleSelector("color_ts##Selector"))
        ref_saved_style = style;
    ...
    if (ImGui::Button("Revert Ref"))
        style = *ref;
    ImGui::SameLine();
    HelpMarker("Save/Revert in local non-persistent storage. Default color_ts definition are not affected. Use \"Export\" below to save them somewhere.");
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
    {
        if (ImGui::BeginTabItem("Sizes"))
        {
            ...
            ImGui::Combo("color_tButtonPosition", (int*)&style.color_tButtonPosition, "Left\0Right\0");
        ...
        if (ImGui::BeginTabItem("color_ts"))
        {
            ...
            if (ImGui::Button("Export"))
            {
                if (output_dest == 0)
                    ...
                    ImGui::LogToTTY();
                ImGui::LogText("ImVec4* colors = ImGui::GetStyle().color_ts;" IM_NEWLINE);
                for (int i = 0; i < ImGuiCol_COUNT; i++)
                {
                    const ImVec4& col = style.color_ts[i];
                    const char* name = ImGui::GetStylecolor_tName(i);
                    if (!output_only_modified || memcmp(&col, &ref->color_ts[i], sizeof(ImVec4)) != 0)
                        ImGui::LogText("colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" IM_NEWLINE, name, 23 - (int)strlen(name), "", col.x, col.y, col.z, col.w);
                }
                ImGui::LogFinish();
            }
            ImGui::SameLine(); ImGui::SetNextItemWidth(120); ImGui::Combo("##output_type", &output_dest, "To Clipboard\0To TTY\0");
            ImGui::SameLine(); ImGui::Checkbox("Only Modified color_ts", &output_only_modified);

            static ImGuiTextFilter filter;
            filter.Draw("Filter colors", ImGui::GetFontSize() * 16);

            static ImGuicolor_tEditFlags alpha_flags = 0;
            ImGui::RadioButton("Opaque", &alpha_flags, 0); ImGui::SameLine();
            ImGui::RadioButton("Alpha", &alpha_flags, ImGuicolor_tEditFlags_AlphaPreview); ImGui::SameLine();
            ImGui::RadioButton("Both", &alpha_flags, ImGuicolor_tEditFlags_AlphaPreviewHalf); ImGui::SameLine();
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++)
            {
                ...
                if (!filter.PassFilter(name))
                    continue;
                ImGui::PushID(i);
                ImGui::color_tEdit4("##color", (float*)&style.color_ts[i], ImGuicolor_tEditFlags_AlphaBar | alpha_flags);
                if (memcmp(&style.color_ts[i], &ref->color_ts[i], sizeof(ImVec4)) != 0)
                {
                    // Tips: in a real user application, you may want to merge and use an icon font into the main font, so instead of "Save"/"Revert" you'd use icons.
                    // Read the FAQ and docs/FONTS.txt about using icon fonts. It's really easy and super convenient!
                    ImGui::SameLine(0.0f, style.ItemInnerSpacing.x); if (ImGui::Button("Save")) ref->color_ts[i] = style.color_ts[i];
                    ImGui::SameLine(0.0f, style.ItemInnerSpacing.x); if (ImGui::Button("Revert")) style.color_ts[i] = ref->color_ts[i];
                }
                        ...
                        for (unsigned int base = 0; base <= IM_UNICODE_CODEPOINT_MAX; base += 256)
                        {
                            ...
                            if (count > 0 && ImGui::TreeNode((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base + 255, count, count > 1 ? "glyphs" : "glyph"))
                            {
                                ...
                                for (unsigned int n = 0; n < 256; n++)
                                {
                                    ...
                                    if (glyph)
                                        font->RenderChar(draw_list, cell_size, cell_p1, ImGui::Getcolor_tU32(ImGuiCol_Text), (ImWchar)(base + n)); // We use ImFont::RenderChar as a shortcut because we don't have UTF-8 conversion functions available to generate a string.
...
static void ShowExampleMenuFile()
{
    ...
    if (ImGui::BeginMenu("color_ts"))
    {
        float sz = ImGui::GetTextLineHeight();
        for (int i = 0; i < ImGuiCol_COUNT; i++)
        {
            const char* name = ImGui::GetStylecolor_tName((ImGuiCol)i);
            ImVec2 p = ImGui::GetCursorScreenPos();
            ImGui::GetWindowDrawList()->AddRectFilled(p, ImVec2(p.x+sz, p.y+sz), ImGui::Getcolor_tU32((ImGuiCol)i));
...
struct ExampleAppConsole
{
    ...
    void    Draw(const char* title, bool* p_open)
    {
        ...
        for (int i = 0; i < Items.Size; i++)
        {
            ...
            if (strstr(item, "[error]"))            { ImGui::PushStylecolor_t(ImGuiCol_Text, ImVec4(1.0f, 0.4f, 0.4f, 1.0f)); pop_color = true; }
            else if (strncmp(item, "# ", 2) == 0)   { ImGui::PushStylecolor_t(ImGuiCol_Text, ImVec4(1.0f, 0.8f, 0.6f, 1.0f)); pop_color = true; }
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    if (ImGui::BeginTabBar("##TabBar"))
    {
        ...
        if (ImGui::BeginTabItem("Primitives"))
        {
            ...
            ImGui::color_tEdit4("color_t", &colf.x);
            const ImVec2 p = ImGui::GetCursorScreenPos();
            const ImU32 col = Imcolor_t(colf);
            ...
            draw_list->AddRectFilledMulticolor_t(ImVec2(x, y), ImVec2(x + sz, y + sz), IM_COL32(0, 0, 0, 255), IM_COL32(255, 0, 0, 255), IM_COL32(255, 255, 0, 255), IM_COL32(0, 255, 0, 255));
        ...
        if (ImGui::BeginTabItem("Canvas"))
        {
            ...
            draw_list->AddRectFilledMulticolor_t(canvas_pos, ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), IM_COL32(50, 50, 50, 255), IM_COL32(50, 50, 60, 255), IM_COL32(60, 60, 70, 255), IM_COL32(50, 50, 60, 255));
...
struct MyDocument
{
    ...
    ImVec4      color_t;          // An arbitrary variable associated to the document
    ...
    static void DisplayContents(MyDocument* doc)
    {
        ...
        ImGui::PushStylecolor_t(ImGuiCol_Text, doc->color_t);
        ImGui::TextWrapped("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.");
        ImGui::PopStylecolor_t();
        ...
        if (ImGui::Button("Save", ImVec2(100, 0)))
            doc->DoSave();
        ImGui::color_tEdit3("color", &doc->color_t.x);  // Useful to test drag and drop and hold-dragged-to-open-tab behavior.
        ImGui::PopID();
    }
