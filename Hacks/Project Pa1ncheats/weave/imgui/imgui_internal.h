...
struct ImGuicolor_tMod;               // Stacked color modifier, backup of modified data so we can restore it
...
struct ImGuiContext
{
    ...
    // Shared stacks
    ImVector<ImGuicolor_tMod> color_tModifiers;                     // Stack for PushStylecolor_t()/PopStylecolor_t()
    ...
    ImGuicolor_tEditFlags     color_tEditOptions;                   // Store user options for color edit widgets
    float                   color_tEditLastHue;                   // Backup of last Hue associated to Lastcolor_t[3], so we can restore Hue in lossy RGB<>HSV round trips
    float                   color_tEditLastcolor_t[3];
    ImVec4                  color_tPickerRef;                     // Initial/reference color at the time of opening the color picker.
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) : BackgroundDrawList(&DrawListSharedData), ForegroundDrawList(&DrawListSharedData)
    {
        ...
        TempInputTextId = 0;
        color_tEditOptions = ImGuicolor_tEditFlags__OptionsDefault;
        color_tEditLastHue = 0.0f;
        color_tEditLastcolor_t[0] = color_tEditLastcolor_t[1] = color_tEditLastcolor_t[2] = FLT_MAX;
...
namespace ImGui
{
    ...
    IMGUI_API void          Rendercolor_tRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, float grid_step, ImVec2 grid_off, float rounding = 0.0f, int rounding_corners_flags = ~0);
    ...
    inline void RenderArrow(ImVec2 pos, ImGuiDir dir, float scale=1.0f) { ImGuiWindow* window = GetCurrentWindow(); RenderArrow(window->DrawList, pos, Getcolor_tU32(ImGuiCol_Text), dir, scale); }
    inline void RenderBullet(ImVec2 pos)                                { ImGuiWindow* window = GetCurrentWindow(); RenderBullet(window->DrawList, pos, Getcolor_tU32(ImGuiCol_Text)); }
#endif
    ...
    // color_t
    IMGUI_API void          color_tTooltip(const char* text, const float* col, ImGuicolor_tEditFlags flags);
    IMGUI_API void          color_tEditOptionsPopup(const float* col, ImGuicolor_tEditFlags flags);
    IMGUI_API void          color_tPickerOptionsPopup(const float* ref_col, ImGuicolor_tEditFlags flags);

    // Plot
    IMGUI_API void          PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size);

    // Shade functions (write over already created vertices)
    IMGUI_API void          ShadeVertsLinearcolor_tGradientKeepAlpha(ImDrawList* draw_list, int vert_start_idx, int vert_end_idx, ImVec2 gradient_p0, ImVec2 gradient_p1, ImU32 col0, ImU32 col1);
