...
bool GUI::Form::BeginWindow( std::string name ) {
	...
	// outline
	D3D::Rect(ctx->pos, ctx->size + 1, Color::Palette_t::MenuColors_t::Outlines().OverrideAlpha(255 * ctx->animation, true));
...
void GUI::Form::EndWindow( ) {
	...
	static float rainbow =  100 * g_csgo.m_globals->m_frametime;
	...
	D3D::RectFilled(ctx->pos + vec2_t(0, 575), vec2_t(700, 26), Color(10, 10, 10).OverrideAlpha(180 * ctx->animation, true));
	D3D::Rect(ctx->pos + vec2_t(0, 576), vec2_t(701, 27), Color::Palette_t::MenuColors_t::Outlines());

	D3D::Text(ctx->pos + vec2_t(6, 5), XOR("Tweex-csgo"), Color(255, 255, 255).OverrideAlpha(rainbow * ctx->animation, true), D3D::D3D9Fonts[D3D::TAB_FONT], 0);
	//D3D::Text(ctx->pos + vec2_t(1, 579), XOR("Build: fixed | Name: ") + name, Color(255, 255, 255).OverrideAlpha(255 * ctx->animation, true), D3D::D3D9Fonts[D3D::INFO_FONT], 0); // computer name
	D3D::Text(ctx->pos + vec2_t(1, 579), XOR("Build: fixed | Name: tweexer"), Color(255, 255, 255).OverrideAlpha(255 * ctx->animation, true), D3D::D3D9Fonts[D3D::INFO_FONT], 0); // without 
	// tabs
	if( !ctx->Tabs.empty( ) ) {
		...
		{
			TabPos.at( i ) = ( i == 0 ) ? ctx->pos + vec2_t( ctx->size.x / 4, 3 ) : TabPos.at( i - 1 ) + vec2_t( TabSize.at( i - 1 ).x + 10, 0 );
			...
			bool Hovered = InputHelper::Hovered( TabPos.at( i ) - vec2_t( 0, 3 ), vec2_t( TabSize.at( i ).x, 20 ) );
