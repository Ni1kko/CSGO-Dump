...
void Animations::AnimationInfo_t::UpdateAnimations(LagComp::LagRecord_t* pRecord, LagComp::LagRecord_t* pPreviousRecord)
{
	...
	for (int i = 0; i < pRecord->m_iChoked; ++i)
	{
		...
		if (pRecord->m_iChoked - 1 == i)
		{
			...
			vecPreviousOrigin = math::Interpolate(pPreviousRecord->m_vecOrigin, pRecord->m_vecOrigin, flLerp);
			m_pEntity->SetAbsOrigin(vecPreviousOrigin);
			auto abs_angles = math::Interpolate(pPreviousRecord->m_angAbsAngles, pRecord->m_angAbsAngles, flLerp);
			m_pEntity->SetAbsAngles(abs_angles);
...
void Animations::UpdateFakeAnimations() {
	...
	if (allocate || change) {
		...
		g_cl.m_fake_state->m_flUpdateTimeDelta = fmaxf(g_csgo.m_globals->m_curtime - g_cl.m_fake_state->m_flLastClientSideAnimationUpdateTime, 0.0);
