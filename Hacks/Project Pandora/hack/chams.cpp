...
void Chams::RenderFakeChams()
{
	...
	Color color2 = g_cfg[XOR("esp_chams_desync_color")].get_color();
	...
	if (g_cl.m_fake_matrix)
	{
		...
		{
		case 0: SetupMaterial(g_chams.debugdrawflat, color, false);
			break;
		case 1: SetupMaterial(g_chams.glowarmsrace, color, false);
			break;
		case 2: SetupMaterial(g_chams.esoglow, color, false);
			break;
		}
		...
		if (tint)
			tint->set_vector(vec3_t(color2.r() / 255.f, color2.g() / 255.f, color2.b() / 255.f));
		if (tint2)
			tint2->set_vector(vec3_t(color2.r() / 255.f, color2.g() / 255.f, color2.b() / 255.f));

		SetAlpha(color2.a() / 255.f);
...
void Chams::RenderPlayer(Player* player)
{
	...
	if (player->m_bIsLocalPlayer())
	{
		...
		else if (g_cfg[XOR("esp_chams_local_enable")].get<bool>())
		{
			...
			case 2: SetupMaterial(g_chams.debugdrawflat, color, false);
				break;
			case 3: SetupMaterial(g_chams.glowarmsrace, color, true);
				break;
			case 4: SetupMaterial(g_chams.esoglow, color, true);
				break;
			}
	...
	else
	{
		...
		if (enemy && g_cfg[XOR("esp_chams_enemies")].get<bool>())
		{
			...
			switch (g_cfg[XOR("esp_chams_enemies_material")].get<int>())
			{
			case 0:
				break;
			case 1: SetupMaterial(g_chams.debugambientcube, color, false);
		...
		else if (!enemy && g_cfg[XOR("esp_chams_team")].get<bool>())
		{
			if (g_cfg[XOR("esp_chams_team_invis")].get<bool>())
			{
				...
				switch (g_cfg[XOR("esp_chams_enemies_invis_material")].get<int>())
				{
				case 0:
					break;
				case 1: SetupMaterial(g_chams.debugambientcube, color, false);
