...
void events::player_hurt(IGameEvent* evt)
{
	...
	if (evt && g_cfg[XOR("misc_logs_harmed")].get<bool>())
	{
		...
		if (attacker != g_cl.m_local && victim == g_cl.m_local)
		{
			...
			g_notify.add(tfm::format(XOR("harmed by %s for %i in the %s"), name, evt->m_keys->FindKey(HASH("dmg_health"))->GetInt(), g_shots.m_groups[evt->m_keys->FindKey(HASH("hitgroup"))->GetInt()]));
		}
	}
}
...
void events::bullet_impact( IGameEvent* evt ) {
	...
	//g_csgo.m_debug_overlay->AddBoxOverlay(pos, vec3_t(-2, -2, -2), vec3_t(2, 2, 2), ang_t(0, 0, 0), 0, 255, 0, 127, 4);
}
