...
void Visuals::HitmarkerScreen()
{
	...
	int x = g_cl.m_width / 2, y = g_cl.m_height / 2, alpha = (1.f - complete) * 240;
...
void Visuals::HitmarkerWorld( ) {
	...
	for( size_t i{ }; i < m_world_hitmarkers.size( ); ++i ) {
		...
		if( g_inputpred.m_stored_variables.m_curtime - info.m_time > 2.0f ) {
...
void Visuals::DrawProjectile(Weapon* entity)
{
	vec2_t screen_size;
	vec3_t origin = entity->GetAbsOrigin();

	if (!render::WorldToScreen(origin, screen_size))
		return;

	Color color = g_cfg[XOR("visuals_misc_projectiles_color")].get_color();

	if (entity->is(HASH("CDecoyProjectile")))
	{
		render::grenade_esp.string(screen_size.x, screen_size.y, color, XOR("DECOY GRENADE"), render::ALIGN_CENTER);
	}
	else if (entity->is(HASH("CMolotovProjectile")))
	{
		render::grenade_esp.string(screen_size.x, screen_size.y, color, XOR("MOLOTOV"), render::ALIGN_CENTER);
	}
	else if (entity->is(HASH("CBaseCSGrenadeProjectile")))
	{
		const model_t* model = entity->GetModel();

		if (model)
		{
			std::string name{ entity->GetModel()->m_name };

			if (name.find(XOR("flashbang")) != std::string::npos)
			{
				render::grenade_esp.string(screen_size.x, screen_size.y, color, XOR("FLASHBANG"), render::ALIGN_CENTER);
			}
			else if (name.find(XOR("fraggrenade")) != std::string::npos)
			{
				render::grenade_esp.string(screen_size.x, screen_size.y, color, XOR("HE GRENADE"), render::ALIGN_CENTER);
			}
		}
	}
	else if (entity->is(HASH("CInferno")))
	{
		render::grenade_esp.string(screen_size.x, screen_size.y, color, XOR("MOLOTOV"), render::ALIGN_CENTER);
	}
	else if (entity->is(HASH("CSmokeGrenadeProjectile")))
	{
		render::grenade_esp.string(screen_size.x, screen_size.y, color, XOR("SMOKE"), render::ALIGN_CENTER);
	}
}
...
void Visuals::DrawItem( Weapon* item ) {
	...
	if( g_cfg[ XOR( "visuals_misc_dropped_bomb" ) ].get<bool>( ) && item->is( HASH( "CC4" ) ) )
		render::grenade_esp.string( screen.x, screen.y, { 150, 200, 60, 0xb4 }, XOR( "C4" ), render::ALIGN_CENTER );
	...
	// normal item, get its name.
		...
		if( g_cfg[ XOR( "visuals_misc_dropped_weapons" ) ].get<bool>( ) ) {
			...
			render::grenade_esp.string( screen.x, screen.y, col, name, render::ALIGN_CENTER );
		}
	}
	...
	render::grenade_esp.string( screen.x, screen.y - render::grenade_esp.m_size.m_height - 1, col, ammo, render::ALIGN_CENTER );
}
...
void Visuals::DrawPlayer( Player* player ) {
	...
	if( health_esp ) {
		...
		if( hp < 100 )
			render::grenade_esp.string( box.x - 5, y + ( h - fill ) - 5, { 255, 255, 255, low_alpha }, std::to_string( hp ), render::ALIGN_CENTER );
	}
	...
	// draw flags.
	{
		...
		for( size_t i{ }; i < flags.size( ); ++i ) {
			...
			int offset = i * ( render::grenade_esp.m_size.m_height - 1 );

			// draw flag.
			render::grenade_esp.string( box.x + box.w + 3, box.y + offset, f.second, f.first );
		}
	}
			...
			if( data ) {
				...
				if( max != -1 && ( enemy && g_cfg[ XOR( "esp_enemies_ammo" ) ].get<bool>( ) ) || ( !enemy && g_cfg[ XOR( "esp_team_ammo" ) ].get<bool>( ) ) ) {
					...
					if( current <= ( int )std::round( max / 5 ) && !reload )
						render::grenade_esp.string( box.x + bar, box.y + box.h, { 255, 255, 255, low_alpha }, std::to_string( current ), render::ALIGN_CENTER );
				}
				...
				if( weaponText ) {
					...
					render::grenade_esp.string( box.x + box.w / 2, box.y + box.h + offset, weapon_clr.OverrideAlpha( low_alpha ), name, render::ALIGN_CENTER );
				}
...
void Visuals::DrawPlantedC4( Entity* ent ) {
	...
	if( mode_2d ) {
		if( explode_time_diff > 0.f )
			render::grenade_esp.string( g_cl.m_width * 0.5f, 65, colors::white, time_str, render::ALIGN_CENTER );

		if( g_cl.m_local->alive( ) )
			render::grenade_esp.string( g_cl.m_width * 0.5f, 65 + render::grenade_esp.m_size.m_height, damage_color, damage_str, render::ALIGN_CENTER );
	}

	// 'on bomb (3D)'.
	if( mode_3d && is_visible ) {
		if( explode_time_diff > 0.f )
			render::grenade_esp.string( screen_pos.x, screen_pos.y, colors::white, time_str, render::ALIGN_CENTER );

		// only render damage string if we're alive.
		if( g_cl.m_local->alive( ) )
			render::grenade_esp.string( screen_pos.x, ( int )screen_pos.y + render::grenade_esp.m_size.m_height, damage_color, damage_str, render::ALIGN_CENTER );
	}
}
