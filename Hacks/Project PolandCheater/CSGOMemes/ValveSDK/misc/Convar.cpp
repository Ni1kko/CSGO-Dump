...
CCommand::CCommand(int nargsCount, const char **ppArgV)
{
    assert(nargsCount > 0);
...
bool CCommand::Tokenize(const char *pCommand, characterset_t *pBreakSet)
{
    ...
    while(bufParse.IsValid() && (m_nargsCount < COMMAND_MAX_argsCount)) {
        ...
        m_ppArgv[m_nargsCount++] = pArgvBuf;
        if(m_nargsCount >= COMMAND_MAX_argsCount) {
...
{
    int nargsCount = argsCount();
    for(int i = 1; i < nargsCount; i++) {
        if(!_stricmp(Arg(i), pName))
            return (i + 1) < nargsCount ? Arg(i + 1) : "";
    }
    return 0;
}
