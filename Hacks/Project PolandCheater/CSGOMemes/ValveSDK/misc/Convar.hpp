...
public:
    CCommand();
    CCommand(int nargsCount, const char **ppArgV);
...
private:
    ...
    const char*	m_ppArgv[COMMAND_MAX_argsCount];
};
...
inline int CCommand::argsCount() const
{
    return m_nargsCount;
}
...
{
    return m_nargsCount ? (const char**)m_ppArgv : NULL;
}
...
{
    return m_nargsCount ? m_pArgSBuffer : "";
}

inline const char *CCommand::Arg(int nIndex) const
{
    // FIXME: Many command handlers appear to not be particularly careful
    // about checking for valid argsCount range. For now, we're going to
    // do the extra check and return an empty string if it's out of range
    if(nIndex < 0 || nIndex >= m_nargsCount)
        return "";
