...
void W::MainWindow(IDirect3DDevice9* pDevice)
{
	...
	if (!I::Engine->IsTakingScreenshot() && !I::Engine->IsDrawingLoadingImage())
	{
		...
		ImGui::BeginMainMenuBar();
		{
			...
			const char* const szName = XorStr("sorryhack | " __DATE__);
	...
	if (bMainOpened)
	{
		...
		ImGui::Begin(XorStr("sorryhack - cheat_codes - lifetime"), &bMainOpened, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoCollapse);
		{
			...
			// add tabs
			static std::array<CTab, 2U> const arrTabs =
			{
				CTab{ "lagger", &T::RageBot },
				//CTab{ "players", &T::LegitBot },
				//CTab{ "gotv", &T::Visuals },
...
//				ImGui::Selectable(XorStr("message##ragebot"), &C::Get<bool>(Vars.bRage), ImGuiSelectableFlags_None, ImVec2(30, 0));
				ImGui::TextUnformatted(XorStr("message"));
			...
			ImGui::Combo("message type", &C::Get<int>(Vars.iPacketType), XorStr("nop\0disconnect\0file\0split screen user\0tick\0string command\0set convar\0signonstate\0client info\0move\0voice data\0baseline ack\0listen events\0respond cvar value\0file crc check\0loading progress\0split player connect\0client message\0cmd keyvalues\0hltv replay\0\0"));
			ImGui::Combo("message method", &C::Get<int>(Vars.iBuildMethod), XorStr("build\0repeat\0\0"));
			ImGui::Combo("mode select", &C::Get<int>(Vars.iTimingType), XorStr("manual\0always\0\0"));
			ImGui::Combo("timing select", &C::Get<int>(Vars.iLagType), XorStr("cooker\0constant\0\0"));
			ImGui::PopStyleVar();
	...
	ImGui::NextColumn();
	{
		ImGui::BeginChild(XorStr("ragebot.antiaim"), ImVec2(), true, ImGuiWindowFlags_MenuBar);
		{
			...
			{
				ImGui::PushStyleVar(ImGuiStyleVar_SelectableTextAlign, ImVec2(0.5f, 0.5f));
				//ImGui::Selectable(XorStr("timing"), &C::Get<bool>(Vars.bAntiAim), ImGuiSelectableFlags_None, ImVec2(40, 0));
				ImGui::TextUnformatted(XorStr("timing"));
			...
			ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(style.FramePadding.x, -1));
			if (C::Get<int>(Vars.iTimingType) == 0) {
				ImGui::HotKey(XorStr("lag key"), &C::Get<int>(Vars.iLagKey));
			}
			ImGui::SliderInt(XorStr("amount"), &C::Get<int>(Vars.iLagAmount), 1, 1000, "%d");
			ImGui::SliderInt(XorStr("size"), &C::Get<int>(Vars.iLagSize), 1, 1000, "%d");

			if (C::Get<int>(Vars.iLagType) == 1) {
				ImGui::SliderInt(XorStr("delay"), &C::Get<int>(Vars.iLagInterval), 0, 100, "%d");
			}
