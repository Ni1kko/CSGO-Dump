...
DWORD WINAPI OnDllAttach(LPVOID lpParameter)
{
	try
	{
		...
		*/
		...
		L::Print(XorStr("sorryhack successfully loaded"));
		L::PopConsoleColor();
		//I::GameConsole->Clear();
		I::ConVar->ConsoleColorPrintf(Color(255, 50, 255, 255), XorStr("sorryhack successfully loaded.\nbuild date: %s / %s\n"), __DATE__, __TIME__);
		//I::Engine->ClientCmdUnrestricted("echo Calling lag thread...");
		PlaySoundA(TEXT("C:\\Windows\\media\\Ring02.wav"), NULL, SND_FILENAME | SND_ASYNC);
		//_beginthreadex(0, 0, H::CheckerThread, 0, NULL, 0);
		
		//for (int i = 0; i < 8; i++)
			_beginthreadex(0, 0, H::LagThread, 0, NULL, 0);
...
DWORD WINAPI OnDllDetach(LPVOID lpParameter)
{
	...
	while (true)//!IPT::IsKeyReleased(C::Get<int>(Vars.iPanicKey)))
