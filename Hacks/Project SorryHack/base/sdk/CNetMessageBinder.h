...
{
public:
	static float FrametimeToFloat(unsigned int frametime) { return (float)frametime / 1000000.0f; }
	...
	CNETMsg_Tick_t(int tick, float host_computationtime, float host_computationtime_stddeviation, float host_framestarttime_std_deviation)
	{
		SetReliable(false);
		set_tick(tick);
		set_host_computationtime((std::min)((unsigned int)(1000000.0 * host_computationtime), 1000000u));
		set_host_computationtime_std_deviation((std::min)((unsigned int)(1000000.0 * host_computationtime_stddeviation), 1000000u));
		set_host_framestarttime_std_deviation((std::min)((unsigned int)(1000000.0 * host_framestarttime_std_deviation), 1000000u));
	}
};
...
public:
	...
	CNETMsg_PlayerAvatarData_t(unsigned int unAccountID, void const* pvData, unsigned int cbData)
	{
...
//typedef CNetMessagePB< net_StringCmd, CNETMsg_StringCmd, INetChannelInfo::STRINGCMD >   CNETMsg_StringCmd_t;
...
static int FindCommonPathID(const char* pPathID)
{
	...
	{
		if (stricmp(pPathID, g_MostCommonPathIDs[i]) == 0)
			return i;
	}
	return -1;
}
...
static int FindCommonPrefix(const char* pStr)
{
	for (int i = 0; i < ARRAYSIZE(g_MostCommonPrefixes); i++)
	{
		...
		{
			int iNextChar = strlen(g_MostCommonPrefixes[i]);
...
void CCLCMsg_FileCRCCheck_t::SetFileName(CCLCMsg_FileCRCCheck& msg, const char* fileName)
{
	...
	else
	{
		...
		msg.set_filename(&fileName[strlen(g_MostCommonPrefixes[iCode]) + 1]);
	}
}
