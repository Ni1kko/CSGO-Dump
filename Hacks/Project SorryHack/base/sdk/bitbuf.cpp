...
/// Compatability funcs
	...
	inline void Q_memcpy(void* dest, void* src, int count) // Quake Memcpy
	{
		...
		if ((((long)dest | (long)src | count) & 3) == 0)
		{
			...
			for (i = 0; i < count; i++) {
				(reinterpret_cast<std::byte*>(dest))[i] = (reinterpret_cast<std::byte*>(src))[i]; // C style cast crashes compiler... but reinterpret_cast doesn't? What MSCV?????
			}
		}
	}
...
void bf_write::WriteLongLong(int64 val)
{
	...
	WriteUBitLong(pLongs[(*((int*)idx))++], sizeof(long) << 3);
	WriteUBitLong(pLongs[*((int*)idx)], sizeof(long) << 3);
}
...
int64 bf_read::ReadLongLong()
{
	...
	const short endianIndex = 0x0100;
	std::byte* idx = (std::byte*)&endianIndex;
	pLongs[(*(int*)idx)++] = ReadUBitLong(sizeof(long) << 3);
	pLongs[(*(int*)idx)] = ReadUBitLong(sizeof(long) << 3);
