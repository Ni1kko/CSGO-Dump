...
unsigned char *CLZSS::CompressNoAlloc( unsigned char *pInput, int inputLength, unsigned char *pOutputBuf, unsigned int *pOutputSize )
{
	...
	unsigned char *pEncodedPosition = 0;
...
unsigned char* CLZSS::CompressLeave8(unsigned char* pInput, int inputLength, unsigned int* pOutputSize)
{
	unsigned char* pStart = (unsigned char*)malloc(inputLength+sizeof(unsigned int));
	unsigned char* pFinal = CompressNoAlloc(pInput, inputLength, pStart + sizeof(unsigned int), pOutputSize);
	if (!pFinal)
	{
		free(pStart);
		return NULL;
