...
void BulletBeamsEvent::FireGameEvent(IGameEvent *event)
{
	if (!g_LocalPlayer || !event)
		return;
	if (g_Options.more_more_more_bu_ll_ert_bejmz)
	{
		if (g_EngineClient->GetPlayerForUserID(event->GetInt(XorStr("userid"))) == g_EngineClient->GetLocalPlayer() && g_LocalPlayer && g_LocalPlayer->IsAlive())
		{
