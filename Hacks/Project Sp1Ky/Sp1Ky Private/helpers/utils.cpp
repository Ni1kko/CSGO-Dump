...
namespace Utils {
	...
	{
		static auto ranSeed = reinterpret_cast<void(*)(int)>(GetProcAddress(GetModuleHandle(L"vstdlib.dll"), "RandomSeed"));
		if (ranSeed)
			ranSeed(iSeed);
	}
	float RandomFloat(float min, float max)
	{
		static auto ranFloat = reinterpret_cast<float(*)(float, float)>(GetProcAddress(GetModuleHandle(L"vstdlib.dll"), "RandomFloat"));
		if (ranFloat)
	...
	{
		static auto ranInt = reinterpret_cast<int(*)(int, int)>(GetProcAddress(GetModuleHandle(L"vstdlib.dll"), "RandomInt"));
		if (ranInt)
