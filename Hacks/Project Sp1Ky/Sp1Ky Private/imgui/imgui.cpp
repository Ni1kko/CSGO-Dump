...
void ImGui::StyleColorsClassic(ImGuiStyle* dst)
{
	...
	colors[ImGuiCol_Themecolor] = ImVec4(0.90f, 0.90f, 0.90f, 0.50f);
...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
	...
	case ImGuiCol_Themecolor: return "Themecolor";
	}
	IM_ASSERT(0);
	return "Unknown";
}
...
void ImGui::TextC(const char* fmt, va_list args)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	ImGuiContext& g = *GImGui;
	const char* text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
	TextCustom(g.TempBuffer, text_end);
}
...
void ImGui::MenuText(const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	TextC(fmt, args);
...
void ImGui::SpacerText(const char* fmt, ...)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	ImVec2 text_size = ImGui::CalcTextSize(fmt, NULL, false);
	float x1 = window->Pos.x + 22.f;
	float y1 = window->Pos.x + (window->Size.x / 6.f) - 10.f;
	float x2 = y1 + text_size.x + 20.f;
	float y2 = window->Pos.x + window->Size.x - 22.f;
	if (!window->DC.GroupStack.empty())
		x1 += window->DC.IndentX;

	const ImRect bb(ImVec2(x1, window->DC.CursorPos.y + (text_size.y / 2.f)), ImVec2(y1, window->DC.CursorPos.y + (text_size.y / 2.f)));// first line
	const ImRect aa(ImVec2(x2, window->DC.CursorPos.y + (text_size.y / 2.f)), ImVec2(y2, window->DC.CursorPos.y + (text_size.y / 2.f)));// second line
	ItemSize(ImVec2(0.0f, text_size.y));
	if (!ItemAdd(bb, 0))
	{
		if (window->DC.ColumnsCount > 1)
			PushColumnClipRect();
	...
	window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Themecolor));
	window->DrawList->AddLine(aa.Min, ImVec2(aa.Max.x, aa.Min.y), GetColorU32(ImGuiCol_Themecolor));
	window->DrawList->AddText(ImVec2((window->Pos.x + (window->Size.x / 6.f)), aa.Min.y - (text_size.y / 2.3f)), GetColorU32(ImGuiCol_Themecolor), fmt, NULL);
...
void ImGui::TextCustom(const char* text, const char* text_end)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
...
bool ImGui::ButtonEx2(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
	window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 2.f), bb.Max, GetColorU32(ImGuiCol_Separator));
...
bool ImGui::ButtonMenu(const char* label, const ImVec2& size_arg)
{
...
void ImGui::ImageCustom(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& pos, ImVec4 color, bool itemadd)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;

	ImRect bb(window->DC.CursorPos + pos, window->DC.CursorPos + size + pos);
	if (itemadd)
	{
		ItemSize(bb);
		if (!ItemAdd(bb, 0))
	...
	//dont need this anymore cuz valve has magic source code for csgo
	/*for (int i = 0; i < 64; i++)
	{
	if (ImagesFix::size > 64)
	continue;

	if (ImagesFix::size == 0)
	{
	ImagesFix::TextureID[ImagesFix::size] = (void*)user_texture_id; //no item in array
	ImagesFix::size++; //add 1 to size of array
	break; //skip loop
	}
	if (ImagesFix::TextureID[i] == (void*)user_texture_id) //check if item is in array
	break; //skip loop

	if (i > ImagesFix::size) //if we got here that means we didn't find yet item in array
	{
	ImagesFix::TextureID[ImagesFix::size] = (void*)user_texture_id; //if we didn't find the item in array we want to add that item then
	ImagesFix::size++;//add 1 to size of array
	break; //skip loop
	}
	}
	*/
	window->DrawList->AddImage(user_texture_id, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(color));
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	window->DrawList->AddRectFilled(bb_slider.Min, bb_slider.Max, GetColorU32(ImGuiCol_Themecolor), style.GrabRounding);
	PopClipRect();

	window->DrawList->AddRect(grab_bb.Min - ImVec2(2, 3), grab_bb.Max + ImVec2(2, 3) - ImVec2(5.f, 0.f), GetColorU32(ImVec4(0.30f, 0.30f, 0.30f, 1.f)), 0.f);
	window->DrawList->AddRectFilled(grab_bb.Min - ImVec2(1, 2), grab_bb.Max + ImVec2(1, 2) - ImVec2(5.f, 0.f), GetColorU32(g.ActiveId == id ? (ImVec4(0.58f, 0.58f, 0.58f, 1.f)) : (ImVec4(0.82f, 0.82f, 0.82f, 1.f))), 0.f);
	return value_changed;
}
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 5.f, frame_bb.Min.y + style.FramePadding.y - 3.f), label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 0.70, label_size.y + style.FramePadding.y * 0.70)); // We want a square shape to we use Y twice
	...
	if (*v)
	{
		...
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(3, 3), check_bb.Max - ImVec2(3, 3), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
	}
	ImVec2 pos = ImVec2(text_bb.Min.x + 5, check_bb.Min.y + ((check_bb.Max.y - check_bb.Min.y) / 2.f) - (label_size.y / 2.f) - 1);
...
bool ImGui::CheckboxRect(const char* label, bool* v)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 5.f, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
	...
	{
		SameLine(0, style.ItemInnerSpacing.x);
		RenderText(ImVec2(window->DC.CursorPos.x + 5.f, window->DC.CursorPos.y + style.FramePadding.y), label);
...
{
	//ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.76f, 0.76f, 0.76f, 1.f));
	const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items);
	//ImGui::PopStyleColor(); //fix combo color
	return value_changed;
}
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(220.f / 255.f, 220.f / 255.f, 220.f / 255.f, 1.f));
	bool value_changed = false;
