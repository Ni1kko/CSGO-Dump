...
{
	auto* AnimState = pPlayer->GetAnimState();
	auto* Anims = pPlayer->GetAnimOverlays();
	...
	for (int j = backtracking->PlayerRecord[i].records.size() - 1; j >= 0; j--)
	{
		tick_record rec = backtracking->PlayerRecord[i].records.at(0);
	...
	new_record.m_flCycle = Anims->m_flCycle;
	...
	new_record.m_nSequence = Anims->m_nSequence;
	...
	new_record.m_vecMins = pPlayer->GetCollideable()->Mins();
	new_record.m_vecMax = pPlayer->GetCollideable()->Maxs();
	...
	pPlayer->SetupBonesStick(new_record.boneMatrix, 128, 256, INTERFACES::Globals->curtime);
...
void InvalidateBoneCache(SDK::CBaseEntity* entity)
{
