...
CFlashLightEffect* CFlashLight::CreateFlashLight(int nEntIndex, const char *pszTextureName, float flFov, float flFarZ, float flLinearAtten)
{
	...
	CFlashLightEffect *pFlashLight = new CFlashLightEffect; //(CFlashLightEffect*)) INTERFACES::MemAlloc->Alloc(sizeof(CFlashLightEffect));
...
void CFlashLight::RunFrame()
{
	...
	if (UTILS::INPUT::input_handler.GetKeyState(UTILS::INPUT::input_handler.keyBindings(SETTINGS::settings.flashlightkey)) & 1)
	{
