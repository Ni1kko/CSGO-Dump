...
void skinchanger()
{
	auto weapons = g_ctx.m_local->m_hMyWeapons();
	for (size_t i = 0; weapons[i] != INVALID_EHANDLE_INDEX; i++)
	{//*pEntity = INTERFACES::ClientEntityList->GetClientEntityFromHandle(weapons[i]);
		auto *pEntity = g_csgo.m_entitylist()->GetClientEntityFromHandle(weapons[i]);
		if (pEntity)
		{
			weapon_t* pWeapon = (weapon_t*)pEntity;
			if (g_cfg.skins.enable || g_cfg.skins.enableknife)
			{
				int Model = g_cfg.skins.knifemodel;
				int weapon = *pWeapon();
