...
void Hooked_RecvProxy_Viewmodel(CRecvProxyData *pData, void *pStruct, void *pOut)
{
	...
	if (g_cfg.skins.enableknife && g_ctx.m_local)
	{
			...
			pData->m_Value.m_Int == Talon))
		{
			...
			if (g_cfg.skins.knifemodel == 0)
				pData->m_Value.m_Int = iBayonet;
			else if (g_cfg.skins.knifemodel == 9)
				pData->m_Value.m_Int = iBowie;
			else if (g_cfg.skins.knifemodel == 6)
				pData->m_Value.m_Int = iButterfly;
			else if (g_cfg.skins.knifemodel == 7)
				pData->m_Value.m_Int = iFalchion;
			else if (g_cfg.skins.knifemodel == 1)
				pData->m_Value.m_Int = iFlip;
			else if (g_cfg.skins.knifemodel == 2)
				pData->m_Value.m_Int = iGut;
			else if (g_cfg.skins.knifemodel == 5)
				pData->m_Value.m_Int = iHuntsman;
			else if (g_cfg.skins.knifemodel == 3)
				pData->m_Value.m_Int = iKarambit;
			else if (g_cfg.skins.knifemodel == 4)
				pData->m_Value.m_Int = iM9Bayonet;
			else if (g_cfg.skins.knifemodel == 8)
				pData->m_Value.m_Int = iDagger;
			else if (g_cfg.skins.knifemodel == 10)
				pData->m_Value.m_Int = Navaja;
			else if (g_cfg.skins.knifemodel == 11)
				pData->m_Value.m_Int = Stiletto;
			else if (g_cfg.skins.knifemodel == 12)
				pData->m_Value.m_Int = Ursus;
			else if (g_cfg.skins.knifemodel == 13)
				pData->m_Value.m_Int = Talon;
		}
	}
...
void SetViewModelSequence2(const CRecvProxyData *pDataConst, void *pStruct, void *pOut)
{
	...
	if (pViewModel)
	{
		player_t* pOwner = static_cast<player_t *>(g_csgo.m_entitylist()->GetClientEntity(pViewModel->m_hOwner & 0xFFF));

		// Compare the owner entity of this view model to the local player entity.
		if (pOwner && pOwner->EntIndex() == g_csgo.m_engine()->GetLocalPlayer())
		{
