...
void UI::UpdateWeaponSettings()
{
	...
	AimbotWeapon_t settings = {
			...
			noShootEnabled, ignoreJumpEnabled, smokeCheck, flashCheck, hitChanceEnabled, hitChance, autoWallEnabled, autoWallValue, autoAimRealDistance, autoSlow, predEnabled, moveMouse
	};
	...
	ImGui::NextColumn();
	{
		...
		ImGui::BeginChild(XORSTR("COL1"), ImVec2(0, 0), true);
		{
			...
			if( ImGui::Checkbox(XORSTR("Hit Chance"), &hitChanceEnabled) )
				UI::UpdateWeaponSettings();
			if( ImGui::SliderFloat(XORSTR("##HITCHANCE"), &hitChance, 0, 100, "%0.1f") )
				UI::UpdateWeaponSettings();
			ImGui::EndChild();
		}
	}
