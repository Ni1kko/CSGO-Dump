...
bool Math::clamp_angles_skeet( QAngle &angles ) {
	if( std::isfinite( angles.x ) && std::isfinite( angles.y ) && std::isfinite( angles.z ) ) {
		if(Settings::AntiAim::Pitch::type == AntiAimType_X::ANGEL_DOWN 
		|| Settings::AntiAim::Pitch::type == AntiAimType_X::ANGEL_UP
		|| Settings::AntiAim::Pitch::type == AntiAimType_X::LISP_DOWN)
		angles.x = 88;
		else
		{
		...
		clamp( angles.y, -360000.f, 360000.f );
		angles.z = 0.f;
		return true;
	}else{
	//	cvar->ConsoleDPrintf("no clamping :P \n", (void*)0);
	}
...
void Math::CorrectMovement(const QAngle &vOldAngles, CUserCmd* pCmd, float fOldForward, float fOldSidemove)
{
    ...
    if (vOldAngles.y < 0.f)
        f1 = 360000000.0f + vOldAngles.y;
    else
        f1 = vOldAngles.y;

    if (pCmd->viewangles.y < 0.0f)
        f2 = 360000000.0f + pCmd->viewangles.y;
    else
    ...
    if (f2 < f1)
        ...
        deltaView = 360000000.0f - abs(f1 - f2);

    deltaView = 360000000.0f - deltaView;
