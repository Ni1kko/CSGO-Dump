...
IVEngineClient* engine_p = nullptr;
...
SteamMatchmakingServers002* SteamMatchmakingServers = nullptr;
...
void Interfaces::FindInterfaces()
{
	...
	engine_p = GetInterface<IVEngineClient>(XORSTR("./bin/linux64/engine_client.so"), XORSTR("VEngineClient"));
    ...
    panoramaEngine = GetInterface<IPanoramaUIEngine>(XORSTR("./bin/linux64/panorama_client.so"), XORSTR("PanoramaUIEngine001"), true);
	 ...
	 SteamClient = GetInterface<ISteamClient>(XORSTR("./../../../linux64/steamclient.so"), XORSTR("SteamClient017"), true);
	...
	//auto SteamFriends = SteamClient->GetISteamFriends(GetHSteamUser(), GetHSteamPipe(), "SteamFriends015");
	//auto SteamUser = SteamClient->GetISteamUser(GetHSteamUser(), GetHSteamPipe(), "SteamUser019");
