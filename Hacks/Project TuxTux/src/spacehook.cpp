...
char SpaceHook::buildID[NAME_MAX] = {
...
void MainThread()
{
    ...
    cvar->ConsoleDPrintf("PanelArray is at %p\n", (void*)panorama::panelArray);
  	...
  	//    uiEngineVMT->HookVM((void*)Hooks::CreatePanel, 140);
  		// uiEngineVMT->HookVM((void*)GetEngineBuildNumber, 221);
	...
	eventListener = new EventListener({ XORSTR("cs_game_disconnected"), XORSTR("player_connect_full"), XORSTR("player_death"), XORSTR("player_hurt"), XORSTR("switch_team"), XORSTR("bullet_impact") });
    ...
    cvar->ConsoleColorPrintf(ColorRGBA(0, 225, 0), XORSTR("\pysen Successfully loaded.\n"));
...
void __attribute__((destructor)) Shutdown()
{
	...
	*s_bOverridePostProcessingDisable = false;
	...
	cvar->ConsoleColorPrintf(ColorRGBA(255, 0, 0), XORSTR("pysen Unloaded successfully.\n"));
}
void SpaceHook::SelfShutdown()
{
    // Beta Feature.
