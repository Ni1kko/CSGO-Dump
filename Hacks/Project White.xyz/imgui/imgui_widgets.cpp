...
#include "../cheat/config/options.hpp"
#include "../cheat/other/ui.hpp"
...
bool ImGui::TabButtonEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (active && alpha < 0.95f)
	{
		alpha += 0.05f;
	}

	if (active && alpha2 < 0.95f)
	{
		alpha2 += 0.05f;
	}

	GetWindowDrawList()->AddRectFilled(bb.Max, ImVec2(bb.Min.x, bb.Max.y - 50), GetColorU32(ImVec4(0.f, 0.f, 0.f, 1.f)));

	if (active)
	{
		PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.00f));
		RenderTextClippedOverlay(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
		PopStyleColor();
	}
	else
	{
	...
	if (active)
	{
		GetWindowDrawList()->AddRectFilled(bb.Max, ImVec2(bb.Min.x, bb.Max.y - 50), GetColorU32(ImVec4(0.61f, 0.19f, 1.00f, alpha)));
	}
...
bool ImGui::TabButton(const char* label, const bool active, const ImVec2& size_arg)
{
	return TabButtonEx(label, active, size_arg, 0);
}
...
void RenderCheckMarkK(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float sz)
{
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        RenderCheckMarkK(window->DrawList, ImVec2(total_bb.Min.x + 2, total_bb.Min.y + 11), ImColor(60, 60, 60), 9.f);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, frame_col, false, style.FrameRounding, (2.f));
	...
	// Render grab 
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(3, 3), grab_bb.Max - ImVec2(1, 1), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
	RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
	...
	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Max.x + 3, frame_bb.Min.y + 2), label);
