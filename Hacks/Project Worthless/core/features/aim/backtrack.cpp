...
void c_backtrack::update() noexcept {
	...
	if (!vars::checkbox["#backtrack"]->get_bool() || !local_player || !local_player->is_alive()) {
		if (!records->empty())
			records->clear();
	...
	for (int i = 1; i <= interfaces::globals->max_clients; i++) {
		...
		while (records[i].size() > 3 && records[i].size() > static_cast<size_t>(time_to_ticks(static_cast<float>(vars::slider["#backtrack_amount"]->get_value()) / 1000.f)))
			records[i].pop_back();
...
void c_backtrack::run(c_usercmd* cmd) noexcept {
	if (!vars::checkbox["#backtrack"]->get_bool())
		return;
	...
	for (int i = 1; i <= interfaces::globals->max_clients; i++) {
		...
		auto angle = math::calculate_angle2(csgo::local_player->get_eye_pos(), head_position, cmd->viewangles);
