...
void c_sound_esp::event_player_footstep(i_game_event * event) noexcept {
	if (vars::checkbox["#sound_footstep"]->get_bool())
		return;
...
void c_sound_esp::draw() noexcept {
	...
	auto colornormal = (vars::colorpicker["#sound_footstep_color"]->get_color());

	auto red = colornormal.m_red * 255;
	auto green = colornormal.m_green * 255;
	auto blue = colornormal.m_blue * 255;
	auto alpha = colornormal.m_alpha * 255;
