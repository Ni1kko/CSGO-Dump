...
public:
	...
	{
		ConfigList.clear();
		string ConfigDir = SystemDisk + XorStr("XONE\\Settings\\*.json");
	...
	static bool LineGoesThroughSmoke(Vector vStartPos, Vector vEndPos)
	{
		auto LLineGoesThroughSmoke = [&]()->bool
		{
			...
			if (!SearchFunction)
			{
				DWORD dwFunctionAddress = CSX::Memory::FindPattern("client_panorama.dll", SMOK_PATTERN, XorStr("xxxxxxxx????xxx"), 0);
