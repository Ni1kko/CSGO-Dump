...
{
	int item_id = GetItemId(XorStr("SCROLLY"));
	GuiIO &IO = X1Gui().G_IO;
...
bool CGui::SliderFloat(string Label, float *var, float min, float max, const char* display_format)
{
	...
	mouse_to_start = (*var - min) * power; // длина линии
...
bool CGui::ColorEdit(string Label, float *var, Vec2 Size)
{
	...
	bool changed = button_check(r_rect, var[0], XorStr("R:"));
	changed = button_check(g_rect, var[1], XorStr("G:"));
	changed = button_check(b_rect, var[2], XorStr("B:"));
...
{
	DWORD id = GetItemId(XorStr("##Labels"));
	Vec2 pos = GetCursorPos();
	GuiStyle &style = X1Gui().G_Style;
	GuiIO &IO = X1Gui().G_IO;
...
void CGui::EndColumns()//не после BeginColumns
{
