...
#define DCheckBox(eng, rus, var, ops)  X1Gui().CheckBox(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), &var); if (X1Gui().IsItemHovered()) { MainSettings().HintMsg = XorStr(ops); }
/*ImGui::Checkbox(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus) , &var); \
if (ImGui::IsItemHovered()) { GHintMsg = XorStr(ops); }*/

#define DComboBox(eng, rus, var, items_eng, items_rus, ops) X1Gui().ComboBox(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), &var, true ? items_eng : items_rus); \
if (X1Gui().IsItemHovered()) { MainSettings().HintMsg = XorStr(ops); }
/* ImGui::Combo(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus), &var, !MainCheatConfig.RusLanguage ? items_eng : items_rus, IM_ARRAYSIZE(items_eng));\
if (ImGui::IsItemHovered()) { GHintMsg = XorStr(ops); }	*/

#define SliderInts(eng, rus, var,  min_, max_, ops)  X1Gui().SliderInt(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), &var, min_, max_);\
if (X1Gui().IsItemHovered()) { MainSettings().HintMsg = XorStr(ops); }
/*ImGui::SliderInt(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus), &var, min, max);\
if (ImGui::IsItemHovered()) { GHintMsg = XorStr(ops); }*/	

#define SliderIntsProc(eng, rus, var,  min_, max_, ops) X1Gui().SliderIntProc(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), &var, min_, max_); \
if (X1Gui().IsItemHovered()) { MainSettings().HintMsg = XorStr(ops); }
/* ImGui::SliderIntProc(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus), &var, min, max);\
if (ImGui::IsItemHovered()) { GHintMsg = XorStr(ops); }	*/


#define SliderFloats(eng, rus, var,  min_, max_, ops) X1Gui().SliderFloat(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), &var, min_, max_); \
if (X1Gui().IsItemHovered()) { MainSettings().HintMsg = XorStr(ops); }
/*ImGui::SliderFloat(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus), &var, min, max);\
if (ImGui::IsItemHovered()) { GHintMsg = XorStr(ops); }	*/

#define HotsKey(eng, rus, var, ops) X1Gui().HotKey(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), &var); \
if (X1Gui().IsItemHovered()) { MainSettings().HintMsg = XorStr(ops); }
/* ImGui::Hotkey(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus), &var); \
if (ImGui::IsItemHovered()) { GHintMsg = XorStr(ops); }	*/

#define DColorEdit(eng, rus, var) X1Gui().ColorEdit2(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), &var); /*ImGui::MyColorEdit5(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus), var##._color, var##.brainbow, 1 << 7 )*/
#define TextEdit(eng, rus, var, buf, sim) strcpy(buf, var.c_str()); X1Gui().InputText(!MainSettings().Russian ? XorStr(eng) : XorStr(rus), buf, sim); var = buf; /*strcpy(buf, var.c_str()); ImGui::InputText(!MainCheatConfig.RusLanguage ? XorStr(eng) : XorStr(rus), buf, sim); var = buf;*/
...
#define lols(e) string(XorStr(e))
#define lolc(e) XorStr(e)
#define lolsc(e) string(XorStr(e)).c_str()
