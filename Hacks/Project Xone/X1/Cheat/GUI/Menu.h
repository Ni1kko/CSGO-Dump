...
private:
	...
	void SettingsMenu()
	{
		...
		{
			HotsKey("Open Menu Button", u8"Кнопка открытия меню", MainSettings().MenuButton, u8"Задает кнопку открытия меню");
			X1Gui().Spacing();
			X1Gui().Separator();
			X1Gui().Spacing();
			...
			X1Gui().Separator();
			X1Gui().Spacing();
			DCheckBox(u8"Watermark", u8"Надпись", MainSettings().Watermark, u8"");
			X1Gui().Spacing();
			X1Gui().Separator();
			X1Gui().Spacing();
			...
			X1Gui().Separator();
			X1Gui().Spacing(); 
			DColorEdit("Color Background", u8"Цвет фона", MainSettings().BackgroundColor);
			DColorEdit("Color Title", u8"Цвет фона заголовка", MainSettings().TitleColor);
			DColorEdit("Color Text", u8"Цвет текста", MainSettings().TextColor);
			DColorEdit("Color Frame", u8"Цвет фрейма", MainSettings().FrameColor);
			DColorEdit("Color Button", u8"Цвет кнопок", MainSettings().ButtonColor);
			X1Gui().SameLine();
			DColorEdit("Color DisableButton", u8"Цвет выключенной кнопки", MainSettings().DisableButtonColor);
			DColorEdit("Color Line", u8"Цвет линий", MainSettings().LineColor);
			X1Gui().Spacing();
			X1Gui().Separator();
			X1Gui().Spacing();
			if (X1Gui().Button(!MainSettings().Russian ? XorStr("Reset Color") : XorStr(u8"Сбросить цвета")))
			{
...
public:
	...
	void Changers()
	{
		...
		if (ChangerMode == 0)
		{
			DCheckBox("Enable Skins", u8"Включение скинов", GP_Skins->SkinsEnable, u8"");
		}
		else if (ChangerMode == 1)
		{
			DCheckBox("Synchronization Inventory", u8"Синхронизация инвентаря", GP_Inventory->SkinsSyncEnable, u8"");
		}
	...
	void SelectWindow()
	{
		...
		VectorEx<const char*>tabs_rus = { u8"Визуалы",u8"Дополнительно",  u8"Радар", "Grenade Helper", u8"Ченжеры", u8"Настройки" };
#endif 
		...
		{
			X1Gui().SameLine(5);
			X1Gui().Text(XorStr("Expiration date:"));
			X1Gui().Spacing();
			X1Gui().SameLine(5);
