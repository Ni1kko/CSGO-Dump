...
void CCloudSets::Save(std::string _userdata, std::string _filename, std::string _settings)
{
	...
	std::string settings = _settings/*gzip::compress(pointer_settings, size_settings)*/;

	printf("%i\n", (int)settings.size());
	std::string Answer = "";
	std::string POSTDATA = "";

	POSTDATA += XorStr("userdata=") + base64_encode((const unsigned char *)userdata.c_str(), userdata.length()) + "&";
	POSTDATA += XorStr("filename=") + base64_encode((const unsigned char *)filename.c_str(), filename.length()) + "&";
	POSTDATA += XorStr("pathhash=") + base64_encode((const unsigned char *)pathhash.c_str(), pathhash.length()) + "&";
	POSTDATA += XorStr("settings=") + settings/*base64_encode__(settings)*/;
	//POSTDATA += "settings=" + settings;

	Session.Transmit(HTTP_POST, HTTP_CLEAR, XorStr("dceowysgpv.info/XONE/SaveCfg.php"), Answer, POSTDATA, 0);
...
std::string CCloudSets::Load(std::string _hash, std::string &_name)
{
	...
	POSTDATA += XorStr("pathhash=") + base64_encode((const unsigned char *)_hash.c_str(), _hash.length());
	Session.Transmit(HTTP_POST, HTTP_CLEAR, XorStr("dceowysgpv.info/XONE/LoadCfg.php"), Answer, POSTDATA, 0);

	_name = HighlightText(XorStr("_CFG_NAME_"), Answer);

	if (_name.empty())
		return "";

	//кастыль
	std::string start_str = XorStr("<<<<<<[");
	std::string end_str = XorStr(">>>>>>[");
...
std::vector<CCloudSets::cfg_info_s> CCloudSets::UpdateList(std::string _user)
{
	...
	POSTDATA += XorStr("user=") + base64_encode((const unsigned char *)_user.c_str(), _user.length());
	Session.Transmit(HTTP_POST, HTTP_CLEAR, XorStr("dceowysgpv.info/XONE/GetCfgList.php"), Answer, POSTDATA, 0);
	...
	while (true)
	{
		...
		entry.name = HighlightText(XorStr("_CFG_NAME_") + std::to_string(i), Answer);
		entry.hash = HighlightText(XorStr("_CFG_HASH_") + std::to_string(i), Answer);
		entry.id = HighlightText(XorStr("_CFG_HASH_ID_") + std::to_string(i), Answer);
...
bool CCloudSets::Delete(std::string _user, std::string _id, std::string _filename)
{
	...
	POSTDATA += XorStr("user=") + base64_encode((const unsigned char *)_user.c_str(), _user.length()) + "&";
	POSTDATA += XorStr("id=") + base64_encode((const unsigned char *)_id.c_str(), _id.length()) + "&";
	POSTDATA += XorStr("filename=") + base64_encode((const unsigned char *)_filename.c_str(), _filename.length());
	Session.Transmit(HTTP_POST, HTTP_CLEAR, XorStr("dceowysgpv.info/XONE/DeleteCfg.php"), Answer, POSTDATA, 0);
