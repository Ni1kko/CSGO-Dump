...
#define MV_WEAPON_TEXT_ICO XorStr("S")
//#define MV_AMMO_TEXT "[30/120]"
...
{
	//CRender::IRender* Draw = GP_Render;
	CGui* Draw = &X1Gui();
	//GetThis()
	GuiIO& g = X1Gui().GetIO();
	...
	//Settings::Esp::esp_HealthStyle

	//color_t NameColor = color_t(Esp::texHealth_Color[0], Esp::texHealth_Color[1], Esp::texHealth_Color[2], 1.f);
	//color_t WeaponColor = color_t(Esp::texWeapon_Color[0], Esp::texWeapon_Color[1], Esp::texWeapon_Color[2], 1.f);
	//color_t DistanceColor = color_t(Esp::texDistance_Color[0], Esp::texDistance_Color[1], Esp::texDistance_Color[2], 1.f);

	IsMousPress = g.LMPressDur > 0.0f;
	IsMousClick = g.LMClicked;

	/*if (!DisableMoveMainW && IsMousPress)
		IsBlockMove = true;
	if (IsBlockMove && !IsMousPress)
		IsBlockMove = false;*/
	...
	{
		//	DisableMoveMainW = true;
	}
	//else
	//	DisableMoveMainW = false;
	...
	if (!VisualBodyImage)
		D3DXCreateTextureFromFileEx(GP_Render->m_pDevice, (CGlobal::SystemDisk + XorStr("XONE\\Resources\\Images\\Body.png")).c_str(), 222, 579,
	...
	if (VisualBodyImage)
		...
		//ImGui::Image(VisualBodyImage, Vec2(IMAGE_X, IMAGE_Y));
	...
	if (Skeleton)
	{
		...
		//ImGui::SetCursorPos(Vec2(159.f, 213.f));
	...
	if (Health)
	{
		...
		{
			//ImGui::PushFont(GP_Render->SzFonts[TextHealthSize]);
	...
	if (Armor)
	{
		...
		{
			//ImGui::PushFont(GP_Render->SzFonts[TextArmorSize]);
	...
	if (Name)
	{
		//ImGui::PushFont(GP_Render->SzFonts[TextNameSize]);
	...
	if (Weapon)
	{
		...
		//MV_Weapon->Color1[0] = Esp::texWeapon_Color[0];
		//MV_Weapon->Color1[1] = Esp::texWeapon_Color[1];
		//MV_Weapon->Color1[2] = Esp::texWeapon_Color[2];
		...
		{
			//ImGui::PushFont(GP_Render->SzFontsIcon[TextWeaponSize]);
	...
	if (Ammo)
	{
		//ImGui::PushFont(GP_Render->SzFonts[TextAmmoSize]);
	...
	if (Distance)
	{
		//ImGui::PushFont(GP_Render->SzFonts[TextDistanceSize]);

		//MV_Distance->Color1[0] = Esp::texDistance_Color[0];
		//MV_Distance->Color1[1] = Esp::texDistance_Color[1];
		//MV_Distance->Color1[2] = Esp::texDistance_Color[2];
	...
	if (Money)
	{
	//	ImGui::PushFont(GP_Render->SzFonts[TextMoneySize]);
	...
	if (Defusing)
	{
		//ImGui::PushFont(GP_Render->SzFonts[TextDefusingSize]);
	...
	if (Scope)
	{
		//ImGui::PushFont(GP_Render->SzFonts[TextScopedSize]);
	...
	if (Flashing)
	{
		//ImGui::PushFont(GP_Render->SzFonts[TextFlashedSize]);
	...
	auto CalcVisOffset = [StartBoxPos](MoveStruct& Curt, bool IsText = false)
	{
		...
		{
			DisttX = ((StartBoxPos.x - Curt.CorrectPadding.x - Curt.Size.x) - (Curt.Position.x));
			DisttY = ((StartBoxPos.y + BoxSize_Y - Curt.Size.y + Curt.CorrectPadding.y) - (Curt.Position.y /*+ Curt.Size.y*/));
		}
	...
	//ImGui::SetCursorPos(Vec2(StartPos.x, StartPos.y + VIS_PREW_ZONE_Y + 3));
	//style.AntiAliasedLines = true;
	//style.AntiAliasedShapes = true;
	//style.CurveTessellationTol = 1.25;
