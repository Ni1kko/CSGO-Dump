...
string CGHelper::GetGName(WEAPON_ID id)
{
	if (id == WEAPON_ID::WEAPON_INCGRENADE)
		return !MainSettings().Russian ? "Molotov & Incendiary" : u8"Зажигательная граната или молотов";
	else if (id == WEAPON_ID::WEAPON_MOLOTOV)
		return !MainSettings().Russian ? "Molotov & Incendiary" : u8"Зажигательная граната или молотов";
	else if (id == WEAPON_ID::WEAPON_HEGRENADE)
		return !MainSettings().Russian ? "HE Granade" : u8"Осколочная граната";
	else if (id == WEAPON_ID::WEAPON_SMOKEGRENADE)
		return !MainSettings().Russian ? "Smoke" : u8"Дымовая граната";
	else if (id == WEAPON_ID::WEAPON_DECOY)
		return !MainSettings().Russian ? "Decoy" : u8"Ложная граната";
	else if (id == WEAPON_ID::WEAPON_FLASHBANG)
		return !MainSettings().Russian ? "Flashbang" : u8"Световая граната";
...
void CGHelper::AutoAddHelp(string _game_name)
{
	...
	if (pLocal)
	{
		...
		//entry.crossh_pos = BestCrosshair;
...
void CGHelper::AddHelp(string _game_name, GHInfo Info)
{
	...
	if (pLocal)
	{
		...
		{
			Message::Get().Start(!MainSettings().Russian ? XorStr("Already added!") : XorStr(u8"Уже добавлено!"));
			return;
		}
	...
	if (cur_map && pLocal)
	{
		...
		for (size_t i = 0; i < cur_map->helpers.size(); i++)
		{
			...
			if (CGlobal::WorldToScreen(InfoPos, InfoPosScreen))
			{
				...
				if (selected)
					GP_Render->DrawString(16, Vec2(InfoPosScreen.x, InfoPosScreen.y - 40), Color::White(), false, true, MainSettings().Russian ? u8"Выбрано" : "Selected");
			}
		...
		{		
			...
			else if (CrosshairStyle == 1)
			{
				/*struct line_s
				{
					...
					bool W2S(line_s &ent)
					{
						if (WorldToScreen(start, ent.start) && WorldToScreen(end, ent.end))
							return true;
			...
			if (ShowDiscp)
			{
				...
				//GP_IRender->render_Box(desc_pos.x, desc_pos.y, desc_size.x + 6, desc_size.y + 6, Color(MainCheatConfig.BackgroundColor.r(),
				//	MainCheatConfig.BackgroundColor.g(),
				//	MainCheatConfig.BackgroundColor.b(),
				//	100));
...
{
	std::ofstream o("C:\\XONE\\GrenadeHelper");
	nlohmann::json JO;
...
{
	string JCont = readFile("C:\\XONE\\GrenadeHelper");
