...
void CLegitAim::SetSelectedWeapon(bool MenuCheck)
{
	if (MenuCheck ? !CGlobal::IsGuiVisble : true)
	{
		...
		if (WeaponCustomTypes == 1) // standart
		{
			...
			{
			case WEAPON_TYPE_RIFEL: SelectedWeapon = 34; StandartSubSelected = 0; CheckAimCust = true; break;
			case WEAPON_TYPE_PISTOL: SelectedWeapon = 35; StandartSubSelected = 1; CheckAimCust = true;  break;
			case WEAPON_TYPE_SHOTGUN: SelectedWeapon = 36; StandartSubSelected = 2; CheckAimCust = true;  break;
			case WEAPON_TYPE_SNIPER: SelectedWeapon = 37; StandartSubSelected = 3; CheckAimCust = true;  break;
...
void CLegitAim::Draw()
{
	...
	//	float dur = chrono::duration<float>(TimeEndWork - TimeStartWork).count();

		//GP_Render->DrawString(15, Vec2(500, VTab), Color::Yellow(),  (XorStr("fTickTimeWork: ") + to_string(dur) + XorStr(" s->mcs ") + to_string(dur * 1000000.f)).c_str()); VTab += VTabSz;
		...
		GP_Render->DrawString(15, Vec2(500, VTab), Color::Yellow(),  (XorStr("bIsSniper & Type: ") + to_string(IsSniper) + XorStr(" : ") + to_string(IsSniperClass)).c_str()); VTab += VTabSz;
		GP_Render->DrawString(15, Vec2(500, VTab), Color::Yellow(),  (XorStr("bIsShotgun: ") + to_string(IsShotgun)).c_str()); VTab += VTabSz;

		GP_Render->DrawString(15, Vec2(500, VTab), Color::Yellow(),  (XorStr("bFastZoom & bZoomed: ") + to_string(FastZoom) + XorStr(" : ") + to_string(IsZoomed)).c_str()); VTab += VTabSz;
	...
	if (ShowEntitysName)
	{
		for (int EntIndex = 0; EntIndex < I::EntityList()->GetHighestEntityIndex(); EntIndex++)
		{
			...
			if (true/*string(pModelName).find("crosshair") != string::npos || string(pModelName) == "*171"*/)
			{
	...
	if (ShowBacktrack && Weapons[GetWeap(SelectedWeapon)].Backtrack && Weapons[GetWeap(SelectedWeapon)].BacktrackTicks)
	{
		...
		//	Color BackColorVisble = Color((int)(Settings::Aimbot::BacktrackColorVisble[0] * 255.f), (int)(Settings::Aimbot::BacktrackColorVisble[1] * 255.f), (int)(Settings::Aimbot::BacktrackColorVisble[2] * 255.f), Settings::Aimbot::BackTrackAlpha);
...
{
	//if (Debug)
	//	TimeStartWork = chrono::high_resolution_clock::now();
	...
	{
		//if (Debug)
		//	TimeEndWork = chrono::high_resolution_clock::now();
	...
	//bool EnableAimBacktrack = Settings::Aimbot::AimPlusBacktrack && Settings::Aimbot::aim_Backtrack && !FindedTarg && iBackTrackbestTarget != -1 && iBackTrackBestSimTime != -1;
	...
	//bool shotgun_check = false/*CGlobal::GWeaponType == WEAPON_TYPE_SHOTGUN*/;
	//if (CGlobal::GWeaponType == WEAPON_TYPE_SHOTGUN)
	//{
	//	shotgun_check = CanShoot();
	//}
	//else
	//	shotgun_check = true;


	if ((((IsAttack || (AutoPistol && IsPistol && PistolPreFire)) && FindedTarg) || FFDelayOCActive) && true/*shotgun_check*/)
	{
...
bool CLegitAim::CheckOpportWork(CUserCmd* pCmd)
{
	...
	//pLocalPlayer = (CBaseEntity*)I::EntityList()->GetClientEntityFromHandle(pLocalPlayer->GetObserverTarget());
	...
	else if (FaceItMode && !CGlobal::IsGuiVisble)
	{
		...
		else
		{
			...
			if (LpCmd)
			{
				...
				//	ScreenTargPoint.x = (ScreenCenterX + ((((960.f / GFovView) * (TargetAng.y - LpCmd->viewangles.y))) * -1.f));
				//	ScreenTargPoint.y = (ScreenCenterY + (((960.f / GFovView) * (TargetAng.x - LpCmd->viewangles.x))));
...
Vector CLegitAim::PlayerAnglToScreen(Vector Angl)
{
	if (LpCmd)
	{
		...
		//if (viewangles.y < 0 && fabsf(viewangles.y) > 90.f)
		//{
		//	viewangles.y = 360.f - fabsf(viewangles.y);
		//}

		//if (viewangles.x < 0 && fabsf(viewangles.x) > 90.f)
		//{
		//	viewangles.x = 360.f - fabsf(viewangles.y);
		//}


		//if (Angl.x >= 180)
		//{
		//	Angl.x = (180.f - (180.f - (360.f - Angl.x))) * -1.f;
		//}

		//if (Angl.y >= 180)
		//{
		//	Angl.y = (180.f - (180.f - (360.f - Angl.y))) * -1.f;
		//}
	...
	if (BestFov > 0.1f)
	{
		...
		if (!EntityAim)
		{
			...
			else
			{
				...
				//else
					//_BestHitBoxPos = headPositions[iBackTrackbestTarget][iBackTrackBestSimTime].AimBackTrackHitBoxes[_HitBox];
			}
	...
	if (CanFire(MyAngle, pCmd, pTriggerBestTarget, TriggerBestIdx, pLocalPlayer, Weapons[GetWeap(SelectedWeapon)].TriggerAllHitboxes))
	{
		...
		if (CanFireChanse)
		{
			...
			if (!delay)
			{
				...
				if (!(pCmd->buttons & IN_ATTACK))
				{
					//if(Settings::Aimbot::weapon_aim_settings[iWeaponID].TriggerFastZoom)
					//	pCmd->buttons |= IN_ZOOM;
			...
			if (!entity->IsDead())
			{
				...
				if (ShowBacktrack && Weapons[GetWeap(SelectedWeapon)].BacktrackTicks)
				{
					...
					for (int h(0); h < 18; h++)
					{
						...
						/*bfg.VisbleBones[h] = entity->IsVisibleHitBox(g_pPlayers->GetLocal()->m_pEntity, g_pPlayers->GetLocal()->m_pEntity->GetEyePosition(), vHitboxSkeletonArrayBuf[h][0], true, false) ||
							entity->IsVisibleHitBox(g_pPlayers->GetLocal()->m_pEntity, g_pPlayers->GetLocal()->m_pEntity->GetEyePosition(), vHitboxSkeletonArrayBuf[h][1], true, false);*/
					}
				}
				...
				if (Weapons[GetWeap(SelectedWeapon)].BacktrackTicks)
				{
					...
					/*if (Settings::Aimbot::AimPlusBacktrack)
					{
						for (int j(0); j < 20; j++)
						{
							headPositions[i][pCmd->command_number % MAXBACKTRACKTICKS].AimBackTrackHitBoxes[j] = entity->GetHitboxPosition(j);
						}
					}*/
