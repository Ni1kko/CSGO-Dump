...
public:
	...
	{
		string WeaponName = XorStr("<Default>");
...
#define CHECK_VAR(l) if(l != src.##l) return false;
	...
	void ReinitWeapons()
	{
		...
		Buf.WeaponName = XorStr("Rifles");
		Weapons.push_back(Buf);
		Buf.WeaponName = XorStr("Snipers");
		Weapons.push_back(Buf);
		Buf.WeaponName = XorStr("Shotguns");
		Weapons.push_back(Buf);
		Buf.WeaponName = XorStr("Pistols");
		Weapons.push_back(Buf);
		Buf.WeaponName = XorStr("AllWeapon");
		Weapons.push_back(Buf);
	}
...
private:
	...
	//SuperTime TimeStartWork;
	//SuperTime TimeEndWork;
