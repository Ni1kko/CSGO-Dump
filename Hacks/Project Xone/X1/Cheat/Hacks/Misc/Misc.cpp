	...
	if (Enable && CGlobal::IsGameReady && !CGlobal::FullUpdateCheck)
	{
		if (plocal)
		{
			...
			if (ClanTagChanger)
			{
				...
				{
				case 0: NewClan = XorStr("XONE > All"); break;
				case 1: NewClan = XorStr("[VALV\xE1\xB4\xB1]"); break;
				case 2:
				{
					switch (int((float)plocal->GetTickBase() * I::GlobalVars()->interval_per_tick) % 8)
					{
					case 0: NewClan = XorStr("----X"); break;
					case 1: NewClan = XorStr("---XO"); break;
					case 2: NewClan = XorStr("--XON"); break;
					case 3: NewClan = XorStr("XONE"); break;
					case 4: NewClan = XorStr("ONE--"); break;
					case 5: NewClan = XorStr("NE---"); break;
					case 6: NewClan = XorStr("E----"); break;
					case 7: NewClan = XorStr("------"); break;
					}
					break;
				}
				case 3:
				{
					...
					{
					case 0: NewClan = XorStr("8==>()"); break;
					case 1: NewClan = XorStr("  8==()"); break;
					case 2: NewClan = XorStr("    8=()"); break;
					case 3: NewClan = XorStr("     8()"); break;
					case 4: NewClan = XorStr("    8=()"); break;
					case 5: NewClan = XorStr("  8==()"); break;
					}
					break;
				}
				default: break;
			...
			if (ChatSpam && ChatSpamStart)
			{
				...
				{
				case 0: chatspam = { XorStr("vk.com/xone_software | XONE | BEST CHEAT") }; break;
...
void CMisc::AutoAcceptEmit()
{
	...
	{
		static auto SetLocalPlayerReadyFn = reinterpret_cast<bool(__stdcall*)(const char*)>(CSX::Memory::FindPattern(GetClientDll(), XorStr("55 8B EC 83 E4 F8 8B 4D 08 BA ? ? ? ? E8 ? ? ? ? 85 C0 75 12"), 0));
		//HWND Hwnd;
		...
		//		DWORD color = GetPixel(hdc, lprect.right / 2, lprect.bottom / 2);
...
{
	/*if (AutoAccept && !Panorama && !strcmp(pszSoundName, XorStr("!UI/competitive_accept_beep.wav")))
	{
		IsReadyCallBackFn IsReadyCallBack = 0;

		if (!IsReadyCallBack)
			IsReadyCallBack = (IsReadyCallBackFn)(CSX::Memory::FindPattern(GetClientDll(), XorStr("55 8B EC 83 E4 F8 83 EC 08 56 8B 35 ? ? ? ? 57 83 BE 8C 00 00 00 02"), 0));
...
void CMisc::ShowSpectatorList()
{
	...
	if (X1Gui().Begin(XorStr("Spectator List"), GWF_Window, old_cursor_pos, Vec2(0, 0)))
	{
...
{
	SoundList.clear();
	string SoundsDir = CGlobal::SystemDisk + XorStr("XONE\\Resources\\Sounds\\*.wav");
...
void CHitListener::Draw()
{
	if (GP_Misc)
	{
		...
		if (GP_Misc->HitTrace)
		{
			if (!GP_Misc->HitImpacts.empty())
			{
				for (size_t i(0); i < GP_Misc->HitImpacts.size(); i++)
				{
					if (!CGlobal::IsGameReady)
						break;

					CMisc::HitImpact_t &ImpEntry = GP_Misc->HitImpacts[i];

					if (ImpEntry.EndTime >= I::GlobalVars()->curtime)
					{
						//п–асчет
						float alpha_perc = (255.f * ((((ImpEntry.EndTime - I::GlobalVars()->curtime) * 100.f) / HIT_TRACE_SHOW_TIME) / 100.f));

						Vector ScreenPosition;
						Vector HeadScreenPosition;

						Color MarkColor(255, 255, 255, alpha_perc);
						Color LineColor(GP_Misc->HitTraceColor[0], GP_Misc->HitTraceColor[1], GP_Misc->HitTraceColor[2], alpha_perc);

						if (CGlobal::WorldToScreen(ImpEntry.Pos, ScreenPosition) && CGlobal::WorldToScreen(ImpEntry.MyHeadPos, HeadScreenPosition))
						{
							HitMark(5, 1.3f, ScreenPosition.x, ScreenPosition.y, MarkColor);
							GP_Render->DrawLine(HeadScreenPosition.x, HeadScreenPosition.y, ScreenPosition.x, ScreenPosition.y, LineColor, 2.f);
						}
					}
					else
					{
						GP_Misc->HitImpacts.erase(GP_Misc->HitImpacts.begin() + i);
					}
				}
			}
		}
		...
		if (GP_Misc->HitSound)
		{
			if (PlaySound)
			{
				if (!GP_Misc->SoundList.empty() && GP_Misc->HitSoundStyle >= 0 && GP_Misc->HitSoundStyle < (int)GP_Misc->SoundList.size())
					FastCall::G().t_PlaySoundA((CGlobal::SystemDisk + XorStr("XONE\\Resources\\Sounds\\") + GP_Misc->SoundList[GP_Misc->HitSoundStyle]).c_str(),
