...
void Hook_SetViewModelSequence(const CRecvProxyData *pDataConst, void *pStruct, void *pOut)
{
	...
	if (pViewModel)
	{
		...
		if (pOwner && pOwner->EntIndex() == I::Engine()->GetLocalPlayer())
		{
			...
			else if (!strcmp(szModel, XorStr("models/weapons/v_knife_ursus.mdl")))
			{
				switch (m_nSequence)
				{
				case SEQUENCE_DEFAULT_DRAW:
			...
			else if (!strcmp(szModel, XorStr("models/weapons/v_knife_stiletto.mdl")))
			{
				switch (m_nSequence)
				{
				case SEQUENCE_DEFAULT_LOOKAT01:
			...
			else if (!strcmp(szModel, XorStr("models/weapons/v_knife_widowmaker.mdl")))
			{
				switch (m_nSequence)
				{
				case SEQUENCE_DEFAULT_LOOKAT01:
...
{
	if (CGlobal::IsGameReady && GP_Skins /*&& !CheatUnload*/)
...
auto CSkins::initialize_kits() -> void
{
	...
	const auto sig_address = CSX::Memory::FindPattern(GetClientDll(), XorStr("\xE8\x00\x00\x00\x00\xFF\x76\x0C\x8D\x48\x04\xE8"), XorStr("x????xxxxxxx"), 0);
	...
	// Dump sticker kits
	{
		const auto sticker_sig = CSX::Memory::FindPattern(GetClientDll(), XorStr("\x53\x8D\x48\x04\xE8\x00\x00\x00\x00\x8B\x4D\x10"), XorStr("xxxxx????xxx"), 4);
