...
void CSkins::Menu()
{
	...
	auto SkinParams = [&](SkinSettings &Item) -> void
	{
		...
		SliderFloats("Seed", u8"Яркость", Item.seed, 0.f, 2.f,"");
		...
		X1Gui().InputText(!MainSettings().Russian ? "Name##34343": u8"Имя##34343", Item.custom_name, 128);
	...
	VectorEx<const char*> SkinArrRus = { lolc(u8"Оружие"), lolc(u8"Ножи"),lolc(u8"Перчатки") };
	...
	if (SkinSettingsMode == 0)
	{
		...
		if (X1Gui().Button(true ? XorStr("Update") : XorStr(u8"Обновить"), Vec2(long_item_w, 22)))
		{
			UpdateSkins();
			Message::Get().Start(!MainSettings().Russian ? XorStr("Updated!") : XorStr(u8"Обновлено!"));
		}
			...
			X1Gui().BeginGroup();
			{
				...
				else if (WItem->Skin.skins_mode == 1)
				{
					...
					else
					{
						...
						X1Gui().Text(true ? XorStr("Loading skins...") : XorStr(u8"Загрузка скинов..."));
						X1Gui().ListBoxFooter();
					}
		...
		{
			DCheckBox("Stikers", u8"Стикеры##2432134", WeaponNames[SelectedWeapon].Skin.striker_enable, u8"");
			if (WeaponNames[SelectedWeapon].Skin.striker_enable)
			{
				...
				else if (StikersMode == 1)
				{
					...
					{
						X1Gui().Text(true ? XorStr("Loading stickers...") : XorStr(u8"Загрузка стикеров..."));
					}
				}
	...
	else if (SkinSettingsMode == 1)
	{
		...
		VectorEx<const char*> KnfTeamArrRus = { lolc(u8"КТ"),lolc(u8"Т") };
	...
	else if (SkinSettingsMode == 2)
	{
		...
		VectorEx<const char*> glvTeamArrRus = { lolc(u8"КТ"),lolc(u8"Т") };
