...
bool CLicense::Check()
{
	...
	auto LicCh = [&]()
	{
		try
		{
			...
			string ServerBuffer = "";
			string zUrl = XorStr("/XONE/Chel.php");
			zUrl += XorStr("?K=") + OCrypt(fStr) + XorStr("&CTP=") + OCrypt(CHEAT_TYPE);
			...
			if (ServerBuffer.find(XorStr("MSQL:")) != -1)
			{
			...
			if (ErrorStr == XorStr("NULL")) { ADD_LOG("License: NULL\n"); return false; }
			if (ErrorStr == XorStr("EMPT")) { ADD_LOG("License: EMPT\n"); return false; }

			string ServerMD5 = HighlightText(XorStr("_MD5"), ServerBuffer);
			string ServerEndDate = HighlightText(XorStr("_END_D"), ServerBuffer);
			string ServerCheatType = HighlightText(XorStr("_CHET"), ServerBuffer);
			string ServerLogin = HighlightText(XorStr("_LOG"), ServerBuffer);
			string ServerBan = HighlightText(XorStr("_BAN"), ServerBuffer);
			string ServerLicFreez = HighlightText(XorStr("_FREZ"), ServerBuffer);

			if (ServerLogin != MainSettings().UserName) { return false; }
			//if (ServerCheatType != CHEAT_TYPE) { ADD_LOG("License: ErrType\n"); return false; }
			if (ServerBan == XorStr("1")) { ADD_LOG("License: Ban\n"); return false; }
...
void CLicense::GetCheatUser()
{
	...
	if (file)
	{
		...
		MainSettings().key = HighlightText(XorStr("_MD5_"), MainBuffer);
		MainSettings().UserName = HighlightText(XorStr("_USER_NAME_"), MainBuffer);
		MainSettings().EndDate = HighlightText(XorStr("_DATA_USER_"), MainBuffer);
		MainSettings().Days = HighlightText(XorStr("_END_USER_"), MainBuffer);
		MainSettings().id = HighlightText(XorStr("_ID_USER_"), MainBuffer);
