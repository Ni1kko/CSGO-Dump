...
bool TraceToExitalt(Vector& end, C_Trace& tr, Vector start, Vector vEnd, C_Trace* trace)
{
	...
	static TraceToExitFn TraceToExit = (TraceToExitFn)Utils::FindSignature("client.dll", "55 8B EC 83 EC 30 F3 0F 10 75");
...
{
	auto data = FireBulletData(Globals::LocalPlayer->GetEyePosition(), Globals::LocalPlayer);
	...
	if (SimulateFireBullet(Globals::LocalPlayer, Globals::LocalPlayer->GetActiveWeapon(), data))
...
bool Autowall::CanHitFloatingPoint(const Vector &point, const Vector &source)
{
	...
	FireBulletData data = FireBulletData(source, Globals::LocalPlayer);

	Vector angles = g_Math.CalcAngle(data.src, point);
	g_Math.AngleVectors(angles, &data.direction);
	VectorNormalize(data.direction);

	C_BaseCombatWeapon *pWeapon = (C_BaseCombatWeapon*)Globals::LocalPlayer->GetActiveWeapon();
	...
	TraceLine(data.src, end, mask_shot | contents_hitbox, Globals::LocalPlayer, &data.enter_trace);
...
bool Autowall::CanWallbang(float &dmg)
{
	...
	FireBulletData data = FireBulletData(Globals::LocalPlayer->GetEyePosition(), Globals::LocalPlayer);
//	data.filter = trace;
