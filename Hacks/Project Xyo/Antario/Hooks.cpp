...
void SetViewModelSequence(const CRecvProxyData *pDataConst, void *pStruct, void *pOut) {
	...
	if (pViewModel) {
		...
		if (pOwner && pOwner->EntIndex() == g_pEngine->GetLocalPlayer()) {
			...
			else if (!strcmp(szModel, "models/weapons/v_canis_ursus.mdl")) {
				switch (m_nSequence)
				{
				case SEQUENCE_DEFAULT_DRAW:
					m_nSequence = RandomInt(SEQUENCE_BUTTERFLY_DRAW, SEQUENCE_BUTTERFLY_DRAW2);
...
const char* getKnifeModel(bool viewmodel = true)
{
	...
	case 14:
		...
		break;
	case 15:
		return viewmodel ? "models/weapons/v_knife_cord.mdl" : "models/weapons/v_knife_cord.mdl";
		break;
	case 16:
		return viewmodel ? "models/weapons/v_knife_canis.mdl" : "models/weapons/v_knife_canis.mdl";
		break;
	case 17:
		return viewmodel ? "models/weapons/v__knife_outdoor.mdl" : "models/weapons/v__knife_outdoor.mdl";
		break;
	case 18:
		return viewmodel ? "models/weapons/v__knife_skeleton.mdl" : "models/weapons/v__knife_skeleton.mdl";
		break;
	default:
		return "";
		break;
...
static T* find_hud_element(const char* name) {
	static auto fn = *reinterpret_cast<DWORD**>(pattern_scan(GetModuleHandleA("client.dll"), ("B9 ? ? ? ? E8 ? ? ? ? 8B 5D 08")) + 1);

	static auto find_hud_element = reinterpret_cast<DWORD(__thiscall*)(void*, const char*)>(pattern_scan(GetModuleHandleA("client.dll"), ("55 8B EC 53 8B 5D 08 56 57 8B F9 33 F6 39 77 28")));
	return (T*)find_hud_element(fn, name);
}

void KnifeApplyCallbk()
{
	static auto fn = reinterpret_cast<std::int32_t(__thiscall*)(void*, std::int32_t)>(pattern_scan(GetModuleHandleA("client.dll"), ("55 8B EC 51 53 56 8B 75 08 8B D9 57 6B FE 2C")));
...
IMaterial *create_material(bool shade, bool wireframe, bool ignorez, int rimlight_boost = 0) {
	...
	auto init_key_values = [](KeyValuesv2 *keyvalues, const char *key_name) -> void {
		...
		static auto pThis = *reinterpret_cast<DWORD**>(Utils::FindSignature("client.dll", "B9 ? ? ? ? E8 ? ? ? ? 8B 5D 08") + 1);
		if (!InitKeyValuesEx)
			InitKeyValuesEx = *reinterpret_cast<InitKeyValues_t>(Utils::FindSignature("client.dll", "55 8B EC 51 33 C0 C7 45"));
	...
	auto load_from_buffer = [](KeyValuesv2 *key_values, const char *resource_name, const char *buf, void *file_sys = nullptr, const char *path_id = nullptr, void *eval_sym_proc = nullptr, void *unk = nullptr) -> void {
		...
		if (!LoadFromBufferEx)
			LoadFromBufferEx = *reinterpret_cast<LoadFromBuffer_t>(Utils::FindSignature("client.dll", "55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89"));
