...
{
	//static IDirect3DDevice9* d3dDevice  = **(IDirect3DDevice9 * **)(Utils::FindSignature("shaderapidx9.dll", "A1 ? ? ? ? 50 8B 08 FF 51 0C") + 1);
	//D3DDEVICE_CREATION_PARAMETERS params;

	//if (FAILED(d3dDevice->GetCreationParameters(&params)))
	//	throw std::runtime_error("[InputSys] GetCreationParameters failed.");

	m_hTargetWindow = g_Hooks.hCSGOWindow;
...
LRESULT __stdcall InputSys::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (ImGui_ImplDX9_WndProcHandler(hWnd, msg, wParam, lParam) && g_Settings.bMenuOpened)
		return true;
