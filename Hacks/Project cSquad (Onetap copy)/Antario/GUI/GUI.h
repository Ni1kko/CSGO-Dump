...
namespace ui
{
    ...
    struct MenuStyle
    {
        ...
        Color colHover        = { 70, 70, 70, 30 };		/*- Color applied on the obj when mouse hovers above it -*/
        Color colSectionOutl  = { 30, 30, 30, 0 };	    /*- Color of the section outline                        -*/
        Color colSectionFill  = { 0, 0, 0, 0 };         /*- Color filling the section bounds                    -*/
        Color colCheckboxFill = { 50, 50, 50, 255 };    /*- Color of the first gradient color of the checkbox   -*/
		Color colMenuControlFill = { 180, 130, 200, 255 };
        Color colMenuStyle    = { 35, 35, 35, 255 };    /*- Color of the menu style controls                    -*/
        Color colText         = { 255, 255, 255, 255 }; /*- Color of the text inside the main window            -*/
        Color colHeader       = { 50, 50, 50, 230 };    /*- Color of the header background                      -*/
        Color colHeaderText   = { 130, 130, 130, 255 }; /*- Color of the text inside the header strip           -*/
    ...
    public:
		...
		virtual void AddCheckBoxUntrusted(const std::string& strSelectableLabel, bool * bValue);
		...
		virtual void AddColor(const std::string& strLabelName, Color* colorOutput);
		virtual void AddMulti(const std::string& strSelectableLabel, bool * bEnabled, std::vector<std::string> vecBoxOptions);
		void AddText(const std::string & strLabel);
    protected:
	...
	class CheckboxUntrusted : public Control
	{
	public:
		CheckboxUntrusted(const std::string& strLabel, bool *bValue, ObjectPtr pParent);
		void Render() override;
		void SetupPositions() override;
		bool HandleMouseInput(UINT uMsg, WPARAM wParam, LPARAM lParam) override;
	...
	class MultiBox : public Control
	{
	public:
		MultiBox(const std::string& strLabel, std::vector<std::string> vecBoxOptions, bool* bEnabled, ObjectPtr pParent);
		void Initialize()         override;
		void Render()             override;
		bool HandleMouseInput(UINT uMsg, WPARAM wParam, LPARAM lParam) override;
		void SetupPositions()     override;
	...
	public:
		...
		bool UpdateData();
		bool CanHaveFocus() const override { return true; }
		void OnFocusOut()         override { this->bCopyPaste = false; this->bIsActive = false; };
		Color* colOutput = nullptr;
	private:
		int    iPickerID = -1;
		int    idHovered;
		bool   bCopyPaste = false;
		bool   bIsActive = false;
		int    iPickerSize;
		int    iHueBarSize;
		float  colHSV[3];
		std::vector<std::string> vecSelectables;
	...
	{
	public:
		Text(const std::string& strLabel, ObjectPtr pParent);
		void Render()         override;
		void SetupPositions() override;
	private:
		SRect rcBox;
