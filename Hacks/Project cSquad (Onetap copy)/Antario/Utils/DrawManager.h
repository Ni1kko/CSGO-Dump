...
public:
	...
	void InvalidateDeviceObjects()
	{
		try
		{
			...
			pFontName->InvalidateDeviceObjects();
		}
		catch (const HRESULT& hr)
		{
	...
	void InitDeviceObjects(LPDIRECT3DDEVICE9 pDevice)
	{
		...
		try
		{
			...
			pFontName->InitDeviceObjects(pDevice);
			pFontName->RestoreDeviceObjects();
		}
		catch (const HRESULT& hr)
		{
	...
	std::shared_ptr<CD3DFont> pFontName;
};
