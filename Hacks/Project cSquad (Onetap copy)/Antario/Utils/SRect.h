...
public:
	...
	constexpr SPoint Pos2()    const { return SPoint(left + 2, bottom); }
	constexpr SPoint Pos3()    const { return SPoint(right, bottom); }
    constexpr SPoint Mid()    const { return SPoint((left + right) / 2, (top + bottom) / 2); }
	constexpr SPoint Button()    const { return SPoint((left + 10), (top + bottom) / 2); }
