...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags) {
	...
	ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(child_bb.x + 1, child_bb.y + 2), ImVec2(child_bb.x + ImGui::GetWindowWidth() - 1, child_bb.y + 9), ImGui::GetColorU32(ImVec4(g_Settings.cMenuCol[0], g_Settings.cMenuCol[1], g_Settings.cMenuCol[2], 255)), 3.2f);

	//ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x + ImGui::GetWindowWidth(), child_bb.y + 4), ImColor(menur, menug, menub, 255), 3.3f);
	//ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(child_bb.x, child_bb.y + 3), ImVec2(child_bb.x + ImGui::GetWindowWidth(), child_bb.y + 7), ImColor(menur, menug, menub, 255), ImColor(menur, menug, menub, 255), ImColor(menur, menug, menub, 255), ImColor(menur, menug, menub, 255));
	parent_window->DrawList->AddText(child_bb + ImVec2(0, -15), ImColor(menur, menug, menub), name);
...
bool ImGui::ButtonExT(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border, ImColor clr) {
	...
	if (page == in) {
		...
		PushStyleColor(ImGuiCol_Text, ImVec4(114 / 255.f, 111 / 255.f, 152 / 255.f, 1));
		RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.5, 0.5), &bb);
		PopStyleColor();
	}
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power) {
	...
	if (hovered)
	{
		//window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - ImGui::CalcTextSize(value_buf).x, frame_bb.Min.y - 3), ImVec2(frame_bb.Max.x - ImGui::CalcTextSize(value_buf).x + 30, frame_bb.Min.y - 17), GetColorU32(ImVec4(74 / 255.f, 87 / 255.f, 109 / 255.f, 1.0f)) , style.FrameRounding);
		RenderText(ImVec2(frame_bb.Max.x - frame_bb.Max.x / 2 - ImGui::CalcTextSize(value_buf).x, frame_bb.Min.y - 17), value_buf, value_buf_end);
	}
	// RenderTextClipped( frame_bb.Min - ImVec2( 0, 17 ), frame_bb.Max - ImVec2( 0, 14 ), value_buf, value_buf_end, NULL, ImVec2( 1.f, 1.f ) );

	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Min.x - 70, frame_bb.Min.y - 7), label);
...
bool ImGui::Checkbox(const char* label, bool* v) {
	...
	window->DrawList->AddRect(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(74 / 255.f, 87 / 255.f, 109 / 255.f, 1.0f)), style.FrameRounding);

	if (*v) {
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(74 / 255.f, 87 / 255.f, 109 / 255.f, 1.0f)), style.FrameRounding); // Main gradient.
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.1f)), style.FrameRounding);
	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), style.FrameRounding);
	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(58 / 255.f, 62 / 255.f, 71 / 255.f, 1.0f)), style.FrameRounding);
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(74 / 255.f, 87 / 255.f, 109 / 255.f, 1.0f)), style.FrameRounding);
